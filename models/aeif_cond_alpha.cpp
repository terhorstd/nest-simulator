...
template <> void RecordablesMap<aeif_cond_alpha>::create() {
...
extern "C" int nest::aeif_cond_alpha_dynamics(double, const double y[],
                                              double f[], void *pnode) {
  // a shorthand
...
nest::aeif_cond_alpha::State_::State_(const Parameters_ &p) : r_(0) {
  y_[0] = p.E_L;
...
nest::aeif_cond_alpha::State_::State_(const State_ &s) : r_(s.r_) {
  for (size_t i = 0; i < STATE_VEC_SIZE; ++i) {
    y_[i] = s.y_[i];
  }
}

nest::aeif_cond_alpha::State_ &nest::aeif_cond_alpha::State_::
...
void nest::aeif_cond_alpha::update(Time const &origin, const long from,
                                   const long to) {
