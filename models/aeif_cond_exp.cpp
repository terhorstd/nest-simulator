 ...
 */
template <> void RecordablesMap<aeif_cond_exp>::create() {
...
extern "C" int nest::aeif_cond_exp_dynamics(double, const double y[],
                                            double f[], void *pnode) {
  // a shorthand
  ...
  f[S::V_M] = is_refractory
                  ? 0.
                  : (-node.P_.g_L * (V - node.P_.E_L) + I_spike - I_syn_exc -
                     I_syn_inh - w + node.P_.I_e + node.B_.I_stim_) /
                        node.P_.C_m;
...
nest::aeif_cond_exp::State_::State_(const Parameters_ &p) : r_(0) {
  y_[0] = p.E_L;
...
nest::aeif_cond_exp::State_::State_(const State_ &s) : r_(s.r_) {
  for (size_t i = 0; i < STATE_VEC_SIZE; ++i) {
    y_[i] = s.y_[i];
  }
}

nest::aeif_cond_exp::State_ &nest::aeif_cond_exp::State_::
...
void nest::aeif_cond_exp::State_::set(const DictionaryDatum &d,
                                      const Parameters_ &) {
...
void nest::aeif_cond_exp::update(const Time &origin, const long from,
                                 const long to) {
