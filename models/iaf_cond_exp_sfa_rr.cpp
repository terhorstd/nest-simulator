...
template <> void RecordablesMap<iaf_cond_exp_sfa_rr>::create() {
...
extern "C" inline int nest::iaf_cond_exp_sfa_rr_dynamics(double,
                                                         const double y[],
                                                         double f[],
                                                         void *pnode) {
  // a shorthand
...
nest::iaf_cond_exp_sfa_rr::State_::State_(const Parameters_ &p) : r_(0) {
  y_[V_M] = p.E_L;
...
nest::iaf_cond_exp_sfa_rr::State_::State_(const State_ &s) : r_(s.r_) {
  for (size_t i = 0; i < STATE_VEC_SIZE; ++i) {
    y_[i] = s.y_[i];
  }
}

nest::iaf_cond_exp_sfa_rr::State_ &nest::iaf_cond_exp_sfa_rr::State_::
...
void nest::iaf_cond_exp_sfa_rr::update(Time const &origin, const long from,
                                       const long to) {
