...
nest::music_cont_out_proxy::Parameters_::Parameters_()
    : interval_(Time::ms(1.0)), port_name_("cont_out"), record_from_(),
      target_gids_() {}

nest::music_cont_out_proxy::Parameters_::Parameters_(const Parameters_ &p)
    : interval_(p.interval_), port_name_(p.port_name_),
      record_from_(p.record_from_), target_gids_(p.target_gids_) {
  interval_.calibrate();
}

nest::music_cont_out_proxy::State_::State_()
    : published_(false), port_width_(0) {}

nest::music_cont_out_proxy::State_::State_(const State_ &s)
    : published_(s.published_), port_width_(s.port_width_) {}

nest::music_cont_out_proxy::Buffers_::Buffers_()
    : has_targets_(false), data_() {}

nest::music_cont_out_proxy::Buffers_::Buffers_(const Buffers_ &b)
    : has_targets_(b.has_targets_), data_(b.data_) {}

/* ----------------------------------------------------------------
 * Parameter extraction and manipulation functions
 * ---------------------------------------------------------------- */

void nest::music_cont_out_proxy::Parameters_::get(DictionaryDatum &d) const {
...
void nest::music_cont_out_proxy::Parameters_::set(const DictionaryDatum &d,
                                                  const Node &self,
                                                  const State_ &state,
...
void nest::music_cont_out_proxy::State_::get(DictionaryDatum &d) const {
...
nest::music_cont_out_proxy::music_cont_out_proxy(const music_cont_out_proxy &n)
    : DeviceNode(n), P_(n.P_), S_(n.S_), B_(n.B_) {}

void nest::music_cont_out_proxy::init_state_(const Node & /* np */) {}

void nest::music_cont_out_proxy::init_buffers_() { B_.data_.clear(); }

void nest::music_cont_out_proxy::finalize() {}

nest::port nest::music_cont_out_proxy::send_test_event(Node &target,
                                                       rport receptor_type,
                                                       synindex, bool) {
  ...
  if (S_.published_ == false) {
    ...
    for (t = P_.target_gids_.begin(); t != P_.target_gids_.end(); ++t) {
      ...
      if (kernel().node_manager.is_local_gid(*t)) {
        kernel().connection_manager.connect(get_gid(), *t, dummy_params,
                                            synmodel_id);
      }
    }
...
void nest::music_cont_out_proxy::get_status(DictionaryDatum &d) const {
...
void nest::music_cont_out_proxy::update(Time const &origin, const long from,
                                        const long) {
...
void nest::music_cont_out_proxy::handle(DataLoggingReply &reply) {
