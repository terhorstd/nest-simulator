...
nest::weight_recorder::weight_recorder()
    : DeviceNode(), device_(*this, RecordingDevice::WEIGHT_RECORDER, "csv",
                            true, true, true, true),
      user_set_precise_times_(false), P_() {}

nest::weight_recorder::weight_recorder(const weight_recorder &n)
    : DeviceNode(n), device_(*this, n.device_),
      user_set_precise_times_(n.user_set_precise_times_), P_(n.P_) {}

nest::weight_recorder::Parameters_::Parameters_() : senders_(), targets_() {}

nest::weight_recorder::Parameters_::Parameters_(const Parameters_ &p)
    : senders_(p.senders_), targets_(p.targets_) {}

void nest::weight_recorder::Parameters_::get(DictionaryDatum &d) const {
  (*d)[names::senders] = senders_;
  (*d)[names::targets] = targets_;
}

void nest::weight_recorder::Parameters_::set(const DictionaryDatum &d) {
...
void nest::weight_recorder::init_state_(const Node &np) {
  const weight_recorder &wr = static_cast<const weight_recorder &>(np);
  device_.init_state(wr.device_);
  init_buffers_();
}

void nest::weight_recorder::init_buffers_() {
  device_.init_buffers();
  B_.events_ = std::vector<WeightRecorderEvent>();
}

void nest::weight_recorder::calibrate() {
  if (kernel().event_delivery_manager.get_off_grid_communication() and
      not device_.is_precise_times_user_set()) {
...
void nest::weight_recorder::update(Time const &, const long from,
                                   const long to) {
...
void nest::weight_recorder::get_status(DictionaryDatum &d) const {
...
void nest::weight_recorder::set_status(const DictionaryDatum &d) {
...
void nest::weight_recorder::handle(WeightRecorderEvent &e) {
  ...
  if (device_.is_active(e.get_stamp())) {
    ...
    if ((not P_.senders_.empty() and
         not std::binary_search(P_.senders_.begin(), P_.senders_.end(),
                                e.get_sender_gid())) or
        (not P_.targets_.empty() and
         not std::binary_search(P_.targets_.begin(), P_.targets_.end(),
                                e.get_receiver_gid()))) {
      return;
    }
