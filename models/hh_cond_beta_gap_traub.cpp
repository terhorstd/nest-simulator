...
template <> void RecordablesMap<hh_cond_beta_gap_traub>::create() {
  // use standard names whereever you can for consistency!
  insert_(names::V_m, &hh_cond_beta_gap_traub::get_y_elem_<
                          hh_cond_beta_gap_traub::State_::V_M>);
  insert_(names::g_ex, &hh_cond_beta_gap_traub::get_y_elem_<
                           hh_cond_beta_gap_traub::State_::G_EXC>);
  insert_(names::g_in, &hh_cond_beta_gap_traub::get_y_elem_<
                           hh_cond_beta_gap_traub::State_::G_INH>);
  insert_(names::Act_m, &hh_cond_beta_gap_traub::get_y_elem_<
                            hh_cond_beta_gap_traub::State_::HH_M>);
  insert_(names::Act_h, &hh_cond_beta_gap_traub::get_y_elem_<
                            hh_cond_beta_gap_traub::State_::HH_H>);
  insert_(names::Inact_n, &hh_cond_beta_gap_traub::get_y_elem_<
                              hh_cond_beta_gap_traub::State_::HH_N>);
}

extern "C" int hh_cond_beta_gap_traub_dynamics(double time, const double y[],
                                               double f[], void *pnode) {
  // a shorthand
...
nest::hh_cond_beta_gap_traub::State_::State_(const Parameters_ &p) : r_(0) {
  y_[0] = p.E_L;
...
nest::hh_cond_beta_gap_traub::State_::State_(const State_ &s) : r_(s.r_) {
...
void nest::hh_cond_beta_gap_traub::Parameters_::get(DictionaryDatum &d) const {
...
void nest::hh_cond_beta_gap_traub::Parameters_::set(const DictionaryDatum &d) {
...
void nest::hh_cond_beta_gap_traub::State_::get(DictionaryDatum &d) const {
...
void nest::hh_cond_beta_gap_traub::State_::set(const DictionaryDatum &d,
                                               const Parameters_ &) {
...
void nest::hh_cond_beta_gap_traub::init_state_(const Node &proto) {
  const hh_cond_beta_gap_traub &pr = downcast<hh_cond_beta_gap_traub>(proto);
  S_ = pr.S_;
}

void nest::hh_cond_beta_gap_traub::init_buffers_() {
...
void nest::hh_cond_beta_gap_traub::calibrate() {
...
bool nest::hh_cond_beta_gap_traub::update_(Time const &origin, const long from,
                                           const long to,
  ...
  for (long lag = from; lag < to; ++lag) {
    ...
    if (called_from_wfr_update) {
      ...
      if (interpolation_order == 3) {
        hh_cond_beta_gap_traub_dynamics(0, S_.y_, f_temp,
    ...
    } else // if(called_from_wfr_update)
    {
      ...
      case 3:
        y_ip1 = S_.y_[State_::V_M];
        hh_cond_beta_gap_traub_dynamics(B_.step_, S_.y_, f_temp,
                                        reinterpret_cast<void *>(this));
...
void nest::hh_cond_beta_gap_traub::handle(SpikeEvent &e) {
...
void nest::hh_cond_beta_gap_traub::handle(CurrentEvent &e) {
  assert(e.get_delay_steps() > 0);
...
void nest::hh_cond_beta_gap_traub::handle(DataLoggingRequest &e) {
  B_.logger_.handle(e);
}

void nest::hh_cond_beta_gap_traub::handle(GapJunctionEvent &e) {
