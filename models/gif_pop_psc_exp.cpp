...
template <> void RecordablesMap<gif_pop_psc_exp>::create() {
...
nest::gif_pop_psc_exp::State_::State_()
    : y0_(0.0), I_syn_ex_(0.0), I_syn_in_(0.0), V_m_(0.0), n_expect_(0.0),
      theta_hat_(0.0) // initialization value has no effect for theta_hat_
      ,
      n_spikes_(0), initialized_(false) {}

/* ----------------------------------------------------------------
 * Parameter and state extractions and manipulation functions
 * ---------------------------------------------------------------- */

void nest::gif_pop_psc_exp::Parameters_::get(DictionaryDatum &d) const {
...
void nest::gif_pop_psc_exp::Parameters_::set(const DictionaryDatum &d) {
...
void nest::gif_pop_psc_exp::State_::get(DictionaryDatum &d,
...
void nest::gif_pop_psc_exp::State_::set(const DictionaryDatum &d,
                                        const Parameters_ &) {
  ...
  initialized_ = false; // vectors of the state should be initialized with new
                        // parameter set.
}

nest::gif_pop_psc_exp::Buffers_::Buffers_(gif_pop_psc_exp &n) : logger_(n) {}
...
nest::gif_pop_psc_exp::gif_pop_psc_exp() : Node(), P_(), S_(), B_(*this) {
  recordablesMap_.create();
}

nest::gif_pop_psc_exp::gif_pop_psc_exp(const gif_pop_psc_exp &n)
    : Node(n), P_(n.P_), S_(n.S_), B_(n.B_, *this) {}

/* ----------------------------------------------------------------
 * Node initialization functions
 * ---------------------------------------------------------------- */

void nest::gif_pop_psc_exp::init_state_(const Node &proto) {
  const gif_pop_psc_exp &pr = downcast<gif_pop_psc_exp>(proto);
  S_ = pr.S_;
}

void nest::gif_pop_psc_exp::init_buffers_() {
...
void nest::gif_pop_psc_exp::calibrate() {
...
inline double nest::gif_pop_psc_exp::escrate(const double x) {
  return P_.lambda_0_ * std::exp(x / P_.Delta_V_);
}

inline long nest::gif_pop_psc_exp::draw_poisson(const double n_expect_) {
...
inline long nest::gif_pop_psc_exp::draw_binomial(const double n_expect_) {
...
inline double nest::gif_pop_psc_exp::adaptation_kernel(const int k) {
...
inline int nest::gif_pop_psc_exp::get_history_size() {
...
void nest::gif_pop_psc_exp::update(Time const &origin, const long from,
                                   const long to) {
  assert(to >= 0 and
         (delay) from < kernel().connection_manager.get_min_delay());
  assert(from < to);
...
void gif_pop_psc_exp::handle(SpikeEvent &e) {
...
void nest::gif_pop_psc_exp::handle(CurrentEvent &e) {
  assert(e.get_delay_steps() > 0);
...
void nest::gif_pop_psc_exp::handle(DataLoggingRequest &e) {
  B_.logger_.handle(e);
}
