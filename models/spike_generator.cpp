...
nest::spike_generator::Parameters_::Parameters_()
    : spike_stamps_(), spike_offsets_(), spike_weights_(),
      spike_multiplicities_(), precise_times_(false),
      allow_offgrid_times_(false), shift_now_spikes_(false),
      deprecation_warning_issued_(false) {}

nest::spike_generator::Parameters_::Parameters_(const Parameters_ &op)
    : spike_stamps_(op.spike_stamps_), spike_offsets_(op.spike_offsets_),
...
nest::spike_generator::State_::State_() : position_(0) {}
...
void nest::spike_generator::Parameters_::assert_valid_spike_time_and_insert_(
    double t, const Time &origin, const Time &now) {
...
void nest::spike_generator::Parameters_::set(const DictionaryDatum &d,
  ...
  if (allow_offgrid_spikes_changed and allow_offgrid_times_changed) {
    throw BadProperty("allow_offgrid_spikes and allow_offgrid_times can "
  ...
  flags_changed = flags_changed or allow_offgrid_spikes_changed or
                  allow_offgrid_times_changed;
  if (precise_times_ && (allow_offgrid_times_ || shift_now_spikes_)) {
    throw BadProperty("Option precise_times cannot be set to true when either "
  ...
  if (updated_spike_weights) {
    ...
    if (spike_weights.empty()) {
      ...
      if (spike_weights.size() != spike_stamps_.size()) {
        throw BadProperty("spike_weights must have the same number of elements "
                          "as spike_times,"
...
nest::spike_generator::spike_generator()
    : DeviceNode(), device_(), P_(), S_() {}

nest::spike_generator::spike_generator(const spike_generator &n)
    : DeviceNode(n), device_(n.device_), P_(n.P_), S_(n.S_) {}
...
void nest::spike_generator::init_buffers_() { device_.init_buffers(); }

void nest::spike_generator::calibrate() { device_.calibrate(); }

/* ----------------------------------------------------------------
 * Other functions
 * ---------------------------------------------------------------- */

void nest::spike_generator::update(Time const &sliceT0, const long from,
                                   const long to) {
...
// inline
void nest::spike_generator::set_status(const DictionaryDatum &d) {
