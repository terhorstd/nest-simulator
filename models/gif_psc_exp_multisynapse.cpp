...
template <> void RecordablesMap<gif_psc_exp_multisynapse>::create() {
...
nest::gif_psc_exp_multisynapse::Parameters_::Parameters_()
    : g_L_(4.0) // nS
      ...
      tau_syn_(1, 2.0) // ms
      ,
      has_connections_(false), I_e_(0.0) // pA
{}

nest::gif_psc_exp_multisynapse::State_::State_()
    : I_stim_(0.0), V_(-70.0), sfa_(0.0), stc_(0.0), sfa_elems_(), stc_elems_(),
      i_syn_(), r_ref_(0) {}
...
void nest::gif_psc_exp_multisynapse::Parameters_::get(
...
void nest::gif_psc_exp_multisynapse::Parameters_::set(
  ...
  if (updateValue<std::vector<double>>(d, names::tau_syn, tau_tmp)) {
    if (has_connections_ && tau_tmp.size() < tau_syn_.size()) {
      throw BadProperty("The neuron has connections, "
...
void nest::gif_psc_exp_multisynapse::State_::get(DictionaryDatum &d,
...
void nest::gif_psc_exp_multisynapse::State_::set(const DictionaryDatum &d,
                                                 const Parameters_ &p) {
...
void nest::gif_psc_exp_multisynapse::update(Time const &origin, const long from,
                                            const long to) {
