 ...
 */
template <> void RecordablesMap<iaf_cond_beta>::create() {
...
extern "C" inline int nest::iaf_cond_beta_dynamics(double, const double y[],
                                                   double f[], void *pnode) {
  // a shorthand
...
nest::iaf_cond_beta::State_::State_(const Parameters_ &p) : r(0) {
...
nest::iaf_cond_beta::State_::State_(const State_ &s) : r(s.r) {
  for (size_t i = 0; i < STATE_VEC_SIZE; ++i) {
    y[i] = s.y[i];
  }
}

nest::iaf_cond_beta::State_ &nest::iaf_cond_beta::State_::
...
void nest::iaf_cond_beta::State_::set(const DictionaryDatum &d,
                                      const Parameters_ &) {
...
double nest::iaf_cond_beta::get_normalisation_factor(double tau_rise,
                                                     double tau_decay) {
...
void nest::iaf_cond_beta::update(Time const &origin, const long from,
                                 const long to) {
