...
template <> void RecordablesMap<ht_neuron>::create() {
  ...
  insert_(names::g_GABA_A,
          &ht_neuron::get_y_elem_<ht_neuron::State_::G_GABA_A>);
  insert_(names::g_GABA_B,
          &ht_neuron::get_y_elem_<ht_neuron::State_::G_GABA_B>);
...
extern "C" inline int ht_neuron_dynamics(double, const double y[], double f[],
                                         void *pnode) {
  // shorthand
  ...
  //    then dV/dt ~ Sum(I)
  const double I_syn =
      -y[S::G_AMPA] * (V - node.P_.E_rev_AMPA) -
  ...
  node.S_.I_NaP_ =
      -node.P_.g_peak_NaP * std::pow(m_inf_NaP, 3.0) * (V - node.P_.E_rev_NaP);
  ...
  // I_T
  const double tau_m_T =
      0.22 / (std::exp(-(V + 132.0) / 16.7) + std::exp((V + 16.8) / 18.2)) +
      0.13;
  const double tau_h_T = 8.2 + (56.6 + 0.27 * std::exp((V + 115.2) / 5.0)) /
...
inline double nest::ht_neuron::m_eq_h_(double V) const {
  const double I_h_Vthreshold = -75.0;
  return 1.0 / (1.0 + std::exp((V - I_h_Vthreshold) / 5.5));
}

inline double nest::ht_neuron::h_eq_T_(double V) const {
  return 1.0 / (1.0 + std::exp((V + 83.0) / 4));
}

inline double nest::ht_neuron::m_eq_T_(double V) const {
  return 1.0 / (1.0 + std::exp(-(V + 59.0) / 6.2));
}

inline double nest::ht_neuron::D_eq_KNa_(double V) const {
...
inline double nest::ht_neuron::m_eq_NMDA_(double V) const {
  return 1.0 / (1.0 + std::exp(-P_.S_act_NMDA * (V - P_.V_act_NMDA)));
}

inline double nest::ht_neuron::m_NMDA_(double V, double m_eq, double m_fast,
                                       double m_slow) const {
...
inline double nest::ht_neuron::get_g_NMDA_() const {
  return S_.y_[State_::G_NMDA_TIMECOURSE] *
         m_NMDA_(S_.y_[State_::V_M], m_eq_NMDA_(S_.y_[State_::V_M]),
                 S_.y_[State_::m_fast_NMDA], S_.y_[State_::m_slow_NMDA]);
}
...
nest::ht_neuron::Parameters_::Parameters_()
    : E_Na(30.0) // mV
      ...
      E_K(-90.0) // mV
      ,
      g_NaL(0.2), g_KL(1.0), tau_m(16.0) // ms
      ,
      theta_eq(-51.0) // mV
      ,
      tau_theta(2.0) // ms
      ...
      t_ref(2.0) // ms
      ,
      g_peak_AMPA(0.1), tau_rise_AMPA(0.5) // ms
      ,
      tau_decay_AMPA(2.4) // ms
      ,
      E_rev_AMPA(0.0) // mV
      ,
      g_peak_NMDA(0.075), tau_rise_NMDA(4.0) // ms
      ,
      ...
      ,
      instant_unblock_NMDA(false), g_peak_GABA_A(0.33),
      ...
      ,
      g_peak_GABA_B(0.0132), tau_rise_GABA_B(60.0) // ms
      ,
      tau_decay_GABA_B(200.0) // ms
      ,
      E_rev_GABA_B(-90.0) // mV
      ,
      g_peak_NaP(1.0), E_rev_NaP(30.0) // mV
      ,
      g_peak_KNa(1.0), E_rev_KNa(-90.0) // mV
      ,
      tau_D_KNa(1250.0) // ms
      ,
      g_peak_T(1.0), E_rev_T(0.0) // mV
      ,
      g_peak_h(1.0), E_rev_h(-40.0) // mV
      ,
      voltage_clamp(false) {}

nest::ht_neuron::State_::State_(const ht_neuron &node, const Parameters_ &p)
    : ref_steps_(0), I_NaP_(0.0), I_KNa_(0.0), I_T_(0.0), I_h_(0.0) {
...
nest::ht_neuron::State_::State_(const State_ &s)
    : ref_steps_(s.ref_steps_), I_NaP_(s.I_NaP_), I_KNa_(s.I_KNa_),
      I_T_(s.I_T_), I_h_(s.I_h_) {
...
nest::ht_neuron::Buffers_::Buffers_(ht_neuron &n)
    : logger_(n),
      spike_inputs_(std::vector<RingBuffer>(SUP_SPIKE_RECEPTOR - 1)), s_(0),
      c_(0), e_(0), step_(Time::get_resolution().get_ms()),
      integration_step_(step_), I_stim_(0.0) {}
...
nest::ht_neuron::ht_neuron()
    : Archiving_Node(), P_(), S_(*this, P_), B_(*this) {
...
double nest::ht_neuron::get_synapse_constant(double tau_1, double tau_2,
                                             double g_peak) {
...
void ht_neuron::update(Time const &origin, const long from, const long to) {
...
void nest::ht_neuron::handle(DataLoggingRequest &e) { B_.logger_.handle(e); }
} // namespace nest
