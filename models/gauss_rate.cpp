...
void nonlinearities_gauss_rate::get(DictionaryDatum &d) const {
  def<double>(d, names::g, g_);
  def<double>(d, names::mu, mu_);
  def<double>(d, names::sigma, sigma_);
}

void nonlinearities_gauss_rate::set(const DictionaryDatum &d) {
 ...
 */
template <> void RecordablesMap<nest::gauss_rate_ipn>::create() {
  // use standard names whereever you can for consistency!
  insert_(names::rate, &nest::gauss_rate_ipn::get_rate_);
  insert_(names::noise, &nest::gauss_rate_ipn::get_noise_);
}

template <> void RecordablesMap<nest::rate_transformer_gauss>::create() {
  // use standard names whereever you can for consistency!
