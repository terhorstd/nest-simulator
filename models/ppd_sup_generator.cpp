...
nest::ppd_sup_generator::Age_distribution_::Age_distribution_(
    size_t num_age_bins, unsigned long ini_occ_ref, unsigned long ini_occ_act) {
...
nest::ppd_sup_generator::ppd_sup_generator() : DeviceNode(), device_(), P_() {}
...
void nest::ppd_sup_generator::init_buffers_() { device_.init_buffers(); }
...
void nest::ppd_sup_generator::calibrate() {
  ...
  Age_distribution_ age_distribution0(num_age_bins, ini_occ_0,
                                      P_.n_proc_ - ini_occ_0 * num_age_bins);
...
void nest::ppd_sup_generator::update(Time const &T, const long from,
                                     const long to) {
  ...
  for (long lag = from; lag < to; ++lag) {
    ...
    if (P_.amplitude_ > 0.0 && (P_.frequency_ > 0.0 || P_.frequency_ < 0.0)) {
      ...
      V_.hazard_step_t_ =
          V_.hazard_step_ * (1.0 + P_.amplitude_ * std::sin(V_.omega_ * t_ms));
    } else {
