...
ContDelayConnection<targetidentifierT>::ContDelayConnection()
    : ConnectionBase(), weight_(1.0), delay_offset_(0.0) {}
...
template <typename targetidentifierT>
void ContDelayConnection<targetidentifierT>::get_status(
    DictionaryDatum &d) const {
  ConnectionBase::get_status(d);

  def<double>(d, names::weight, weight_);
  def<double>(d, names::delay,
              Time(Time::step(get_delay_steps())).get_ms() - delay_offset_);
  def<long>(d, names::size_of, sizeof(*this));
}

template <typename targetidentifierT>
void ContDelayConnection<targetidentifierT>::set_status(
...
template <typename targetidentifierT>
void ContDelayConnection<targetidentifierT>::check_synapse_params(
    const DictionaryDatum &syn_spec) const {
  if (syn_spec->known(names::delay)) {
    LOG(M_WARNING, "Connect",
