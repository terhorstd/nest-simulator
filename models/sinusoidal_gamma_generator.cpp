...
template <> void RecordablesMap<sinusoidal_gamma_generator>::create() {
...
nest::sinusoidal_gamma_generator::Parameters_::Parameters_()
    : om_(0.0) // radian/ms
      ...
      phi_(0.0) // radian
      ,
      order_(1.0), rate_(0.0) // spikes/ms
      ,
      amplitude_(0.0) // spikes/ms
      ,
      individual_spike_trains_(true), num_trains_(0) {}

nest::sinusoidal_gamma_generator::Parameters_::Parameters_(const Parameters_ &p)
    : om_(p.om_), phi_(p.phi_), order_(p.order_), rate_(p.rate_),
...
nest::sinusoidal_gamma_generator::State_::State_() : rate_(0) {}

nest::sinusoidal_gamma_generator::Buffers_::Buffers_(
    sinusoidal_gamma_generator &n)
    : logger_(n), t0_ms_(), // will be set in init_buffers_
      Lambda_t0_(),         // will be set in init_buffers_
      P_prev_(n.P_)         // when creating Buffer, base on current parameters
{}

nest::sinusoidal_gamma_generator::Buffers_::Buffers_(
    const Buffers_ &b, sinusoidal_gamma_generator &n)
    : logger_(n), t0_ms_(b.t0_ms_), Lambda_t0_(b.Lambda_t0_),
      P_prev_(b.P_prev_) {}
...
void nest::sinusoidal_gamma_generator::Parameters_::get(
...
void nest::sinusoidal_gamma_generator::Parameters_::set(
    const DictionaryDatum &d, const sinusoidal_gamma_generator &n) {
...
void nest::sinusoidal_gamma_generator::init_buffers_() {
  ...
  std::vector<double>(P_.num_trains_,
                      kernel().simulation_manager.get_time().get_ms())
      .swap(B_.t0_ms_);
...
inline double
nest::sinusoidal_gamma_generator::deltaLambda_(const Parameters_ &p, double t_a,
                                               double t_b) const {
  if (t_a == t_b) {
    return 0.0;
  }
