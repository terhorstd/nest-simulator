...
template <>
void DynamicRecordablesMap<iaf_psc_exp_multisynapse>::create(
...
Name iaf_psc_exp_multisynapse::get_i_syn_name(size_t elem) {
  std::stringstream i_syn_name;
  i_syn_name << "I_syn_" << elem + 1;
  return Name(i_syn_name.str());
}

void iaf_psc_exp_multisynapse::insert_current_recordables(size_t first) {
  for (size_t receptor = first; receptor < P_.tau_syn_.size(); ++receptor) {
    size_t elem =
        iaf_psc_exp_multisynapse::State_::I_SYN +
        receptor *
...
void iaf_psc_exp_multisynapse::State_::get(DictionaryDatum &d,
                                           const Parameters_ &p) const {
  def<double>(d, names::V_m, V_m_ + p.E_L_); // Membrane potential
}

void iaf_psc_exp_multisynapse::State_::set(const DictionaryDatum &d,
                                           const Parameters_ &p,
                                           double delta_EL) {
...
void iaf_psc_exp_multisynapse::update(const Time &origin, const long from,
                                      const long to) {
...
port iaf_psc_exp_multisynapse::handles_test_event(SpikeEvent &,
                                                  rport receptor_type) {
  if (receptor_type <= 0 ||
