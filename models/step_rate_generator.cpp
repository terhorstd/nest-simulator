...
template <> void RecordablesMap<step_rate_generator>::create() {
...
nest::step_rate_generator::State_::State_() : rate_(0.0) {}

nest::step_rate_generator::Buffers_::Buffers_(step_rate_generator &n)
    : idx_(0), amp_(0), logger_(n) {}
...
void nest::step_rate_generator::Parameters_::get(DictionaryDatum &d) const {
...
nest::step_rate_generator::Parameters_::validate_time_(double t,
  ...
  if (t <= 0.0) {
    throw BadProperty("Amplitude can only be changed at strictly "
                      "positive times (t > 0).");
  }
  ...
  if (t_amp <= t_previous) {
    throw BadProperty("step_rate_generator: amplitude "
...
void nest::step_rate_generator::Parameters_::set(const DictionaryDatum &d,
                                                 Buffers_ &b) {
  std::vector<double> new_times;
  const bool times_changed =
      updateValue<std::vector<double>>(d, names::amplitude_times, new_times);
  const bool values_changed =
      updateValue<std::vector<double>>(d, names::amplitude_values, amp_values_);
  ...
  if (allow_offgrid_changed and
      not(times_changed or amp_time_stamps_.empty())) {
    // times_changed implies values_changed
    throw BadProperty("allow_offgrid_times can only be changed before "
...
void nest::step_rate_generator::init_state_(const Node &proto) {
  const step_rate_generator &pr = downcast<step_rate_generator>(proto);

  device_.init_state(pr.device_);
}

void nest::step_rate_generator::init_buffers_() {
...
void nest::step_rate_generator::calibrate() {
  B_.logger_.init();
...
void nest::step_rate_generator::update(Time const &origin, const long from,
                                       const long to) {
...
void nest::step_rate_generator::handle(DataLoggingRequest &e) {
  B_.logger_.handle(e);
}
