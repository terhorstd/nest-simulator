...
template <> void RecordablesMap<hh_psc_alpha>::create() {
...
extern "C" int hh_psc_alpha_dynamics(double, const double y[], double f[],
                                     void *pnode) {
  // a shorthand
...
nest::hh_psc_alpha::State_::State_(const Parameters_ &) : r_(0) {
  y_[0] = -65; // p.E_L;
...
nest::hh_psc_alpha::State_::State_(const State_ &s) : r_(s.r_) {
  for (size_t i = 0; i < STATE_VEC_SIZE; ++i) {
    y_[i] = s.y_[i];
  }
}

nest::hh_psc_alpha::State_ &nest::hh_psc_alpha::State_::
...
nest::hh_psc_alpha::hh_psc_alpha() : Archiving_Node(), P_(), S_(P_), B_(*this) {
...
void nest::hh_psc_alpha::update(Time const &origin, const long from,
                                const long to) {
...
void nest::hh_psc_alpha::handle(DataLoggingRequest &e) { B_.logger_.handle(e); }
