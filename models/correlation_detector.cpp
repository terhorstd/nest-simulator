...
nest::correlation_detector::Parameters_::Parameters_()
    : delta_tau_(Time::ms(1.0)), tau_max_(10 * delta_tau_),
      Tstart_(Time::ms(0.0)), Tstop_(Time::pos_inf()) {}
...
nest::correlation_detector::State_::State_()
    : n_events_(2, 0), incoming_(2), histogram_(), histogram_correction_(),
      count_histogram_() {}
...
bool nest::correlation_detector::Parameters_::set(
    const DictionaryDatum &d, const correlation_detector &n) {
  bool reset = false;
  double t;
...
void nest::correlation_detector::State_::reset(const Parameters_ &p) {
  ...
  histogram_.clear();
  histogram_.resize(1 + 2 * p.tau_max_.get_steps() / p.delta_tau_.get_steps(),
                    0);
...
void nest::correlation_detector::calibrate() { device_.calibrate(); }

/* ----------------------------------------------------------------
 * Other functions
 * ---------------------------------------------------------------- */

void nest::correlation_detector::update(Time const &, const long, const long) {}
...
void nest::correlation_detector::handle(SpikeEvent &e) {
  ...
  if (device_.is_active(stamp)) {
    ...
    while (not otherSpikes.empty() &&
           (spike_i - otherSpikes.front().timestep_) - 0.5 * other >=
               tau_edge) {
    ...
    SpikelistType::iterator insert_pos = std::find_if(
        S_.incoming_[sender].begin(), S_.incoming_[sender].end(),
