...
private:
  ...
  inline double get_V_m_() const { return S_.y3_ + P_.E_L_; }
  ...
  inline double get_I_syn_ex_() const { return S_.I_ex_; }

  inline double get_I_syn_in_() const { return S_.I_in_; }
...
inline port nest::iaf_psc_alpha::send_test_event(Node &target,
...
inline port iaf_psc_alpha::handles_test_event(SpikeEvent &,
                                              rport receptor_type) {
  if (receptor_type != 0) {
...
inline port iaf_psc_alpha::handles_test_event(CurrentEvent &,
                                              rport receptor_type) {
  if (receptor_type != 0) {
    throw UnknownReceptorType(receptor_type, get_name());
  }
...
inline port iaf_psc_alpha::handles_test_event(DataLoggingRequest &dlr,
                                              rport receptor_type) {
  if (receptor_type != 0) {
    throw UnknownReceptorType(receptor_type, get_name());
  }
...
inline void iaf_psc_alpha::get_status(DictionaryDatum &d) const {
  P_.get(d);
  S_.get(d, P_);
...
inline void iaf_psc_alpha::set_status(const DictionaryDatum &d) {
