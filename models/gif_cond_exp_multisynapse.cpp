...
template <> void RecordablesMap<gif_cond_exp_multisynapse>::create() {
  // use standard names wherever you can for consistency!
  insert_(names::V_m, &gif_cond_exp_multisynapse::get_y_elem_<
...
extern "C" int nest::gif_cond_exp_multisynapse_dynamics(double, const double *y,
                                                        double *f,
                                                        void *pnode) {
  // a shorthand
...
nest::gif_cond_exp_multisynapse::Parameters_::Parameters_()
    : g_L_(4.0) // nS
      ...
      I_e_(0.0) // pA
      ,
      has_connections_(false), gsl_error_tol(1e-3) {}

nest::gif_cond_exp_multisynapse::State_::State_(const Parameters_ &p)
    : y_(STATE_VEC_SIZE + NUM_STATE_ELEMENTS_PER_RECEPTOR, 0.0), I_stim_(0.0),
      sfa_(0.0), stc_(0.0), sfa_elems_(), stc_elems_(), r_ref_(0) {
  y_[V_M] = p.E_L_;
}

nest::gif_cond_exp_multisynapse::State_::State_(const State_ &s)
    : I_stim_(s.I_stim_), sfa_(s.sfa_), stc_(s.stc_), r_ref_(s.r_ref_) {
...
void nest::gif_cond_exp_multisynapse::Parameters_::get(
...
void nest::gif_cond_exp_multisynapse::Parameters_::set(
  ...
  if (Erev_flag || tau_flag) { // receptor arrays have been modified
    if ((E_rev_.size() != old_n_receptors ||
         tau_syn_.size() != old_n_receptors) and
        (not Erev_flag || not tau_flag)) {
      throw BadProperty(
...
void nest::gif_cond_exp_multisynapse::State_::get(DictionaryDatum &d,
...
void nest::gif_cond_exp_multisynapse::State_::set(const DictionaryDatum &d,
                                                  const Parameters_ &p) {
...
nest::gif_cond_exp_multisynapse::Buffers_::Buffers_(
    ...
    : logger_(n), s_(0), c_(0), e_(0), step_(Time::get_resolution().get_ms()),
...
nest::gif_cond_exp_multisynapse::Buffers_::Buffers_(
    const Buffers_ &b, gif_cond_exp_multisynapse &n)
    : logger_(n), s_(0), c_(0), e_(0), step_(b.step_),
...
void nest::gif_cond_exp_multisynapse::update(Time const &origin,
  ...
  for (long lag = from; lag < to; ++lag) {
    ...
    if (S_.r_ref_ == 0) // neuron is not in refractory period
    {
      ...
      const double lambda =
          P_.lambda_0_ * std::exp((S_.y_[State_::V_M] - S_.sfa_) / P_.Delta_V_);
