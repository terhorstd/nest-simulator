...
Multimeter::Multimeter()
    : DeviceNode(),
      device_(*this, RecordingDevice::MULTIMETER, "dat", true, true), P_(),
      S_(), B_(), V_() {}

Multimeter::Multimeter(const Multimeter &n)
    : DeviceNode(n), device_(*this, n.device_), P_(n.P_), S_(), B_(), V_() {}

port Multimeter::send_test_event(Node &target, rport receptor_type, synindex,
                                 bool) {
...
nest::Multimeter::Parameters_::Parameters_()
    : interval_(Time::ms(1.0)), offset_(Time::ms(0.)), record_from_() {}

nest::Multimeter::Parameters_::Parameters_(const Parameters_ &p)
    : interval_(p.interval_), offset_(p.offset_), record_from_(p.record_from_) {
  interval_.calibrate();
}

nest::Multimeter::Buffers_::Buffers_() : has_targets_(false) {}
...
void nest::Multimeter::Parameters_::set(const DictionaryDatum &d,
                                        const Buffers_ &b) {
  if (b.has_targets_ && (d->known(names::interval) || d->known(names::offset) ||
  ...
  if (updateValue<double>(d, names::offset, v)) {
    ...
    if (v != 0 && Time(Time::ms(v)) < Time::get_resolution()) {
      throw BadProperty("The offset for the sampling interval must be at least "
                        "as long as the "
...
void Multimeter::init_buffers_() { device_.init_buffers(); }
...
void Multimeter::post_run_cleanup() { device_.post_run_cleanup(); }

void Multimeter::finalize() { device_.finalize(); }

void Multimeter::update(Time const &origin, const long from, const long) {
...
void Multimeter::handle(DataLoggingReply &reply) {
  ...
  for (size_t j = 0; j < info.size(); ++j) {
    ...
    if (not device_.to_accumulator()) {
      ...
      {
        S_.data_.push_back(info[j].data);
      } else { // add data; offset j from current_request_data_start_, but
               // inactive
        // skipped entries subtracted
        assert(j >= inactive_skipped);
        assert(V_.current_request_data_start_ + j - inactive_skipped <
               S_.data_.size());
        assert(S_.data_[V_.current_request_data_start_ + j - inactive_skipped]
                   .size() == info[j].data.size());
...
void Multimeter::print_value_(const std::vector<double> &values) {
  if (values.size() < 1) {
    return;
  }
