...
template <typename targetidentifierT>
inline void
HTConnection<targetidentifierT>::send(Event &e, thread t,
...
HTConnection<targetidentifierT>::HTConnection()
    : ConnectionBase(), weight_(1.0), tau_P_(500.0), delta_P_(0.125), p_(1.0),
      t_lastspike_(0.0) {}

template <typename targetidentifierT>
HTConnection<targetidentifierT>::HTConnection(const HTConnection &rhs)
    : ConnectionBase(rhs), weight_(rhs.weight_), tau_P_(rhs.tau_P_),
      delta_P_(rhs.delta_P_), p_(rhs.p_), t_lastspike_(rhs.t_lastspike_) {}
...
template <typename targetidentifierT>
void HTConnection<targetidentifierT>::set_status(const DictionaryDatum &d,
                                                 ConnectorModel &cm) {
