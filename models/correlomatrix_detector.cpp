...
nest::correlomatrix_detector::Parameters_::Parameters_()
    : delta_tau_(5 * Time::get_resolution()), tau_max_(10 * delta_tau_),
...
void nest::correlomatrix_detector::State_::get(DictionaryDatum &d) const {
  ...
  for (size_t i = 0; i < covariance_.size(); ++i) {
    ...
    for (size_t j = 0; j < covariance_[i].size(); ++j) {
      C_i->push_back(
          new DoubleVectorDatum(new std::vector<double>(covariance_[i][j])));
      CountC_i->push_back(
...
bool nest::correlomatrix_detector::Parameters_::set(
    const DictionaryDatum &d, const correlomatrix_detector &n) {
  bool reset = false;
  double t;
  long N;
...
void nest::correlomatrix_detector::calibrate() { device_.calibrate(); }

/* ----------------------------------------------------------------
 * Other functions
 * ---------------------------------------------------------------- */

void nest::correlomatrix_detector::update(Time const &, const long,
                                          const long) {}
  ...
  if (device_.is_active(stamp)) {
    ...
    if (P_.Tstart_ <= stamp && stamp <= P_.Tstop_) {
      ...
      for (SpikelistType::const_iterator spike_j = otherSpikes.begin();
           spike_j != otherSpikes.end(); ++spike_j) {
        size_t bin;
