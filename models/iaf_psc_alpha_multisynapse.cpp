...
template <>
void DynamicRecordablesMap<iaf_psc_alpha_multisynapse>::create(
...
Name iaf_psc_alpha_multisynapse::get_i_syn_name(size_t elem) {
  std::stringstream i_syn_name;
  i_syn_name << "I_syn_" << elem + 1;
  return Name(i_syn_name.str());
}

void iaf_psc_alpha_multisynapse::insert_current_recordables(size_t first) {
  for (size_t receptor = first; receptor < P_.tau_syn_.size(); ++receptor) {
    size_t elem =
        iaf_psc_alpha_multisynapse::State_::I_SYN +
        receptor *
            iaf_psc_alpha_multisynapse::State_::NUM_STATE_ELEMENTS_PER_RECEPTOR;
    recordablesMap_.insert(get_i_syn_name(receptor),
...
iaf_psc_alpha_multisynapse::State_::State_()
    : I_const_(0.0), V_m_(0.0), current_(0.0), refractory_steps_(0) {
  y1_syn_.clear();
  y2_syn_.clear();
}
...
void iaf_psc_alpha_multisynapse::State_::get(DictionaryDatum &d,
                                             const Parameters_ &p) const {
  def<double>(d, names::V_m, V_m_ + p.E_L_); // Membrane potential
}

void iaf_psc_alpha_multisynapse::State_::set(const DictionaryDatum &d,
                                             const Parameters_ &p,
...
void iaf_psc_alpha_multisynapse::update(Time const &origin, const long from,
                                        const long to) {
  ...
  for (long lag = from; lag < to; ++lag) {
    if (S_.refractory_steps_ == 0) {
      ...
      for (size_t i = 0; i < P_.n_receptors_(); i++) {
        S_.V_m_ +=
            V_.P31_syn_[i] * S_.y1_syn_[i] + V_.P32_syn_[i] * S_.y2_syn_[i];
...
port iaf_psc_alpha_multisynapse::handles_test_event(SpikeEvent &,
                                                    rport receptor_type) {
  if (receptor_type <= 0 ||
...
void iaf_psc_alpha_multisynapse::set_status(const DictionaryDatum &d) {
      ...
      P_.tau_syn_.size()) // Number of receptors increased
  {
    for (size_t i_syn = P_.tau_syn_.size(); i_syn < ptmp.tau_syn_.size();
      ...
      recordablesMap_.insert(get_i_syn_name(i_syn),
                             get_data_access_functor(elem));
    }
  } else if (ptmp.tau_syn_.size() <
             P_.tau_syn_.size()) { // Number of receptors decreased
