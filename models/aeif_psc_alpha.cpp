...
template <> void RecordablesMap<aeif_psc_alpha>::create() {
  // use standard names whereever you can for consistency!
  insert_(names::V_m,
          &aeif_psc_alpha::get_y_elem_<aeif_psc_alpha::State_::V_M>);
...
extern "C" int nest::aeif_psc_alpha_dynamics(double, const double y[],
                                             double f[], void *pnode) {
  // a shorthand
...
nest::aeif_psc_alpha::State_::State_(const Parameters_ &p) : r_(0) {
  y_[0] = p.E_L;
...
nest::aeif_psc_alpha::State_::State_(const State_ &s) : r_(s.r_) {
  for (size_t i = 0; i < STATE_VEC_SIZE; ++i) {
    y_[i] = s.y_[i];
  }
}

nest::aeif_psc_alpha::State_ &nest::aeif_psc_alpha::State_::
...
void nest::aeif_psc_alpha::Parameters_::get(DictionaryDatum &d) const {
...
void nest::aeif_psc_alpha::Parameters_::set(const DictionaryDatum &d) {
...
void nest::aeif_psc_alpha::State_::get(DictionaryDatum &d) const {
...
void nest::aeif_psc_alpha::State_::set(const DictionaryDatum &d,
                                       const Parameters_ &) {
...
void nest::aeif_psc_alpha::init_state_(const Node &proto) {
  const aeif_psc_alpha &pr = downcast<aeif_psc_alpha>(proto);
  S_ = pr.S_;
}

void nest::aeif_psc_alpha::init_buffers_() {
...
void nest::aeif_psc_alpha::calibrate() {
...
void nest::aeif_psc_alpha::update(Time const &origin, const long from,
                                  const long to) {
...
void nest::aeif_psc_alpha::handle(SpikeEvent &e) {
...
void nest::aeif_psc_alpha::handle(CurrentEvent &e) {
  assert(e.get_delay_steps() > 0);
...
void nest::aeif_psc_alpha::handle(DataLoggingRequest &e) {
  B_.logger_.handle(e);
}
