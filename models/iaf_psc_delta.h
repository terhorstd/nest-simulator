...
inline port nest::iaf_psc_delta::send_test_event(Node &target,
...
inline port iaf_psc_delta::handles_test_event(SpikeEvent &,
                                              rport receptor_type) {
  if (receptor_type != 0) {
...
inline port iaf_psc_delta::handles_test_event(CurrentEvent &,
                                              rport receptor_type) {
  if (receptor_type != 0) {
    throw UnknownReceptorType(receptor_type, get_name());
  }
...
inline port iaf_psc_delta::handles_test_event(DataLoggingRequest &dlr,
                                              rport receptor_type) {
  if (receptor_type != 0) {
    throw UnknownReceptorType(receptor_type, get_name());
  }
...
inline void iaf_psc_delta::get_status(DictionaryDatum &d) const {
  P_.get(d);
  S_.get(d, P_);
...
inline void iaf_psc_delta::set_status(const DictionaryDatum &d) {
