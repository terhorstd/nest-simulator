...
template <> void RecordablesMap<aeif_cond_alpha_RK5>::create() {
...
nest::aeif_cond_alpha_RK5::State_::State_(const Parameters_ &p) : r_(0) {
  y_[0] = p.E_L;
...
nest::aeif_cond_alpha_RK5::State_::State_(const State_ &s) : r_(s.r_) {
  for (size_t i = 0; i < STATE_VEC_SIZE; ++i) {
    y_[i] = s.y_[i];
  }
}

nest::aeif_cond_alpha_RK5::State_ &nest::aeif_cond_alpha_RK5::State_::
 ...
 * @param to
 */
void nest::aeif_cond_alpha_RK5::update(Time const &origin, const long from,
  ...
  for (long lag = from; lag < to; ++lag) // proceed by stepsize B_.step_
  {
    ...
    while (t < B_.step_) // while not yet reached end of simulation step
    {
      ...
      do {
        ...
        for (int i = 0; i < S_.STATE_VEC_SIZE; ++i) {
          S_.yin[i] =
              S_.y_[i] + h * (3.0 / 40.0 * S_.k1[i] + 9.0 / 40.0 * S_.k2[i]);
        }
        ...
        for (int i = 0; i < S_.STATE_VEC_SIZE; ++i) {
          S_.yin[i] =
              S_.y_[i] + h * (44.0 / 45.0 * S_.k1[i] - 56.0 / 15.0 * S_.k2[i] +
                              32.0 / 9.0 * S_.k3[i]);
        }
        ...
        for (int i = 0; i < S_.STATE_VEC_SIZE; ++i) {
          S_.yin[i] =
              ...
              h * (19372.0 / 6561.0 * S_.k1[i] - 25360.0 / 2187.0 * S_.k2[i] +
                   64448.0 / 6561.0 * S_.k3[i] - 212.0 / 729.0 * S_.k4[i]);
        }
