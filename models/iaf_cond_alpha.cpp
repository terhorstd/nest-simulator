 ...
 */
template <> void RecordablesMap<iaf_cond_alpha>::create() {
...
extern "C" inline int nest::iaf_cond_alpha_dynamics(double, const double y[],
                                                    double f[], void *pnode) {
  // a shorthand
...
nest::iaf_cond_alpha::State_::State_(const Parameters_ &p) : r(0) {
...
nest::iaf_cond_alpha::State_::State_(const State_ &s) : r(s.r) {
  for (size_t i = 0; i < STATE_VEC_SIZE; ++i) {
    y[i] = s.y[i];
  }
}

nest::iaf_cond_alpha::State_ &nest::iaf_cond_alpha::State_::
...
void nest::iaf_cond_alpha::State_::set(const DictionaryDatum &d,
                                       const Parameters_ &) {
...
void nest::iaf_cond_alpha::update(Time const &origin, const long from,
                                  const long to) {
