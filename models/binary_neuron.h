 ...
 */
template <class TGainfunction> class binary_neuron : public Archiving_Node {
...
private:
  ...
  double get_output_state__() const { return S_.y_; }

  //! Read out the summed input of the neuron (= membrane potential)
  double get_input__() const { return S_.h_; }
...
inline port binary_neuron<TGainfunction>::send_test_event(Node &target,
                                                          rport receptor_type,
                                                          synindex, bool) {
  SpikeEvent e;
  e.set_sender(*this);
...
template <class TGainfunction>
inline SignalType binary_neuron<TGainfunction>::sends_signal() const {
  return BINARY;
}

template <class TGainfunction>
inline SignalType binary_neuron<TGainfunction>::receives_signal() const {
  return BINARY;
}

template <class TGainfunction>
inline void binary_neuron<TGainfunction>::get_status(DictionaryDatum &d) const {
  P_.get(d);
  S_.get(d, P_);
...
template <class TGainfunction>
inline void binary_neuron<TGainfunction>::set_status(const DictionaryDatum &d) {
...
binary_neuron<TGainfunction>::State_::State_()
    : y_(false), h_(0.0), last_in_gid_(0),
...
template <class TGainfunction>
void binary_neuron<TGainfunction>::State_::get(DictionaryDatum &d,
...
template <class TGainfunction>
void binary_neuron<TGainfunction>::State_::set(const DictionaryDatum &,
                                               const Parameters_ &) {}
...
binary_neuron<TGainfunction>::binary_neuron(const binary_neuron &n)
    : Archiving_Node(n), gain_(n.gain_), P_(n.P_), S_(n.S_), B_(*this) {}
...
template <class TGainfunction> void binary_neuron<TGainfunction>::calibrate() {
...
template <class TGainfunction>
void binary_neuron<TGainfunction>::update(Time const &origin, const long from,
                                          const long to) {
...
void binary_neuron<TGainfunction>::handle(SpikeEvent &e) {
  ...
  if (m == 1) { // multiplicity == 1, either a single 1->0 event or the first or
                // second of a
    // pair of 0->1 events
