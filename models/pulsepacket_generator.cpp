...
nest::pulsepacket_generator::Parameters_::Parameters_()
    : pulse_times_(), a_(0), sdev_(0.0), sdev_tolerance_(10.0) {}

nest::pulsepacket_generator::Variables_::Variables_()
    : start_center_idx_(0), stop_center_idx_(0), tolerance(0.0) {}
...
nest::pulsepacket_generator::pulsepacket_generator(
    ...
    : Node(ppg), device_(ppg.device_), P_(ppg.P_) {}
...
void nest::pulsepacket_generator::init_buffers_() { device_.init_buffers(); }
...
void nest::pulsepacket_generator::update(Time const &T, const long from,
                                         const long to) {
  assert(to >= from);
  ...
  while (not B_.spiketimes_.empty() &&
    ...
    if (n_spikes > 0 && prev_spike != B_.spiketimes_.front()) {
      ...
      kernel().event_delivery_manager.send(*this, se,
                                           prev_spike - T.get_steps());
      n_spikes = 0;
    }
  }
}
