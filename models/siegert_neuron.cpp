...
double integrand1(double x, void *p) {
...
template <> void RecordablesMap<siegert_neuron>::create() {
...
nest::siegert_neuron::State_::State_() : r_(0.0) {}

/* ----------------------------------------------------------------
 * Parameter and state extractions and manipulation functions
 * ---------------------------------------------------------------- */

void nest::siegert_neuron::Parameters_::get(DictionaryDatum &d) const {
...
void nest::siegert_neuron::Parameters_::set(const DictionaryDatum &d) {
...
void nest::siegert_neuron::State_::get(DictionaryDatum &d) const {
  def<double>(d, names::rate, r_); // Rate
}

void nest::siegert_neuron::State_::set(const DictionaryDatum &d) {
  updateValue<double>(d, names::rate, r_); // Rate
}

nest::siegert_neuron::Buffers_::Buffers_(siegert_neuron &n) : logger_(n) {}
...
double nest::siegert_neuron::siegert1(double theta_shift, double V_reset_shift,
  ...
  gsl_integration_qags(&F, lower_bound, upper_bound, 0.0, 1.49e-8, 1000, gsl_w_,
                       &result, &error);
...
double nest::siegert_neuron::siegert2(double theta_shift, double V_reset_shift,
                                      double mu, double sigma) {
  double y_th;
  y_th = (theta_shift - mu) / sigma;
  double y_r;
...
double nest::siegert_neuron::siegert(double mu, double sigma_square) {
...
void nest::siegert_neuron::init_state_(const Node &proto) {
  const siegert_neuron &pr = downcast<siegert_neuron>(proto);
  S_ = pr.S_;
}

void nest::siegert_neuron::init_buffers_() {
  // resize buffers
...
void nest::siegert_neuron::calibrate() {
  B_.logger_
...
bool nest::siegert_neuron::update_(Time const &origin, const long from,
                                   const long to,
...
void nest::siegert_neuron::handle(DiffusionConnectionEvent &e) {
...
void nest::siegert_neuron::handle(DataLoggingRequest &e) {
  B_.logger_.handle(e);
}
