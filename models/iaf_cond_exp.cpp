...
template <> void RecordablesMap<iaf_cond_exp>::create() {
...
extern "C" inline int nest::iaf_cond_exp_dynamics(double, const double y[],
                                                  double f[], void *pnode) {
  // a shorthand
...
nest::iaf_cond_exp::State_::State_(const Parameters_ &p) : r_(0) {
  y_[V_M] = p.E_L;
  y_[G_EXC] = y_[G_INH] = 0;
}

nest::iaf_cond_exp::State_::State_(const State_ &s) : r_(s.r_) {
  for (size_t i = 0; i < STATE_VEC_SIZE; ++i) {
    y_[i] = s.y_[i];
  }
}

nest::iaf_cond_exp::State_ &nest::iaf_cond_exp::State_::
...
nest::iaf_cond_exp::iaf_cond_exp() : Archiving_Node(), P_(), S_(P_), B_(*this) {
...
void nest::iaf_cond_exp::update(Time const &origin, const long from,
                                const long to) {
...
void nest::iaf_cond_exp::handle(DataLoggingRequest &e) { B_.logger_.handle(e); }
