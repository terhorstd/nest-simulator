...
template <> void RecordablesMap<hh_psc_alpha_clopath>::create() {
  ...
  insert_(names::u_bar_plus, &hh_psc_alpha_clopath::get_y_elem_<
                                 hh_psc_alpha_clopath::State_::U_BAR_PLUS>);
  insert_(names::u_bar_minus, &hh_psc_alpha_clopath::get_y_elem_<
                                  hh_psc_alpha_clopath::State_::U_BAR_MINUS>);
  insert_(names::u_bar_bar, &hh_psc_alpha_clopath::get_y_elem_<
                                hh_psc_alpha_clopath::State_::U_BAR_BAR>);
}

extern "C" int hh_psc_alpha_clopath_dynamics(double, const double y[],
                                             double f[], void *pnode) {
  // a shorthand
...
nest::hh_psc_alpha_clopath::State_::State_(const Parameters_ &) : r_(0) {
  y_[0] = -65; // p.E_L;
...
nest::hh_psc_alpha_clopath::State_::State_(const State_ &s) : r_(s.r_) {
...
void nest::hh_psc_alpha_clopath::Parameters_::get(DictionaryDatum &d) const {
...
void nest::hh_psc_alpha_clopath::Parameters_::set(const DictionaryDatum &d) {
  ...
  if (tau_synE <= 0 or tau_synI <= 0 or tau_plus <= 0 or tau_minus <= 0 or
      tau_bar_bar <= 0) {
...
void nest::hh_psc_alpha_clopath::State_::get(DictionaryDatum &d) const {
...
void nest::hh_psc_alpha_clopath::State_::set(const DictionaryDatum &d) {
...
void nest::hh_psc_alpha_clopath::init_state_(const Node &proto) {
  const hh_psc_alpha_clopath &pr = downcast<hh_psc_alpha_clopath>(proto);
  S_ = pr.S_;
}

void nest::hh_psc_alpha_clopath::init_buffers_() {
...
void nest::hh_psc_alpha_clopath::calibrate() {
...
void nest::hh_psc_alpha_clopath::update(Time const &origin, const long from,
                                        const long to) {
...
void nest::hh_psc_alpha_clopath::handle(SpikeEvent &e) {
...
void nest::hh_psc_alpha_clopath::handle(CurrentEvent &e) {
  assert(e.get_delay_steps() > 0);
...
void nest::hh_psc_alpha_clopath::handle(DataLoggingRequest &e) {
  B_.logger_.handle(e);
}
