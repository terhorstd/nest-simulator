...
template <> void RecordablesMap<pp_psc_delta>::create() {
...
nest::pp_psc_delta::Parameters_::Parameters_()
    : tau_m_(10.0) // ms
      ...
      dead_time_(1.0) // ms
      ,
      dead_time_random_(0), dead_time_shape_(1), with_reset_(1),
      tau_sfa_(34.0) // ms
      ,
      q_sfa_(0.0) // mV, reasonable default is 7 mV [2]
      ,
      multi_param_(1), c_1_(0.0) // Hz / mV
      ,
      c_2_(1.238) // Hz / mV
      ,
      c_3_(0.25) // 1.0 / mV
...
nest::pp_psc_delta::State_::State_()
    : y0_(0.0), y3_(0.0), q_(0.0), r_(0), initialized_(false) {
  q_elems_.clear();
}
...
void nest::pp_psc_delta::Parameters_::set(const DictionaryDatum &d) {
  ...
  if (dead_time_shape_ < 1) {
    throw BadProperty("Shape of the dead time gamma distribution must not be "
                      "smaller than 1.");
  }
...
nest::pp_psc_delta::Buffers_::Buffers_(pp_psc_delta &n) : logger_(n) {}
...
nest::pp_psc_delta::pp_psc_delta() : Archiving_Node(), P_(), S_(), B_(*this) {
...
void nest::pp_psc_delta::update(Time const &origin, const long from,
                                const long to) {
...
void nest::pp_psc_delta::handle(DataLoggingRequest &e) { B_.logger_.handle(e); }
