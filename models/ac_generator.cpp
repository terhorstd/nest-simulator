...
template <> void RecordablesMap<ac_generator>::create() {
...
nest::ac_generator::Parameters_::Parameters_(const Parameters_ &p)
    : amp_(p.amp_), offset_(p.offset_), freq_(p.freq_), phi_deg_(p.phi_deg_) {}
...
nest::ac_generator::Buffers_::Buffers_(ac_generator &n) : logger_(n) {}
...
void nest::ac_generator::update(Time const &origin, const long from,
                                const long to) {
...
void nest::ac_generator::handle(DataLoggingRequest &e) { B_.logger_.handle(e); }
