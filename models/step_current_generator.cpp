...
template <> void RecordablesMap<step_current_generator>::create() {
...
nest::Time nest::step_current_generator::Parameters_::validate_time_(
    double t, const Time &t_previous) {
  if (t <= 0.0) {
  ...
  if (t_amp <= t_previous) {
    throw BadProperty("step_current_generator: amplitude "
...
void nest::step_current_generator::update(Time const &origin, const long from,
                                          const long to) {
...
void nest::step_current_generator::handle(DataLoggingRequest &e) {
  B_.logger_.handle(e);
}
