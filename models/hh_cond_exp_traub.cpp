...
template <> void RecordablesMap<hh_cond_exp_traub>::create() {
...
extern "C" int hh_cond_exp_traub_dynamics(double, const double y[], double f[],
                                          void *pnode) {
  // a shorthand
...
nest::hh_cond_exp_traub::Parameters_::Parameters_()
    : g_Na(20000.0) // Sodium Conductance (nS)
      ...
      ,
      E_K(-90.0), E_L(-60.0), V_T(-63.0) // adjusts threshold to around -50 mV
      ,
      E_ex(0.0), E_in(-80.0),
...
nest::hh_cond_exp_traub::State_::State_(const Parameters_ &p) : r_(0) {
  y_[0] = p.E_L;
...
nest::hh_cond_exp_traub::State_::State_(const State_ &s) : r_(s.r_) {
  for (size_t i = 0; i < STATE_VEC_SIZE; ++i) {
    y_[i] = s.y_[i];
  }
}

nest::hh_cond_exp_traub::State_ &nest::hh_cond_exp_traub::State_::
 ...
 * ---------------------------------------------------------------- */
void nest::hh_cond_exp_traub::update(Time const &origin, const long from,
                                     const long to) {
