...
template <> void RecordablesMap<hh_psc_alpha_gap>::create() {
...
extern "C" int hh_psc_alpha_gap_dynamics(double time, const double y[],
                                         double f[], void *pnode) {
  // a shorthand
...
nest::hh_psc_alpha_gap::State_::State_(const Parameters_ &) : r_(0) {
...
nest::hh_psc_alpha_gap::State_::State_(const State_ &s) : r_(s.r_) {
  for (size_t i = 0; i < STATE_VEC_SIZE; ++i) {
    y_[i] = s.y_[i];
  }
}

nest::hh_psc_alpha_gap::State_ &nest::hh_psc_alpha_gap::State_::
...
void nest::hh_psc_alpha_gap::Parameters_::get(DictionaryDatum &d) const {
...
void nest::hh_psc_alpha_gap::Parameters_::set(const DictionaryDatum &d) {
...
void nest::hh_psc_alpha_gap::State_::get(DictionaryDatum &d) const {
...
void nest::hh_psc_alpha_gap::State_::set(const DictionaryDatum &d) {
...
void nest::hh_psc_alpha_gap::init_state_(const Node &proto) {
...
void nest::hh_psc_alpha_gap::calibrate() {
...
bool nest::hh_psc_alpha_gap::update_(Time const &origin, const long from,
                                     const long to,
  ...
  for (long lag = from; lag < to; ++lag) {
    ...
    if (called_from_wfr_update) {
      ...
      if (interpolation_order == 3) {
        hh_psc_alpha_gap_dynamics(0, S_.y_, f_temp,
    ...
    } else // if(called_from_wfr_update)
    {
      ...
      case 3:
        y_ip1 = S_.y_[State_::V_M];
        hh_psc_alpha_gap_dynamics(B_.step_, S_.y_, f_temp,
                                  reinterpret_cast<void *>(this));
...
void nest::hh_psc_alpha_gap::handle(SpikeEvent &e) {
...
void nest::hh_psc_alpha_gap::handle(CurrentEvent &e) {
  assert(e.get_delay_steps() > 0);
...
void nest::hh_psc_alpha_gap::handle(DataLoggingRequest &e) {
  B_.logger_.handle(e);
}

void nest::hh_psc_alpha_gap::handle(GapJunctionEvent &e) {
