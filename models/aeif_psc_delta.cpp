 ...
 */
template <> void RecordablesMap<aeif_psc_delta>::create() {
  // use standard names whereever you can for consistency!
  insert_(names::V_m,
          &aeif_psc_delta::get_y_elem_<aeif_psc_delta::State_::V_M>);
  insert_(names::w, &aeif_psc_delta::get_y_elem_<aeif_psc_delta::State_::W>);
}
} // namespace nest

extern "C" int nest::aeif_psc_delta_dynamics(double, const double y[],
                                             double f[], void *pnode) {
  // a shorthand
...
nest::aeif_psc_delta::Parameters_::Parameters_()
    : V_peak_(0.0) // mV
      ...
      I_e(0.0) // pA
      ,
      gsl_error_tol(1e-6), with_refr_input_(false) {}

nest::aeif_psc_delta::State_::State_(const Parameters_ &p) : r_(0) {
  y_[0] = p.E_L;
...
nest::aeif_psc_delta::State_::State_(const State_ &s) : r_(s.r_) {
  for (size_t i = 0; i < STATE_VEC_SIZE; ++i) {
    y_[i] = s.y_[i];
  }
}

nest::aeif_psc_delta::State_ &nest::aeif_psc_delta::State_::
...
void nest::aeif_psc_delta::Parameters_::get(DictionaryDatum &d) const {
...
void nest::aeif_psc_delta::Parameters_::set(const DictionaryDatum &d) {
...
void nest::aeif_psc_delta::State_::get(DictionaryDatum &d) const {
  def<double>(d, names::V_m, y_[V_M]);
  def<double>(d, names::w, y_[W]);
}

void nest::aeif_psc_delta::State_::set(const DictionaryDatum &d,
                                       const Parameters_ &) {
...
void nest::aeif_psc_delta::init_state_(const Node &proto) {
  const aeif_psc_delta &pr = downcast<aeif_psc_delta>(proto);
  S_ = pr.S_;
}

void nest::aeif_psc_delta::init_buffers_() {
...
void nest::aeif_psc_delta::calibrate() {
...
void nest::aeif_psc_delta::update(const Time &origin, const long from,
                                  const long to) {
...
void nest::aeif_psc_delta::handle(SpikeEvent &e) {
...
void nest::aeif_psc_delta::handle(CurrentEvent &e) {
  assert(e.get_delay_steps() > 0);
...
void nest::aeif_psc_delta::handle(DataLoggingRequest &e) {
  B_.logger_.handle(e);
}
