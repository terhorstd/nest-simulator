...
inline long STDPDopaCommonProperties::get_vt_gid() const {
  if (vt_ != 0) {
    return vt_->get_gid();
  } else {
    return -1;
...
public:
  ...
  void trigger_update_weight(thread t,
                             ...
                             double t_trig, const STDPDopaCommonProperties &cp);
...
STDPDopaConnection<targetidentifierT>::STDPDopaConnection()
    : ConnectionBase(), weight_(1.0), Kplus_(0.0), c_(0.0), n_(0.0),
      dopa_spikes_idx_(0), t_last_update_(0.0), t_lastspike_(0.0) {}

template <typename targetidentifierT>
STDPDopaConnection<targetidentifierT>::STDPDopaConnection(
    const STDPDopaConnection &rhs)
    : ConnectionBase(rhs), weight_(rhs.weight_), Kplus_(rhs.Kplus_), c_(rhs.c_),
      n_(rhs.n_), dopa_spikes_idx_(rhs.dopa_spikes_idx_),
      t_last_update_(rhs.t_last_update_), t_lastspike_(rhs.t_lastspike_) {}

template <typename targetidentifierT>
void STDPDopaConnection<targetidentifierT>::get_status(
...
template <typename targetidentifierT>
void STDPDopaConnection<targetidentifierT>::set_status(const DictionaryDatum &d,
                                                       ConnectorModel &cm) {
...
template <typename targetidentifierT>
void STDPDopaConnection<targetidentifierT>::check_synapse_params(
  ...
  std::string param_arr[] = {"A_minus", "A_plus", "Wmax",  "Wmin",
                             "b",       "tau_c",  "tau_n", "tau_plus"};
...
template <typename targetidentifierT>
inline void STDPDopaConnection<targetidentifierT>::update_dopamine_(
    const std::vector<spikecounter> &dopa_spikes,
    const STDPDopaCommonProperties &cp) {
...
template <typename targetidentifierT>
inline void STDPDopaConnection<targetidentifierT>::update_weight_(
    double c0, double n0, double minus_dt, const STDPDopaCommonProperties &cp) {
...
template <typename targetidentifierT>
inline void STDPDopaConnection<targetidentifierT>::process_dopa_spikes_(
    const std::vector<spikecounter> &dopa_spikes, double t0, double t1,
    const STDPDopaCommonProperties &cp) {
  ...
  if ((dopa_spikes.size() > dopa_spikes_idx_ + 1) &&
    ...
    // of last dopa spike
    double n0 = n_ * std::exp((dopa_spikes[dopa_spikes_idx_].spike_time_ - t0) /
                              cp.tau_n_); // dopamine trace n at time t0
    update_weight_(c_, n0, t0 - dopa_spikes[dopa_spikes_idx_ + 1].spike_time_,
                   cp);
...
template <typename targetidentifierT>
inline void STDPDopaConnection<targetidentifierT>::facilitate_(
    double kplus, const STDPDopaCommonProperties &cp) {
  c_ += cp.A_plus_ * kplus;
}

template <typename targetidentifierT>
inline void STDPDopaConnection<targetidentifierT>::depress_(
    double kminus, const STDPDopaCommonProperties &cp) {
 ...
 */
template <typename targetidentifierT>
inline void STDPDopaConnection<targetidentifierT>::send(
    Event &e, thread t, const STDPDopaCommonProperties &cp) {
...
template <typename targetidentifierT>
inline void STDPDopaConnection<targetidentifierT>::trigger_update_weight(
    thread t, const std::vector<spikecounter> &dopa_spikes, const double t_trig,
    const STDPDopaCommonProperties &cp) {
  ...
  get_target(t)->get_history(t_last_update_ - dendritic_delay,
                             t_trig - dendritic_delay, &start, &finish);
