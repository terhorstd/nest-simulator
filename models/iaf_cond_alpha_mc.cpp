...
template <> void RecordablesMap<iaf_cond_alpha_mc>::create() {
  insert_(Name("V_m.s"),
...
extern "C" int nest::iaf_cond_alpha_mc_dynamics(double, const double y[],
                                                double f[], void *pnode) {
  // some shorthands
  ...
  for (size_t n = 0; n < N::NCOMP; ++n) {
    ...
    // coupling currents
    const double I_conn =
        (n > N::SOMA ? node.P_.g_conn[n - 1] * (V - y[S::idx(n - 1, S::V_M)])
                     : 0) +
        (n < N::NCOMP - 1 ? node.P_.g_conn[n] * (V - y[S::idx(n + 1, S::V_M)])
                          : 0);
...
nest::iaf_cond_alpha_mc::Parameters_::Parameters_(const Parameters_ &p)
    : V_th(p.V_th), V_reset(p.V_reset), t_ref(p.t_ref) {
  // copy C-arrays
...
nest::iaf_cond_alpha_mc::State_::State_(const Parameters_ &p) : r_(0) {
...
nest::iaf_cond_alpha_mc::State_::State_(const State_ &s) : r_(s.r_) {
  for (size_t i = 0; i < STATE_VEC_SIZE; ++i) {
    y_[i] = s.y_[i];
  }
}

nest::iaf_cond_alpha_mc::State_ &nest::iaf_cond_alpha_mc::State_::
...
void nest::iaf_cond_alpha_mc::update(Time const &origin, const long from,
                                     const long to) {
