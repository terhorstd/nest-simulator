 ...
 */
template <> void RecordablesMap<aeif_psc_exp>::create() {
...
extern "C" int nest::aeif_psc_exp_dynamics(double, const double y[], double f[],
                                           void *pnode) {
  // a shorthand
  ...
  f[S::V_M] = is_refractory
                  ? 0.
                  : (-node.P_.g_L * (V - node.P_.E_L) + I_spike + I_syn_ex -
                     I_syn_in - w + node.P_.I_e + node.B_.I_stim_) /
                        node.P_.C_m;
...
nest::aeif_psc_exp::State_::State_(const Parameters_ &p) : r_(0) {
  y_[0] = p.E_L;
...
nest::aeif_psc_exp::State_::State_(const State_ &s) : r_(s.r_) {
  for (size_t i = 0; i < STATE_VEC_SIZE; ++i) {
    y_[i] = s.y_[i];
  }
}

nest::aeif_psc_exp::State_ &nest::aeif_psc_exp::State_::
...
void nest::aeif_psc_exp::Parameters_::get(DictionaryDatum &d) const {
...
void nest::aeif_psc_exp::Parameters_::set(const DictionaryDatum &d) {
...
void nest::aeif_psc_exp::State_::get(DictionaryDatum &d) const {
...
void nest::aeif_psc_exp::State_::set(const DictionaryDatum &d,
                                     const Parameters_ &) {
...
nest::aeif_psc_exp::aeif_psc_exp() : Archiving_Node(), P_(), S_(P_), B_(*this) {
...
void nest::aeif_psc_exp::init_state_(const Node &proto) {
  const aeif_psc_exp &pr = downcast<aeif_psc_exp>(proto);
  S_ = pr.S_;
}

void nest::aeif_psc_exp::init_buffers_() {
...
void nest::aeif_psc_exp::calibrate() {
...
void nest::aeif_psc_exp::update(const Time &origin, const long from,
                                const long to) {
...
void nest::aeif_psc_exp::handle(SpikeEvent &e) {
...
void nest::aeif_psc_exp::handle(CurrentEvent &e) {
  assert(e.get_delay_steps() > 0);
...
void nest::aeif_psc_exp::handle(DataLoggingRequest &e) { B_.logger_.handle(e); }
