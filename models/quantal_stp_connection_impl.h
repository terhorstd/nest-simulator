...
Quantal_StpConnection<targetidentifierT>::Quantal_StpConnection()
    : ConnectionBase(), weight_(1.0), U_(0.5), u_(U_), tau_rec_(800.0),
      tau_fac_(10.0), n_(1), a_(n_), t_lastspike_(0.0) {}

template <typename targetidentifierT>
Quantal_StpConnection<targetidentifierT>::Quantal_StpConnection(
    const Quantal_StpConnection &rhs)
    : ConnectionBase(rhs), weight_(rhs.weight_), U_(rhs.U_), u_(rhs.u_),
      tau_rec_(rhs.tau_rec_), tau_fac_(rhs.tau_fac_), n_(rhs.n_), a_(rhs.a_),
      t_lastspike_(rhs.t_lastspike_) {}

template <typename targetidentifierT>
void Quantal_StpConnection<targetidentifierT>::get_status(
...
template <typename targetidentifierT>
void Quantal_StpConnection<targetidentifierT>::set_status(
...
template <typename targetidentifierT>
void Quantal_StpConnection<targetidentifierT>::check_synapse_params(
