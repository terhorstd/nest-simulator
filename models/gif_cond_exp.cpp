...
template <> void RecordablesMap<gif_cond_exp>::create() {
...
extern "C" int nest::gif_cond_exp_dynamics(double, const double y[], double f[],
                                           void *pnode) {
  // a shorthand
  ...
  // V dot
  f[0] = (-I_L + node.S_.I_stim_ + node.P_.I_e_ - I_syn_exc - I_syn_inh - stc) /
         node.P_.c_m_;
...
nest::gif_cond_exp::State_::State_(const Parameters_ &p)
    : I_stim_(0.0), sfa_(0.0), stc_(0.0), sfa_elems_(), stc_elems_(),
      r_ref_(0) {
...
nest::gif_cond_exp::State_::State_(const State_ &s)
    : I_stim_(s.I_stim_), sfa_(s.sfa_), stc_(s.stc_), sfa_elems_(),
      stc_elems_(), r_ref_(s.r_ref_) {
...
nest::gif_cond_exp::State_ &nest::gif_cond_exp::State_::
...
void nest::gif_cond_exp::State_::get(DictionaryDatum &d,
...
void nest::gif_cond_exp::State_::set(const DictionaryDatum &d,
                                     const Parameters_ &p) {
...
nest::gif_cond_exp::gif_cond_exp() : Archiving_Node(), P_(), S_(P_), B_(*this) {
...
void nest::gif_cond_exp::update(Time const &origin, const long from,
                                const long to) {
  ...
  for (long lag = from; lag < to; ++lag) {
    ...
    if (S_.r_ref_ == 0) // neuron is not in refractory period
    {
      ...
      const double lambda =
          P_.lambda_0_ *
          std::exp((S_.neuron_state_[State_::V_M] - S_.sfa_) / P_.Delta_V_);
...
void nest::gif_cond_exp::handle(DataLoggingRequest &e) { B_.logger_.handle(e); }
