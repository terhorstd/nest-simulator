...
void nest::inhomogeneous_poisson_generator::Parameters_::get(
...
void nest::inhomogeneous_poisson_generator::Parameters_::
...
void nest::inhomogeneous_poisson_generator::Parameters_::set(
    const DictionaryDatum &d, Buffers_ &b) {
  const bool times = d->known(names::rate_times);
  const bool rates =
      updateValue<std::vector<double_t>>(d, names::rate_values, rate_values_);
  ...
  if (d->known(names::allow_offgrid_times)) {
    ...
    if (flag_offgrid != allow_offgrid_times_ and
        not(times or rate_times_.empty())) {
      throw BadProperty("Option can only be set together with rate times "
...
nest::inhomogeneous_poisson_generator::inhomogeneous_poisson_generator()
    : DeviceNode(), device_(), P_(), B_(), V_() {}

nest::inhomogeneous_poisson_generator::inhomogeneous_poisson_generator(
    const inhomogeneous_poisson_generator &n)
    : DeviceNode(n), device_(n.device_), P_(n.P_), B_(n.B_), V_(n.V_) {}

/* ----------------------------------------------------------------
 * Node initialization functions
 * ---------------------------------------------------------------- */
void nest::inhomogeneous_poisson_generator::init_state_(const Node &proto) {
...
void nest::inhomogeneous_poisson_generator::init_buffers_() {
  device_.init_buffers();
  B_.idx_ = 0;
  B_.rate_ = 0;
}

void nest::inhomogeneous_poisson_generator::calibrate() {
  device_.calibrate();
...
void nest::inhomogeneous_poisson_generator::update(Time const &origin,
                                                   const long from,
                                                   const long to) {
  assert(to >= 0 and
  ...
  while (B_.idx_ < P_.rate_times_.size() and
         P_.rate_times_[B_.idx_].get_steps() <= first) {
    ++B_.idx_;
  }
  ...
  for (long offs = from; offs < to; ++offs) {
    ...
    if (B_.idx_ < P_.rate_times_.size() and
        curr_time + 1 == P_.rate_times_[B_.idx_].get_steps()) {
...
void nest::inhomogeneous_poisson_generator::event_hook(DSSpikeEvent &e) {
  librandom::RngPtr rng = kernel().rng_manager.get_rng(get_thread());
