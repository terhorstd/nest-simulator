...
template <> void RecordablesMap<izhikevich>::create() {
...
void nest::izhikevich::Parameters_::set(const DictionaryDatum &d) {
  ...
  if (not consistent_integration_ && h != 1.0) {
    LOG(M_INFO, "Parameters_::set",
        "Use 1.0 ms as resolution for consistency.");
  }
}
...
nest::izhikevich::Buffers_::Buffers_(izhikevich &n) : logger_(n) {}
...
nest::izhikevich::izhikevich() : Archiving_Node(), P_(), S_(), B_(*this) {
...
void nest::izhikevich::calibrate() { B_.logger_.init(); }

/* ----------------------------------------------------------------
 * Update and spike handling functions
 */

void nest::izhikevich::update(Time const &origin, const long from,
                              const long to) {
  ...
  for (long lag = from; lag < to; ++lag) {
    ...
    // recommended)
      ...
      S_.v_ += h * 0.5 *
               (0.04 * S_.v_ * S_.v_ + 5.0 * S_.v_ + 140.0 - S_.u_ + S_.I_ +
                P_.I_e_ + I_syn);
      S_.v_ += h * 0.5 *
               (0.04 * S_.v_ * S_.v_ + 5.0 * S_.v_ + 140.0 - S_.u_ + S_.I_ +
                P_.I_e_ + I_syn);
...
void nest::izhikevich::handle(DataLoggingRequest &e) { B_.logger_.handle(e); }
