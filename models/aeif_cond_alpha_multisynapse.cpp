...
template <>
void DynamicRecordablesMap<aeif_cond_alpha_multisynapse>::create(
    aeif_cond_alpha_multisynapse &host) {
  // use standard names wherever you can for consistency!
  insert(names::V_m, host.get_data_access_functor(
...
Name aeif_cond_alpha_multisynapse::get_g_receptor_name(size_t receptor) {
  std::stringstream receptor_name;
  receptor_name << "g_" << receptor + 1;
  return Name(receptor_name.str());
}

void aeif_cond_alpha_multisynapse::insert_conductance_recordables(
    size_t first) {
...
extern "C" int aeif_cond_alpha_multisynapse_dynamics(double, const double y[],
                                                     double f[], void *pnode) {
...
aeif_cond_alpha_multisynapse::State_::State_(const State_ &s) : r_(s.r_) {
  y_ = s.y_;
}
...
void aeif_cond_alpha_multisynapse::Parameters_::set(const DictionaryDatum &d) {
  ...
  if (Erev_flag || tau_flag) { // receptor arrays have been modified
    if ((E_rev.size() != old_n_receptors ||
         tau_syn.size() != old_n_receptors) and
...
aeif_cond_alpha_multisynapse::Buffers_::Buffers_(
    ...
    : logger_(n), s_(0), c_(0), e_(0), step_(Time::get_resolution().get_ms()),
      IntegrationStep_(std::min(0.01, step_)), I_stim_(0.0) {}

aeif_cond_alpha_multisynapse::Buffers_::Buffers_(
    const Buffers_ &b, aeif_cond_alpha_multisynapse &n)
    : logger_(n), s_(0), c_(0), e_(0), step_(b.step_),
      IntegrationStep_(b.IntegrationStep_), I_stim_(b.I_stim_) {}
 ...
 * ---------------------------------------------------------------- */
void aeif_cond_alpha_multisynapse::update(Time const &origin, const long from,
                                          const long to) {
