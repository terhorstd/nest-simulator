 ...
 */
extern "C" int iaf_cond_exp_sfa_rr_dynamics(double, const double *, double *,
                                            void *);
...
private:
  ...
  friend int iaf_cond_exp_sfa_rr_dynamics(double, const double *, double *,
                                          void *);
...
inline port nest::iaf_cond_exp_sfa_rr::send_test_event(Node &target,
                                                       rport receptor_type,
                                                       synindex, bool) {
  SpikeEvent e;
  e.set_sender(*this);
...
inline port iaf_cond_exp_sfa_rr::handles_test_event(SpikeEvent &,
                                                    rport receptor_type) {
  if (receptor_type != 0) {
...
inline port iaf_cond_exp_sfa_rr::handles_test_event(CurrentEvent &,
                                                    rport receptor_type) {
  if (receptor_type != 0) {
    throw UnknownReceptorType(receptor_type, get_name());
  }
...
inline port iaf_cond_exp_sfa_rr::handles_test_event(DataLoggingRequest &dlr,
                                                    rport receptor_type) {
  if (receptor_type != 0) {
    throw UnknownReceptorType(receptor_type, get_name());
  }
...
inline void iaf_cond_exp_sfa_rr::get_status(DictionaryDatum &d) const {
  P_.get(d);
  S_.get(d);
...
inline void iaf_cond_exp_sfa_rr::set_status(const DictionaryDatum &d) {
