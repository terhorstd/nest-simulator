...
template <> void RecordablesMap<gif_psc_exp>::create() {
...
nest::gif_psc_exp::State_::State_()
    : I_stim_(0.0), V_(-70.0), sfa_(0.0), stc_(0.0), sfa_elems_(), stc_elems_(),
      I_syn_ex_(0.0), I_syn_in_(0.0), r_ref_(0) {}
...
void nest::gif_psc_exp::State_::get(DictionaryDatum &d,
...
void nest::gif_psc_exp::State_::set(const DictionaryDatum &d,
                                    const Parameters_ &p) {
  updateValue<double>(d, names::V_m, V_);
}

nest::gif_psc_exp::Buffers_::Buffers_(gif_psc_exp &n) : logger_(n) {}
...
nest::gif_psc_exp::gif_psc_exp() : Archiving_Node(), P_(), S_(), B_(*this) {
...
void nest::gif_psc_exp::update(Time const &origin, const long from,
                               const long to) {
...
void nest::gif_psc_exp::handle(DataLoggingRequest &e) { B_.logger_.handle(e); }
