...
nest::rate_neuron_ipn<TNonlinearities>::Parameters_::Parameters_()
    : tau_(10.0) // ms
      ...
      lambda_(1.0) // ms
      ,
      sigma_(1.0), mu_(0.0), linear_summation_(true), rectify_output_(false),
...
template <class TNonlinearities>
void nest::rate_neuron_ipn<TNonlinearities>::Parameters_::get(
...
template <class TNonlinearities>
void nest::rate_neuron_ipn<TNonlinearities>::Parameters_::set(
  ...
  if (updateValue<double>(d, names::mean, mu_)) {
    LOG(M_WARNING, "rate_neuron_ipn< TNonlinearities >::Parameters_::set",
...
template <class TNonlinearities>
void nest::rate_neuron_ipn<TNonlinearities>::State_::get(
    DictionaryDatum &d) const {
...
template <class TNonlinearities>
void nest::rate_neuron_ipn<TNonlinearities>::State_::set(
    const DictionaryDatum &d) {
...
template <class TNonlinearities>
nest::rate_neuron_ipn<TNonlinearities>::Buffers_::Buffers_(
    const Buffers_ &, rate_neuron_ipn<TNonlinearities> &n)
    : logger_(n) {}
...
nest::rate_neuron_ipn<TNonlinearities>::rate_neuron_ipn(
    ...
    : Archiving_Node(n), nonlinearities_(n.nonlinearities_), P_(n.P_), S_(n.S_),
      B_(n.B_, *this) {
  Node::set_node_uses_wfr(kernel().simulation_manager.use_wfr());
}
...
template <class TNonlinearities>
void nest::rate_neuron_ipn<TNonlinearities>::init_state_(const Node &proto) {
  const rate_neuron_ipn &pr = downcast<rate_neuron_ipn>(proto);
  S_ = pr.S_;
}

template <class TNonlinearities>
void nest::rate_neuron_ipn<TNonlinearities>::init_buffers_() {
...
template <class TNonlinearities>
void nest::rate_neuron_ipn<TNonlinearities>::calibrate() {
  B_.logger_
...
template <class TNonlinearities>
bool nest::rate_neuron_ipn<TNonlinearities>::update_(
    Time const &origin, const long from, const long to,
  ...
  for (long lag = from; lag < to; ++lag) {
    ...
    if (P_.linear_summation_) {
      ...
      if (P_.mult_coupling_) {
        ...
        S_.rate_ +=
            V_.P2_ * nonlinearities_.input(delayed_rates_ex + instant_rates_ex +
...
template <class TNonlinearities>
void nest::rate_neuron_ipn<TNonlinearities>::handle(
...
template <class TNonlinearities>
void nest::rate_neuron_ipn<TNonlinearities>::handle(DataLoggingRequest &e) {
  B_.logger_.handle(e);
}
