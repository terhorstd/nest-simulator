 ...
 */
template <typename targetidentifierT>
inline void STDPPLConnectionHom<targetidentifierT>::send(
    Event &e, thread t, const STDPPLHomCommonProperties &cp) {
  ...
  while (start != finish) {
    ...
    weight_ = facilitate_(weight_,
                          Kplus_ * std::exp(minus_dt * cp.tau_plus_inv_), cp);
  }
...
template <typename targetidentifierT>
void STDPPLConnectionHom<targetidentifierT>::get_status(
...
template <typename targetidentifierT>
void STDPPLConnectionHom<targetidentifierT>::set_status(
