...
template <>
void DynamicRecordablesMap<aeif_cond_beta_multisynapse>::create(
...
Name aeif_cond_beta_multisynapse::get_g_receptor_name(size_t receptor) {
  std::stringstream receptor_name;
  receptor_name << "g_" << receptor + 1;
  return Name(receptor_name.str());
}

void aeif_cond_beta_multisynapse::insert_conductance_recordables(size_t first) {
...
void aeif_cond_beta_multisynapse::Parameters_::set(const DictionaryDatum &d) {
  ...
  if (Erev_flag || taur_flag ||
      taud_flag) { // receptor arrays have been modified
...
void aeif_cond_beta_multisynapse::calibrate() {
  ...
  for (size_t i = 0; i < P_.n_receptors(); ++i) {
    ...
    if (denom1 != 0) {
      ...
      denom2 =
          std::exp(-t_p / P_.tau_decay[i]) - std::exp(-t_p / P_.tau_rise[i]);
    }
...
void aeif_cond_beta_multisynapse::set_status(const DictionaryDatum &d) {
