...
nest::correlospinmatrix_detector::Parameters_::Parameters_()
    : delta_tau_(Time::get_resolution()), tau_max_(10 * delta_tau_),
      Tstart_(Time::ms(0.0)), Tstop_(Time::pos_inf()), N_channels_(1) {}

nest::correlospinmatrix_detector::Parameters_::Parameters_(const Parameters_ &p)
    : delta_tau_(p.delta_tau_), tau_max_(p.tau_max_), Tstart_(p.Tstart_),
      Tstop_(p.Tstop_), N_channels_(p.N_channels_) {
...
nest::correlospinmatrix_detector::State_::State_()
    : incoming_(), last_i_(0), t_last_in_spike_(Time::neg_inf()),
      count_covariance_(
          1, std::vector<std::vector<long>>(1, std::vector<long>())) {}

/* ----------------------------------------------------------------
 * Parameter extraction and manipulation functions
 * ---------------------------------------------------------------- */

void nest::correlospinmatrix_detector::Parameters_::get(
...
void nest::correlospinmatrix_detector::State_::get(DictionaryDatum &d) const {
  ...
  for (size_t i = 0; i < count_covariance_.size(); ++i) {
    ...
    for (size_t j = 0; j < count_covariance_[i].size(); ++j) {
      CountC_i->push_back(
          new IntVectorDatum(new std::vector<long>(count_covariance_[i][j])));
    }
...
bool nest::correlospinmatrix_detector::Parameters_::set(
    const DictionaryDatum &d, const correlospinmatrix_detector &n) {
  bool reset = false;
  double t;
  long N;
...
void nest::correlospinmatrix_detector::State_::set(const DictionaryDatum &,
                                                   const Parameters_ &, bool) {}
...
nest::correlospinmatrix_detector::correlospinmatrix_detector(
    ...
    : Node(n), device_(n.device_), P_(n.P_), S_() {
  if (not P_.delta_tau_.is_step()) {
...
void nest::correlospinmatrix_detector::calibrate() { device_.calibrate(); }

/* ----------------------------------------------------------------
 * Other functions
 * ---------------------------------------------------------------- */

void nest::correlospinmatrix_detector::update(Time const &, const long,
                                              const long) {}
...
void nest::correlospinmatrix_detector::handle(SpikeEvent &e) {
  ...
  if (device_.is_active(stamp)) {
    ...
    if (m == 1) { // multiplicity == 1, either a single 1->0 event or the first
                  // or second of
      // a pair of 0->1
      // events
      if (curr_i == S_.last_i_ && stamp == S_.t_last_in_spike_) {
        ...
        S_.tentative_down_ = false; // previous event was first event of two, so
                                    // no down transition
      } else {
    ...
    if (down_transition) // only do something on the downtransitions
    {
      ...
      BinaryPulselistType::iterator insert_pos = std::find_if(
          S_.incoming_.begin(), S_.incoming_.end(),
      ...
      for (BinaryPulselistType::const_iterator pulse_j = otherPulses.begin();
           pulse_j != otherPulses.end(); ++pulse_j) {
        // id of other neuron
