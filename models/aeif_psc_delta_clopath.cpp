 ...
 */
template <> void RecordablesMap<aeif_psc_delta_clopath>::create() {
  // use standard names whereever you can for consistency!
  insert_(names::V_m, &aeif_psc_delta_clopath::get_y_elem_<
  ...
  insert_(names::V_th, &aeif_psc_delta_clopath::get_y_elem_<
                           aeif_psc_delta_clopath::State_::V_TH>);
  insert_(names::u_bar_plus, &aeif_psc_delta_clopath::get_y_elem_<
                                 aeif_psc_delta_clopath::State_::U_BAR_PLUS>);
  insert_(names::u_bar_minus, &aeif_psc_delta_clopath::get_y_elem_<
                                  aeif_psc_delta_clopath::State_::U_BAR_MINUS>);
  insert_(names::u_bar_bar, &aeif_psc_delta_clopath::get_y_elem_<
                                aeif_psc_delta_clopath::State_::U_BAR_BAR>);
}
} // namespace nest

extern "C" int nest::aeif_psc_delta_clopath_dynamics(double, const double y[],
                                                     double f[], void *pnode) {
  // a shorthand
  ...
  f[S::V_M] = (is_refractory || is_clamped)
                  ? 0.0
                  : (-node.P_.g_L * (V - node.P_.E_L) + I_spike - w + z +
...
nest::aeif_psc_delta_clopath::Parameters_::Parameters_()
    : V_peak_(33.0) // mV
      ...
      I_e(0.0) // pA
      ,
      gsl_error_tol(1e-6), t_clamp_(2.0) // ms
      ,
      V_clamp_(33.0) // mV
{}
...
void nest::aeif_psc_delta_clopath::Parameters_::get(DictionaryDatum &d) const {
...
void nest::aeif_psc_delta_clopath::Parameters_::set(const DictionaryDatum &d) {
  ...
  if (tau_w <= 0 or tau_V_th <= 0 or tau_w <= 0 or tau_z <= 0 or
      tau_plus <= 0 or tau_minus <= 0 or tau_bar_bar <= 0) {
...
void nest::aeif_psc_delta_clopath::State_::get(DictionaryDatum &d) const {
...
void nest::aeif_psc_delta_clopath::State_::set(const DictionaryDatum &d,
                                               const Parameters_ &) {
...
nest::aeif_psc_delta_clopath::aeif_psc_delta_clopath()
    : Clopath_Archiving_Node(), P_(), S_(P_), B_(*this) {
  recordablesMap_.create();
}

nest::aeif_psc_delta_clopath::aeif_psc_delta_clopath(
    const aeif_psc_delta_clopath &n)
    : Clopath_Archiving_Node(n), P_(n.P_), S_(n.S_), B_(n.B_, *this) {}
...
void nest::aeif_psc_delta_clopath::init_state_(const Node &proto) {
  const aeif_psc_delta_clopath &pr = downcast<aeif_psc_delta_clopath>(proto);
  S_ = pr.S_;
}

void nest::aeif_psc_delta_clopath::init_buffers_() {
...
void nest::aeif_psc_delta_clopath::calibrate() {
...
void nest::aeif_psc_delta_clopath::update(const Time &origin, const long from,
                                          const long to) {
  ...
  for (long lag = from; lag < to; ++lag) {
    ...
    write_clopath_history(Time::step(origin.get_steps() + lag + 1),
                          S_.y_[State_::V_M], S_.y_[State_::U_BAR_PLUS],
                          S_.y_[State_::U_BAR_MINUS], S_.y_[State_::U_BAR_BAR]);
...
void nest::aeif_psc_delta_clopath::handle(SpikeEvent &e) {
...
void nest::aeif_psc_delta_clopath::handle(CurrentEvent &e) {
  assert(e.get_delay_steps() > 0);
...
void nest::aeif_psc_delta_clopath::handle(DataLoggingRequest &e) {
  B_.logger_.handle(e);
}
