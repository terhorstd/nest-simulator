...
index create_layer(const DictionaryDatum &layer_dict) {
  ...
  ALL_ENTRIES_ACCESSED(*layer_dict, "topology::CreateLayer",
...
MaskDatum create_mask(const DictionaryDatum &mask_dict) {
  ...
  ALL_ENTRIES_ACCESSED(*mask_dict, "topology::CreateMask",
                       "Unread dictionary entries: ");
...
ArrayDatum get_global_children(const index gid, const MaskDatum &maskd,
...
void connect_layers(const index source_gid, const index target_gid,
  ...
  ALL_ENTRIES_ACCESSED(*connection_dict, "topology::CreateLayers",
                       "Unread dictionary entries: ");
...
ParameterDatum create_parameter(const DictionaryDatum &param_dict) {
  ...
  ALL_ENTRIES_ACCESSED(*param_dict, "topology::CreateParameter",
                       "Unread dictionary entries: ");
...
void dump_layer_connections(const Token &syn_model, const index layer_gid,
...
std::vector<index> get_element(const index layer_gid, const TokenArray array) {
  ...
  case 2: {
    ...
    node_gids = layer->get_nodes(Position<2, int>(
