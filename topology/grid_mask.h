 ...
 */
template <int D> class GridMask : public AbstractMask {
public:
  /**
   * Parameters:
  ...
  bool inside(const std::vector<double> &) const {
  ...
  GridMask<D> *clone() const { return new GridMask(*this); }
  ...
  Position<D, int> get_upper_left() const { return upper_left_; }

  Position<D, int> get_lower_right() const { return lower_right_; }
...
template <int D> GridMask<D>::GridMask(const DictionaryDatum &d) {
...
template <> inline Name GridMask<2>::get_name() { return names::grid; }

template <> inline Name GridMask<3>::get_name() { return names::grid3d; }

template <int D> DictionaryDatum GridMask<D>::get_dict() const {
...
template <int D> void GridMask<D>::set_anchor(const Position<D, int> &anchor) {
