 ...
 */
template <int D> class GridLayer : public Layer<D> {
public:
  ...
  public:
     ...
     */
    masked_iterator(const GridLayer<D> &layer, const Mask<D> &mask,
                    const Position<D> &anchor, const Selector &filter);
  ...
  GridLayer(const GridLayer &layer) : Layer<D>(layer), dims_(layer.dims_) {}
  ...
  get_global_positions_vector(Selector filter, const AbstractMask &mask,
...
protected:
  ...
  void insert_global_positions_vector_(
      std::vector<std::pair<Position<D>, index>> &vec, const Selector &filter);
  void insert_local_positions_ntree_(Ntree<D, index> &tree,
                                     const Selector &filter);
};

template <int D> Position<D, index> GridLayer<D>::get_dims() const {
  return dims_;
}

template <int D> void GridLayer<D>::set_status(const DictionaryDatum &d) {
...
template <int D> void GridLayer<D>::get_status(DictionaryDatum &d) const {
...
template <int D> Position<D> GridLayer<D>::lid_to_position(index lid) const {
...
template <int D>
Position<D> GridLayer<D>::gridpos_to_position(Position<D, int> gridpos) const {
...
template <int D> Position<D> GridLayer<D>::get_position(index sind) const {
  return lid_to_position(this->nodes_[sind]->get_lid());
}

template <int D>
index GridLayer<D>::gridpos_to_lid(Position<D, int> pos) const {
  index lid = 0;
...
template <int D>
std::vector<index> GridLayer<D>::get_nodes(Position<D, int> pos) {
...
template <int D>
void GridLayer<D>::insert_local_positions_ntree_(Ntree<D, index> &tree,
                                                 const Selector &filter) {
...
template <int D>
void GridLayer<D>::insert_global_positions_vector_(
    std::vector<std::pair<Position<D>, index>> &vec, const Selector &filter) {
...
template <int D>
inline typename GridLayer<D>::masked_iterator GridLayer<D>::masked_end() {
  ...
  for (int i = 0; i < D; ++i) {
    if (layer.periodic_[i]) {
      lower_left[i] = ceil((bbox.lower_left[i] - layer.lower_left_[i]) *
                           ...
                           0.5);
      upper_right[i] = round((bbox.upper_right[i] - layer.lower_left_[i]) *
                             layer_.dims_[i] / layer.extent_[i]);
    } else {
      ...
      upper_right[i] = std::min(
          index(std::max(round((bbox.upper_right[i] - layer.lower_left_[i]) *
                               layer_.dims_[i] / layer.extent_[i]),
                         0.0)),
          layer.dims_[i]);
    }
...
operator*() {
  ...
  return std::pair<Position<D>, index>(
      layer_.gridpos_to_position(node_),
