...
template <int D> lockPTR<Ntree<D, index>> Layer<D>::cached_ntree_;

template <int D>
std::vector<std::pair<Position<D>, index>> *Layer<D>::cached_vector_ = 0;

template <int D> Selector Layer<D>::cached_selector_;
...
template <int D> void Layer<D>::set_status(const DictionaryDatum &d) {
...
template <int D> void Layer<D>::get_status(DictionaryDatum &d) const {
  Subnet::get_status(d);
...
template <int D>
void Layer<D>::connect(AbstractLayer &target_layer,
...
Layer<D>::get_global_positions_ntree(Selector filter, std::bitset<D> periodic,
...
template <int D>
std::vector<std::pair<Position<D>, index>>
Layer<D>::get_global_positions_vector(Selector filter, const MaskDatum &mask,
...
template <int D>
std::vector<index> Layer<D>::get_global_nodes(const MaskDatum &mask,
  ...
  for (typename Ntree<D, index>::masked_iterator i = masked_layer.begin(anchor);
       i != masked_layer.end(); ++i) {
    nodes.push_back(i->second);
  }
  return nodes;
}

template <int D> void Layer<D>::dump_nodes(std::ostream &out) const {
...
void Layer<D>::dump_connections(std::ostream &out, const Token &syn_model) {
  ...
  for (typename std::vector<std::pair<Position<D>, index>>::iterator src_iter =
           src_vec->begin();
    ...
    for (size_t i = 0; i < connectome.size(); ++i) {
      ...
      if (tgt_layer == 0) {
        ...
        out << ' ';
        tgt_layer->compute_displacement(source_pos, target->get_subnet_index())
            .print(out);
      }
...
void MaskedLayer<D>::check_mask_(Layer<D> &layer, bool allow_oversized) {
  ...
  try // Try to cast to GridMask
  {
    ...
    if (not allow_oversized) {
      bool oversize = false;
      for (int i = 0; i < D; ++i) {
        oversize |= layer.get_periodic_mask()[i] and
                    (grid_mask.get_lower_right()[i] -
    ...
    try // Try to cast to correct dimension Mask
    {
      ...
      if (not allow_oversized) {
        ...
        for (int i = 0; i < D; ++i) {
          oversize |=
              layer.get_periodic_mask()[i] and
              (bb.upper_right[i] - bb.lower_left[i]) > layer.get_extent()[i];
        }
        if (oversize) {
          throw BadProperty("Mask size must not exceed layer size; set "
                            "allow_oversized_mask to "
                            "override.");
        }
      }
    } catch (std::bad_cast &) {
