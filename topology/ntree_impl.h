...
static inline double mod(double x, double p) {
  x = std::fmod(x, p);
  if (x < 0) {
    x += p;
  }
...
Ntree<D, T, max_capacity, max_depth>::masked_iterator::masked_iterator(
    Ntree<D, T, max_capacity, max_depth> &q, const Mask<D> &mask,
    const Position<D> &anchor)
    : ntree_(&q), top_(&q), allin_top_(0), node_(0), mask_(&mask),
      anchor_(anchor), anchors_(), current_anchor_(0) {
...
void Ntree<D, T, max_capacity, max_depth>::masked_iterator::init_() {
  ...
  if (mask_->outside(Box<D>(ntree_->lower_left_ - anchor_,
    ...
    if (mask_->inside(
            Box<D>(ntree_->lower_left_ - anchor_,
                   ntree_->lower_left_ - anchor_ + ntree_->extent_))) {
      first_leaf_inside_();
    } else {
      first_leaf_();
...
void Ntree<D, T, max_capacity, max_depth>::masked_iterator::next_leaf_() {
  ...
  do {
    ...
    if (mask_->inside(
      ...
      mask_->outside(Box<D>(ntree_->lower_left_ - anchor_,
                            ntree_->lower_left_ - anchor_ + ntree_->extent_)));
...
template <int D, class T, int max_capacity, int max_depth>
void Ntree<D, T, max_capacity,
           max_depth>::masked_iterator::first_leaf_inside_() {
...
template <int D, class T, int max_capacity, int max_depth>
void Ntree<D, T, max_capacity, max_depth>::append_nodes_(
...
template <int D, class T, int max_capacity, int max_depth>
void Ntree<D, T, max_capacity, max_depth>::append_nodes_(
    std::vector<std::pair<Position<D>, T>> &v, const Mask<D> &mask,
    const Position<D> &anchor) {
  if (mask.outside(
...
Ntree<D, T, max_capacity, max_depth>::insert(Position<D> pos, const T &node) {
  if (periodic_.any()) {
    ...
    for (int i = 0; i < D; ++i) {
      if (periodic_[i]) {
        pos[i] =
            lower_left_[i] + std::fmod(pos[i] - lower_left_[i], extent_[i]);
