 ...
 */
template <int D> class FreeLayer : public Layer<D> {
public:
...
template <int D> void FreeLayer<D>::set_status(const DictionaryDatum &d) {
  ...
  if (d->known(names::positions)) {
    ...
    for (std::vector<Node *>::iterator i = this->local_begin();
      ...
      Position<D> point =
          getValue<std::vector<double>>(pos[(*i)->get_lid() % nodes_per_depth]);
      if (not((point >= this->lower_left_) and
              (point < this->lower_left_ + this->extent_))) {
...
template <int D> void FreeLayer<D>::get_status(DictionaryDatum &d) const {
...
template <int D> Position<D> FreeLayer<D>::get_position(index sind) const {
...
void FreeLayer<D>::communicate_positions_(Ins iter, const Selector &filter) {
  ...
  kernel().mpi_manager.communicate(local_gid_pos, global_gid_pos,
                                   displacements);
  ...
  for (; pos_ptr < pos_end; pos_ptr++) {
    *iter++ = std::pair<Position<D>, index>(pos_ptr->get_position(),
                                            pos_ptr->get_gid());
  }
}
...
template <int D>
void FreeLayer<D>::insert_local_positions_ntree_(Ntree<D, index> &tree,
                                                 const Selector &filter) {
  assert(this->nodes_.size() >= positions_.size());
...
template <int D>
static bool gid_less(const std::pair<Position<D>, index> &a,
                     const std::pair<Position<D>, index> &b) {
  return a.second < b.second;
}

template <int D>
void FreeLayer<D>::insert_global_positions_vector_(
    std::vector<std::pair<Position<D>, index>> &vec, const Selector &filter) {
