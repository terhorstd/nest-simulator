...
template <int D>
void ConnectionCreator::get_parameters_(const Position<D> &pos,
                                        librandom::RngPtr rng, double &weight,
                                        double &delay) {
...
void ConnectionCreator::connect_to_target_(Iterator from, Iterator to,
                                           Node *tgt_ptr,
  ...
  for (Iterator iter = from; iter != to; ++iter) {
    ...
    if (without_kernel or
        rng->drand() <
            kernel_->value(source.compute_displacement(tgt_pos, iter->first),
                           rng)) {
      const Position<D> disp =
          source.compute_displacement(tgt_pos, iter->first);
      connect_(iter->second, tgt_ptr, tgt_thread, weight_->value(disp, rng),
               delay_->value(disp, rng), synapse_model_);
    }
  }
}
...
ConnectionCreator::PoolWrapper_<D>::PoolWrapper_()
    : masked_layer_(0), positions_(0) {}

template <int D> ConnectionCreator::PoolWrapper_<D>::~PoolWrapper_() {
  if (masked_layer_) {
...
template <int D>
void ConnectionCreator::PoolWrapper_<D>::define(
...
template <int D>
void ConnectionCreator::target_driven_connect_(Layer<D> &source,
                                               Layer<D> &target) {
  ...
  {
    pool.define(new MaskedLayer<D>(source, source_filter_, mask_, true,
                                   allow_oversized_));
  } else {
    ...
    for (std::vector<Node *>::const_iterator tgt_it = target_begin;
      ...
      if (mask_.valid()) {
        connect_to_target_(pool.masked_begin(target_pos), pool.masked_end(),
                           tgt, target_pos, thread_id, source);
      } else {
        connect_to_target_(pool.begin(), pool.end(), tgt, target_pos, thread_id,
                           source);
      }
    } // for target_begin
  }   // omp parallel
...
template <int D>
void ConnectionCreator::source_driven_connect_(Layer<D> &source,
                                               Layer<D> &target) {
  ...
  for (std::vector<Node *>::const_iterator tgt_it = target_begin;
    ...
    if (not(*tgt_it)->has_proxies()) {
      throw IllegalConnection("Topology Divergent connections"
  ...
  if (mask_.valid()) {
    ...
    MaskedLayer<D> masked_layer(source, source_filter_, mask_, true,
                                allow_oversized_, target);
      ...
      if (kernel_.valid()) {
        ...
        for (typename Ntree<D, index>::masked_iterator iter =
          ...
          if (rng->drand() <
            ...
            double w, d;
            get_parameters_(
                target.compute_displacement(iter->first, target_pos), rng, w,
                d);
            kernel().connection_manager.connect(iter->second, *tgt_it,
                                                target_thread, synapse_model_,
                                                dummy_param_, d, w);
          }
        }
      } else {
        ...
        for (typename Ntree<D, index>::masked_iterator iter =
          ...
          double w, d;
          get_parameters_(target.compute_displacement(iter->first, target_pos),
                          rng, w, d);
          kernel().connection_manager.connect(iter->second, *tgt_it,
                                              target_thread, synapse_model_,
                                              dummy_param_, d, w);
...
template <int D>
void ConnectionCreator::convergent_connect_(Layer<D> &source,
                                            Layer<D> &target) {
  ...
  if (mask_.valid()) {
    MaskedLayer<D> masked_source(source, source_filter_, mask_, true,
                                 allow_oversized_);
    ...
    for (std::vector<Node *>::const_iterator tgt_it = target_begin;
      ...
      if (kernel_.valid()) {
        ...
        if (positions.empty() or
            ((not allow_autapses_) and (positions.size() == 1) and
             (positions[0].second == target_id)) or
            ((not allow_multapses_) and
             (positions.size() < number_of_connections_))) {
        ...
        for (int i = 0; i < (int)number_of_connections_; ++i) {
          ...
          get_parameters_(source.compute_displacement(
                          ...
                          rng, w, d);
          kernel().connection_manager.connect(source_id, *tgt_it, target_thread,
                                              synapse_model_, dummy_param_, d,
                                              w);
    ...
    for (std::vector<Node *>::const_iterator tgt_it = target_begin;
      ...
      if (kernel_.valid()) {
        ...
        for (int i = 0; i < (int)number_of_connections_; ++i) {
          ...
          double w, d;
          get_parameters_(source.compute_displacement(target_pos, source_pos),
                          rng, w, d);
          kernel().connection_manager.connect(source_id, *tgt_it, target_thread,
                                              synapse_model_, dummy_param_, d,
                                              w);
...
void ConnectionCreator::divergent_connect_(Layer<D> &source, Layer<D> &target) {
  ...
  MaskedLayer<D> masked_target(target, target_filter_, mask_, true,
                               allow_oversized_);

  std::vector<std::pair<Position<D>, index>> *sources =
      source.get_global_positions_vector(source_filter_);

  for (typename std::vector<std::pair<Position<D>, index>>::iterator src_it =
           sources->begin();
    ...
    for (typename Ntree<D, index>::masked_iterator tgt_it =
         ...
         tgt_it != masked_target.end(); ++tgt_it) {
    ...
    if (targets.empty() or ((not allow_multapses_) and
                            (targets.size() < number_of_connections_))) {
      std::string msg = String::compose(
    ...
    for (long i = 0; i < (long)number_of_connections_; ++i) {
      ...
      kernel().connection_manager.connect(source_id, target_ptr,
                                          target_ptr->get_thread(),
                                          synapse_model_, dummy_param_, d, w);
    }
  }
}
