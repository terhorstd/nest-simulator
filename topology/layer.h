...
public:
  ...
  compute_displacement(const std::vector<double> &from_pos,
 ...
 */
template <int D> class Layer : public AbstractLayer {
public:
  /**
   ...
   */
  const Position<D> &get_lower_left() const { return lower_left_; }

  /**
   * @returns extent of layer.
   */
  const Position<D> &get_extent() const { return extent_; }

  /**
   * @returns center of layer.
   */
  Position<D> get_center() const { return lower_left_ + extent_ / 2; }

  /**
   * @returns a bitmask specifying which directions are periodic
   */
  std::bitset<D> get_periodic_mask() const { return periodic_; }
  ...
  std::vector<double> compute_displacement(const std::vector<double> &from_pos,
                                           const index to) const;
  ...
  get_local_positions_ntree(Selector filter = Selector());
   ...
   */
  lockPTR<Ntree<D, index>>
  get_global_positions_ntree(Selector filter = Selector());
  ...
  get_global_positions_vector(Selector filter, const MaskDatum &mask,
                              const Position<D> &anchor, bool allow_oversized);
   ...
   * @returns false
   */
  bool is_subnet() const { return false; }
...
protected:
  ...
  lockPTR<Ntree<D, index>>
  do_get_global_positions_ntree_(const Selector &filter);
   ...
   */
  virtual void
  insert_global_positions_vector_(std::vector<std::pair<Position<D>, index>> &,
                                  const Selector &filter) = 0;
...
public:
   ...
   *                        periodic b.c.
   */
  MaskedLayer(Layer<D> &layer, Selector filter, const MaskDatum &mask,
              bool include_global, bool allow_oversized);
   ...
   */
  MaskedLayer(Layer<D> &layer, Selector filter, const MaskDatum &mask,
              bool include_global, bool allow_oversized, Layer<D> &target);
...
template <int D>
inline MaskedLayer<D>::MaskedLayer(Layer<D> &layer, Selector filter,
                                   const MaskDatum &maskd, bool include_global,
                                   bool allow_oversized)
    : mask_(maskd) {
  if (include_global) {
...
inline MaskedLayer<D>::MaskedLayer(Layer<D> &layer, Selector filter,
                                   const MaskDatum &maskd, bool include_global,
                                   bool allow_oversized, Layer<D> &target)
    : mask_(maskd) {
  if (include_global) {
    ntree_ = layer.get_global_positions_ntree(
        filter, target.get_periodic_mask(), target.get_lower_left(),
        target.get_extent());
  }
  // else
  //  ntree_ = layer.get_local_positions_ntree(filter,
...
template <int D> inline MaskedLayer<D>::~MaskedLayer() {}
...
template <int D>
inline typename Ntree<D, index>::masked_iterator MaskedLayer<D>::end() {
  return ntree_->masked_end();
}

template <int D> inline Layer<D>::Layer() {
...
inline Layer<D>::Layer(const Layer &other_layer)
    : AbstractLayer(other_layer), lower_left_(other_layer.lower_left_),
      extent_(other_layer.extent_), periodic_(other_layer.periodic_) {}
...
template <int D>
inline Position<D> Layer<D>::compute_displacement(const Position<D> &from_pos,
                                                  const index to) const {
...
template <int D>
inline double Layer<D>::compute_distance(const Position<D> &from_pos,
                                         const index to) const {
  return compute_displacement(from_pos, to).length();
}

template <int D>
inline double Layer<D>::compute_distance(const std::vector<double> &from_pos,
                                         const index to) const {
...
template <int D> inline void Layer<D>::clear_ntree_cache_() const {
  cached_ntree_ = lockPTR<Ntree<D, index>>();
  cached_ntree_layer_ = -1;
}

template <int D> inline void Layer<D>::clear_vector_cache_() const {
  if (cached_vector_ != 0) {
