 ...
 */
template <int D> class Mask : public AbstractMask {
public:
  ~Mask() {}
 ...
 */
template <int D> class AllMask : public Mask<D> {
public:
  ~AllMask() {}
   ...
   */
  bool inside(const Position<D> &) const { return true; }

  /**
   * @returns true always for this mask
   */
  bool inside(const Box<D> &) const { return true; }

  /**
   * @returns false always for this mask
   */
  bool outside(const Box<D> &) const { return false; }
  ...
  Mask<D> *clone() const { return new AllMask(); }
};

/**
 * Mask defining a box region.
 */
template <int D> class BoxMask : public Mask<D> {
public:
  /**
  ...
  BoxMask(const Position<D> &lower_left, const Position<D> &upper_right,
          const double azimuth_angle = 0.0, const double polar_angle = 0.0);
 ...
 */
template <int D> class BallMask : public Mask<D> {
public:
  /**
 ...
 */
template <int D> class EllipseMask : public Mask<D> {
public:
  /**
   ...
   *        ellipsoid
   */
  EllipseMask(Position<D> center, double major_axis, double minor_axis,
              double polar_axis, double azimuth_angle, double polar_angle)
      : center_(center), major_axis_(major_axis), minor_axis_(minor_axis),
        polar_axis_(polar_axis), azimuth_angle_(azimuth_angle),
        polar_angle_(polar_angle), x_scale_(4.0 / (major_axis_ * major_axis_)),
 ...
 */
template <int D> class IntersectionMask : public Mask<D> {
public:
  /**
 ...
 */
template <int D> class UnionMask : public Mask<D> {
public:
  /**
 ...
 */
template <int D> class DifferenceMask : public Mask<D> {
public:
  /**
 ...
 */
template <int D> class ConverseMask : public Mask<D> {
public:
  /**
 ...
 */
template <int D> class AnchoredMask : public Mask<D> {
public:
  /**
   * Construct the converse of the two given mask. A copy is made of the
   * supplied Mask object.
  ...
  AnchoredMask(const Mask<D> &m, Position<D> anchor)
      : m_(m.clone()), anchor_(anchor) {}

  /**
   * Copy constructor
   */
  AnchoredMask(const AnchoredMask &m)
      : Mask<D>(m), m_(m.m_->clone()), anchor_(m.anchor_) {}

  ~AnchoredMask() { delete m_; }
...
template <> inline Name BoxMask<2>::get_name() { return names::rectangular; }

template <> inline Name BoxMask<3>::get_name() { return names::box; }

template <int D> BoxMask<D>::BoxMask(const DictionaryDatum &d) {
...
inline BoxMask<D>::BoxMask(const Position<D> &lower_left,
                           ...
                           const double azimuth_angle, const double polar_angle)
    : lower_left_(lower_left), upper_right_(upper_right),
      azimuth_angle_(azimuth_angle), polar_angle_(polar_angle),
      azimuth_cos_(std::cos(azimuth_angle_ * numerics::pi / 180.)),
      azimuth_sin_(std::sin(azimuth_angle_ * numerics::pi / 180.)),
      polar_cos_(std::cos(polar_angle_ * numerics::pi / 180.)),
      polar_sin_(std::sin(polar_angle_ * numerics::pi / 180.)),
...
template <> inline Name BallMask<2>::get_name() { return names::circular; }

template <> inline Name BallMask<3>::get_name() { return names::spherical; }

template <int D> BallMask<D>::BallMask(const DictionaryDatum &d) {
...
template <> inline Name EllipseMask<2>::get_name() { return names::elliptical; }

template <> inline Name EllipseMask<3>::get_name() {
  return names::ellipsoidal;
}

template <int D> EllipseMask<D>::EllipseMask(const DictionaryDatum &d) {
