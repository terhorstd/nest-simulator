...
template <int D, class T> class Position;

template <int D, class T>
std::ostream &operator<<(std::ostream &os, const Position<D, T> &pos);

template <int D, class T = double> class Position {
public:
  template <int OD, class OT> friend class Position;
  ...
  template <class U> Position(const Position<D, U> &other);
   ...
   */
  template <class OT> Position operator+(const Position<D, OT> &other) const;
   ...
   * elementwise.
   */
  template <class OT> Position &operator+=(const Position<D, OT> &);
 ...
 */
template <int D> class MultiIndex : public Position<D, int> {
public:
  MultiIndex() : Position<D, int>(), lower_left_(), upper_right_() {}
  ...
  MultiIndex(const Position<D, int> &lower_left,
      ...
      : Position<D, int>(lower_left), lower_left_(lower_left),
  ...
  Position<D, int> get_lower_left() const { return lower_left_; }
  Position<D, int> get_upper_right() const { return upper_right_; }
...
template <int D, class T> inline Position<D, T>::Position() {
  for (int i = 0; i < D; ++i) {
    x_[i] = 0;
  }
}
...
template <int D, class T> inline Position<D, T>::Position(const T *const y) {
  for (int i = 0; i < D; ++i) {
    x_[i] = y[i];
  }
}
...
template <int D, class T> Position<D, T>::operator std::vector<T>() const {
...
template <int D, class T> inline T &Position<D, T>::operator[](int i) {
  return x_[i];
}
...
template <int D, class T> Token Position<D, T>::getToken() const {
...
template <int D, class T>
template <class OT>
inline Position<D, T> Position<D, T>::
operator+(const Position<D, OT> &other) const {
  Position p = *this;
  p += other;
  return p;
}
...
template <class OT>
inline Position<D, T> &Position<D, T>::
operator+=(const Position<D, OT> &other) {
  for (int i = 0; i < D; ++i) {
    x_[i] += other.x_[i];
  }
  return *this;
...
template <int D, class T> T Position<D, T>::length() const {
  T lensq = 0;
  for (int i = 0; i < D; ++i) {
    lensq += x_[i] * x_[i];
  }
...
template <int D, class T> Position<D, T>::operator std::string() const {
  std::stringstream ss;
  ss << *this;
  return ss.str();
}
