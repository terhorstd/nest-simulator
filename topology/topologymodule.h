...
public:
   ...
   */
  template <class T> static bool register_mask();
   ...
   *          if a mask type with the same name already exists.
   */
  template <class T> static bool register_mask(const Name &name);
   ...
   */
  static lockPTRDatum<AbstractMask, &TopologyModule::MaskType> /*MaskDatum*/
   ...
   */
  template <class T> static bool register_parameter(const Name &name);
  ...
  static lockPTRDatum<TopologyParameter,
                      &TopologyModule::ParameterType> /*ParameterDatum*/
  create_parameter(const Token &);
...
public:
  LayerExpected() : KernelException("LayerExpected") {}
  ~LayerExpected() throw() {}

  std::string message() const;
};

template <class T> inline bool TopologyModule::register_mask() {
  return mask_factory_().register_subtype<T>(T::get_name());
}

template <class T> inline bool TopologyModule::register_mask(const Name &name) {
  return mask_factory_().register_subtype<T>(name);
}

inline bool TopologyModule::register_mask(const Name &name,
                                          MaskCreatorFunction creator) {
  return mask_factory_().register_subtype(name, creator);
}

inline AbstractMask *TopologyModule::create_mask(const Name &name,
                                                 const DictionaryDatum &d) {
  return mask_factory_().create(name, d);
}

template <class T>
inline bool TopologyModule::register_parameter(const Name &name) {
