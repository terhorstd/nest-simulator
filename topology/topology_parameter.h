...
public:
   ...
   * Default constructor
   */
  TopologyParameter() : cutoff_(-std::numeric_limits<double>::infinity()) {}

  /**
   * Constructor
   * @param cutoff Values less than the cutoff are set to zero.
   */
  TopologyParameter(double cutoff) : cutoff_(cutoff) {}
   ...
   */
  double value(const Position<2> &p, librandom::RngPtr &rng) const {
   ...
   * @returns the value of the parameter at the given point.
   */
  double value(const Position<3> &p, librandom::RngPtr &rng) const {
    double val = raw_value(p, rng);
    if (val < cutoff_) {
      return 0.0;
    } else {
   ...
   * @returns the value of the parameter at the given point.
   */
  virtual double raw_value(const Position<2> &, librandom::RngPtr &) const {
   ...
   * @returns the value of the parameter at the given point.
   */
  virtual double raw_value(const Position<3> &, librandom::RngPtr &) const {
   ...
   */
  virtual TopologyParameter *
  multiply_parameter(const TopologyParameter &other) const;
  /**
   * Create the quotient of this parameter with another.
   * @returns a new dynamically allocated parameter.
   */
  virtual TopologyParameter *
  divide_parameter(const TopologyParameter &other) const;
  /**
   * Create the sum of this parameter with another.
   * @returns a new dynamically allocated parameter.
   */
  virtual TopologyParameter *
  add_parameter(const TopologyParameter &other) const;
  /**
   * Create the difference of this parameter with another.
   * @returns a new dynamically allocated parameter.
   */
  virtual TopologyParameter *
  subtract_parameter(const TopologyParameter &other) const;
...
public:
  ConstantParameter(double value) : TopologyParameter(), value_(value) {}

  /**
   * Parameters:
   * value - constant value of this parameter
   */
  ConstantParameter(const DictionaryDatum &d) : TopologyParameter(d) {
   ...
   */
  double raw_value(const Position<2> &, librandom::RngPtr &) const {
    return value_;
  }
  double raw_value(const Position<3> &, librandom::RngPtr &) const {
    return value_;
  }

  TopologyParameter *clone() const { return new ConstantParameter(value_); }
...
public:
  RadialParameter() : TopologyParameter() {}

  RadialParameter(double cutoff) : TopologyParameter(cutoff) {}

  RadialParameter(const DictionaryDatum &d) : TopologyParameter(d) {}

  virtual double raw_value(double) const = 0;

  double raw_value(const Position<2> &p, librandom::RngPtr &) const {
    return raw_value(p.length());
  }
  double raw_value(const Position<3> &p, librandom::RngPtr &) const {
    return raw_value(p.length());
  }
};
...
public:
  ...
  LinearParameter(const DictionaryDatum &d)
      : RadialParameter(d), a_(1.0), c_(0.0) {
    updateValue<double>(d, names::a, a_);
    updateValue<double>(d, names::c, c_);
  }

  double raw_value(double x) const { return a_ * x + c_; }

  TopologyParameter *clone() const { return new LinearParameter(*this); }
...
public:
  ...
  ExponentialParameter(const DictionaryDatum &d)
      : RadialParameter(d), a_(1.0), c_(0.0), tau_(1.0) {
    updateValue<double>(d, names::a, a_);
    updateValue<double>(d, names::c, c_);
  ...
  double raw_value(double x) const { return c_ + a_ * std::exp(-x / tau_); }

  TopologyParameter *clone() const { return new ExponentialParameter(*this); }
...
public:
  ...
  GaussianParameter(const DictionaryDatum &d)
      : RadialParameter(d), c_(0.0), p_center_(1.0), mean_(0.0), sigma_(1.0) {
    updateValue<double>(d, names::c, c_);
  ...
  double raw_value(double x) const {
    return c_ + p_center_ * std::exp(-std::pow(x - mean_, 2) /
                                     (2 * std::pow(sigma_, 2)));
  }

  TopologyParameter *clone() const { return new GaussianParameter(*this); }
...
public:
  ...
  double raw_value(const Position<2> &pos, librandom::RngPtr &) const {
    return c_ + p_center_ *
  ...
  double raw_value(const Position<3> &pos, librandom::RngPtr &rng) const {
    return raw_value(Position<2>(pos[0], pos[1]), rng);
  }

  TopologyParameter *clone() const { return new Gaussian2DParameter(*this); }
...
public:
  ...
  GammaParameter(const DictionaryDatum &d)
      : RadialParameter(d), kappa_(1.0), theta_(1.0), inv_theta_(1.0 / theta_),
        delta_(1.0) // consistent, cannot be computed explicitly here
  {
    ...
    if (kappa_ <= 0) {
      throw BadProperty("topology::GammaParameter: "
                        "kappa > 0 required.");
    }
    if (theta_ <= 0) {
      throw BadProperty("topology::GammaParameter: "
  ...
  TopologyParameter *clone() const { return new GammaParameter(*this); }
...
public:
  ...
  UniformParameter(const DictionaryDatum &d)
      : TopologyParameter(d), lower_(0.0), range_(1.0) {
  ...
  double raw_value(const Position<2> &, librandom::RngPtr &rng) const {
    return lower_ + rng->drand() * range_;
  }

  double raw_value(const Position<3> &, librandom::RngPtr &rng) const {
    return lower_ + rng->drand() * range_;
  }

  TopologyParameter *clone() const { return new UniformParameter(*this); }
...
public:
  ...
  NormalParameter(const DictionaryDatum &d)
      : TopologyParameter(d), mean_(0.0), sigma_(1.0),
        min_(-std::numeric_limits<double>::infinity()),
        max_(std::numeric_limits<double>::infinity()), rdev() {
    updateValue<double>(d, names::mean, mean_);
    updateValue<double>(d, names::sigma, sigma_);
  ...
  double raw_value(librandom::RngPtr &rng) const {
    double val;
    do {
  ...
  TopologyParameter *clone() const { return new NormalParameter(*this); }
...
public:
  ...
  LognormalParameter(const DictionaryDatum &d)
      : TopologyParameter(d), mu_(0.0), sigma_(1.0),
        min_(-std::numeric_limits<double>::infinity()),
        max_(std::numeric_limits<double>::infinity()), rdev() {
  ...
  TopologyParameter *clone() const { return new LognormalParameter(*this); }
 ...
 */
template <int D> class AnchoredParameter : public TopologyParameter {
public:
  AnchoredParameter(const TopologyParameter &p, const Position<D> &anchor)
      : TopologyParameter(p), p_(p.clone()), anchor_(anchor) {}

  AnchoredParameter(const AnchoredParameter &p)
      : TopologyParameter(p), p_(p.p_->clone()), anchor_(p.anchor_) {}

  ~AnchoredParameter() { delete p_; }

  double raw_value(const Position<D xor 1> &, librandom::RngPtr &) const {
    throw BadProperty("Incorrect dimension.");
  }

  double raw_value(const Position<D> &p, librandom::RngPtr &rng) const {
    return p_->raw_value(p - anchor_, rng);
  }

  TopologyParameter *clone() const { return new AnchoredParameter(*this); }
...
public:
  ...
  ProductParameter(const TopologyParameter &m1, const TopologyParameter &m2)
      : TopologyParameter(), parameter1_(m1.clone()), parameter2_(m2.clone()) {}

  /**
   * Copy constructor.
   */
  ProductParameter(const ProductParameter &p)
      : TopologyParameter(p), parameter1_(p.parameter1_->clone()),
   ...
   */
  double raw_value(const Position<2> &p, librandom::RngPtr &rng) const {
    return parameter1_->value(p, rng) * parameter2_->value(p, rng);
  }
  double raw_value(const Position<3> &p, librandom::RngPtr &rng) const {
    return parameter1_->value(p, rng) * parameter2_->value(p, rng);
  }

  TopologyParameter *clone() const { return new ProductParameter(*this); }
...
public:
  ...
  TopologyParameter *clone() const { return new QuotientParameter(*this); }
...
public:
  ...
  TopologyParameter *clone() const { return new SumParameter(*this); }
...
public:
  ...
  TopologyParameter *clone() const { return new DifferenceParameter(*this); }
...
public:
  ...
  ConverseParameter(const TopologyParameter &p)
      : TopologyParameter(p), p_(p.clone()) {}

  /**
   * Copy constructor.
   */
  ConverseParameter(const ConverseParameter &p)
      : TopologyParameter(p), p_(p.p_->clone()) {}

  ~ConverseParameter() { delete p_; }
  ...
  TopologyParameter *clone() const { return new ConverseParameter(*this); }
