...
index AbstractLayer::create_layer(const DictionaryDatum &layer_dict) {
  ...
  if (layer_dict->known(names::positions)) {
    if (layer_dict->known(names::rows) or layer_dict->known(names::columns) or
        layer_dict->known(names::layers)) {
...
std::vector<Node *>::iterator AbstractLayer::local_begin(int depth) {
  if (depth >= depth_) {
...
std::vector<Node *>::iterator AbstractLayer::local_end(int depth) {
  if (depth >= depth_) {
    throw BadProperty("Selected depth out of range");
  }
  index min_nodes_per_layer = local_size() / depth_;
...
std::vector<Node *>::const_iterator AbstractLayer::local_end(int depth) const {
  if (depth >= depth_) {
    throw BadProperty("Selected depth out of range");
  }
  index min_nodes_per_layer = local_size() / depth_;
