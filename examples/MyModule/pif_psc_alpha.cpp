...
template <> void RecordablesMap<mynest::pif_psc_alpha>::create() {
...
mynest::pif_psc_alpha::State_::State_(const Parameters_ &p)
    : V_m(p.V_reset), dI_syn(0.0), I_syn(0.0), I_ext(0.0), refr_count(0) {}
...
void mynest::pif_psc_alpha::State_::set(const DictionaryDatum &d,
                                        const Parameters_ &p) {
...
mynest::pif_psc_alpha::Buffers_::Buffers_(pif_psc_alpha &n) : logger_(n) {}
...
void mynest::pif_psc_alpha::update(Time const &slice_origin,
                                   const long from_step, const long to_step) {
