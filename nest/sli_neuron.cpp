...
template <> void RecordablesMap<sli_neuron>::create() {
  // use standard names whereever you can for consistency!
  insert_(names::V_m, &sli_neuron::get_V_m_);
}
} // namespace nest

nest::sli_neuron::Buffers_::Buffers_(sli_neuron &n) : logger_(n) {}
...
nest::sli_neuron::sli_neuron()
    : Archiving_Node(), state_(new Dictionary()), B_(*this) {
...
nest::sli_neuron::sli_neuron(const sli_neuron &n)
    : Archiving_Node(n), state_(new Dictionary(*n.state_)), B_(n.B_, *this) {
  init_state_(n);
}
...
void nest::sli_neuron::update(Time const &origin, const long from,
                              const long to) {
 ...
 * region.
 */
int nest::sli_neuron::execute_sli_protected(DictionaryDatum state, Name cmd) {
