...
public:
  TokenArrayObj(void)
      : p(NULL), begin_of_free_storage(NULL), end_of_free_storage(NULL),
        alloc_block_size(ARRAY_ALLOC_SIZE), refs_(1){};
  ...
  Token *begin() const { return p; }

  Token *end() const { return begin_of_free_storage; }

  size_t size(void) const { return (size_t)(begin_of_free_storage - p); }

  size_t capacity(void) const { return (size_t)(end_of_free_storage - p); }

  Token &operator[](size_t i) { return p[i]; }

  const Token &operator[](size_t i) const { return p[i]; }
  ...
  bool index_is_valid(long i) const { return (p + i) < begin_of_free_storage; }
  ...
  unsigned int references(void) { return refs_; }
  ...
  unsigned int add_reference() { return ++refs_; }
  ...
  void assign_move(Token *tp, Token &t) { tp->move(t); }

  void pop_back(void) { (--begin_of_free_storage)->clear(); }
  ...
  void insert(size_t i, const Token &t) { insert(i, 1, t); }
  ...
  static size_t getallocations(void) { return allocations; }
