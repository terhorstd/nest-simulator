...
bool SLIInterpreter::known(const Name &n) const { return DStack->known(n); }

bool SLIInterpreter::baseknown(const Name &n) const {
  return DStack->baseknown(n);
}

void SLIInterpreter::def(Name const &n, Token const &t) { DStack->def(n, t); }

void SLIInterpreter::undef(Name const &n) { DStack->undef(n); }
...
SLIInterpreter::SLIInterpreter(void)
    : debug_mode_(false), show_stack_(false), show_backtrace_(false),
      catch_errors_(false), opt_tailrecursion_(true), call_depth_(0),
      max_call_depth_(10), cycle_count(0), cycle_guard(false),
      cycle_restriction(0), verbositylevel(M_INFO), statusdict(0), errordict(0),
      DStack(0), parse(0), ilookup_name("::lookup"), ipop_name("::pop"),
      isetcallback_name("::setcallback"), iiterate_name("::executeprocedure"),
      iloop_name("::loop"), irepeat_name("::repeat"), ifor_name("::for"),
      iforallarray_name("::forall_a"), iforalliter_name("::forall_iter"),
      ...
      */
      ...
      ,
      e_name("E"), iparse_name("::parse"), stop_name("stop"), end_name("end"),
      null_name("null"), true_name("true"), false_name("false"),
      mark_name("mark"), istopped_name("::stopped"),
      systemdict_name("systemdict"), userdict_name("userdict")
      ...
      */
      ,
      errordict_name("errordict"), quitbyerror_name("quitbyerror"),
      newerror_name("newerror"), errorname_name("errorname"),
      commandname_name("commandname"), signo_name("sys_signo"),
      recordstacks_name("recordstacks"), estack_name("estack"),
      ostack_name("ostack"), dstack_name("dstack"),
      ...
      InternalKernelError("InternalKernelError"), OStack(100), EStack(100) {
  inittypes();
...
void SLIInterpreter::addlinkedusermodule(SLIModule *m) {
  m->install(std::cerr, this);
...
void SLIInterpreter::raiseerror(std::exception &err) {
  ...
  errordict->insert("command",
                    EStack.top()); // store the func/trie that caused the error.
...
void SLIInterpreter::verbosity(int level) { verbositylevel = level; }

int SLIInterpreter::verbosity(void) const { return verbositylevel; }
...
void SLIInterpreter::removecycleguard(void) { cycle_guard = false; }
...
void SLIInterpreter::backtrace_on() {
  ...
  std::string msg = "Showing stack backtrace on error.  Disabling tail "
  ...
  do {
    ...
    if (command == "show") {
      ...
      if (arg == "backtrace") {
        ...
        show_stack_ = not show_stack_;
        std::cerr << "Stack display is now " << (show_stack_ ? " On." : "Off.")
                  << std::endl;
      } else if (arg == "catch") {
      ...
      std::cerr << "Unknown command. Type 'help' for help, or 'quit' to leave "
                   "debugger."
                << std::endl;
    }
  } while (true);
