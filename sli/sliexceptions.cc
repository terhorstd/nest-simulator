...
std::string DivisionByZero::message() const {
  return "You cannot divide by zero.";
}

std::string TypeMismatch::message() const {
...
std::string RangeCheck::message() const {
  if (size_ > 0) {
...
std::string ArgumentType::message() const {
  std::ostringstream out;
...
std::string BadParameterValue::message() const { return msg_; }

std::string UndefinedName::message() const {
  return "Key '/" + name_ + "' does not exist in dictionary.";
}

std::string EntryTypeMismatch::message() const {
  return "Expected datatype: " + expected_ +
         "\nProvided datatype: " + provided_;
}

std::string StackUnderflow::message() const {
  std::ostringstream out;
  if (needed) {
...
std::string IOError::message() const { return std::string(); }

std::string SystemSignal::message() const {
  std::ostringstream out;
...
std::string UnaccessedDictionaryEntry::message() const {
  return "Unused dictionary items: " + msg_;
}

std::string DynamicModuleManagementError::message() const {
  if (msg_.empty()) {
...
std::string NamingConflict::message() const { return msg_; }

std::string NotImplemented::message() const { return msg_; }
