...
std::string SLIStartup::checkenvpath(std::string const &envvar,
  ...
  if (envpath != "") {
    ...
    if (dirptr != NULL) {
      ...
      if (defaultval != "") {
        i->message(
            SLIInterpreter::M_ERROR, "SLIStartup",
...
SLIStartup::SLIStartup(int argc, char **argv)
    : startupfilename("sli-init.sli"), slilibpath("/sli"),
      ...
      ,
      debug_(false), argv_name("argv"), prgname_name("prgname"),
      exitcode_name("exitcode"), prgmajor_name("prgmajor"),
      prgminor_name("prgminor"), prgpatch_name("prgpatch"),
      prgbuilt_name("built"), prefix_name("prefix"),
      prgdatadir_name("prgdatadir"), prgdocdir_name("prgdocdir"),
      host_name("host"), hostos_name("hostos"), hostvendor_name("hostvendor"),
      hostcpu_name("hostcpu"), getenv_name("getenv"),
      statusdict_name("statusdict"), start_name("start"), intsize_name("int"),
      longsize_name("long"), havelonglong_name("have_long_long"),
      longlongsize_name("long_long"), doublesize_name("double"),
      pointersize_name("void_ptr"), architecturedict_name("architecture"),
      platform_name("platform"), threading_name("threading"),
      have_mpi_name("have_mpi"), ismpi_name("is_mpi"),
      have_gsl_name("have_gsl"), have_music_name("have_music"),
      have_libneurosim_name("have_libneurosim"), ndebug_name("ndebug"),
      exitcodes_name("exitcodes"), exitcode_success_name("success"),
      ...
      exitcode_scripterror_name("scripterror"), exitcode_abort_name("abort"),
      exitcode_userabort_name("userabort"), exitcode_segfault_name("segfault"),
      exitcode_exception_name("exception"), exitcode_fatal_name("fatal"),
...
void SLIStartup::init(SLIInterpreter *i) {
  ...
  if (slihomepath_env != "") {
    ...
    i->message(SLIInterpreter::M_DEBUG, "SLIStartup",
  ...
  if (not checkpath(slihomepath, fname)) {
    i->message(SLIInterpreter::M_FATAL, "SLIStartup",
               "Your NEST installation seems broken. \n");
    i->message(SLIInterpreter::M_FATAL, "SLIStartup",
               "I could not find the startup file that");
    i->message(
        SLIInterpreter::M_FATAL, "SLIStartup",
  ...
  statusdict->insert(prgname_name,
                     Token(new StringDatum(NEST_VERSION_PRGNAME)));
  statusdict->insert(exitcode_name, Token(new IntegerDatum(EXIT_SUCCESS)));
  statusdict->insert(prgmajor_name,
                     Token(new IntegerDatum(NEST_VERSION_MAJOR_REVISION)));
  statusdict->insert(prgminor_name,
                     Token(new IntegerDatum(NEST_VERSION_MINOR_REVISION)));
  statusdict->insert(prgpatch_name,
                     Token(new StringDatum(NEST_VERSION_PATCHLEVEL)));
  statusdict->insert(
      prgbuilt_name,
      Token(new StringDatum(String::compose("%1 %2", __DATE__, __TIME__))));
  ...
  architecturedict->insert(doublesize_name,
                           Token(new IntegerDatum(sizeof(double))));
  architecturedict->insert(pointersize_name,
                           Token(new IntegerDatum(sizeof(void *))));
  architecturedict->insert(intsize_name, Token(new IntegerDatum(sizeof(int))));
  architecturedict->insert(longsize_name,
                           Token(new IntegerDatum(sizeof(long))));
  architecturedict->insert("Token", Token(new IntegerDatum(sizeof(Token))));
  architecturedict->insert("TokenMap",
                           Token(new IntegerDatum(sizeof(TokenMap))));
  architecturedict->insert("Dictionary",
                           Token(new IntegerDatum(sizeof(Dictionary))));
  architecturedict->insert("DictionaryDatum",
                           Token(new IntegerDatum(sizeof(DictionaryDatum))));
  architecturedict->insert("IntegerDatum",
                           Token(new IntegerDatum(sizeof(IntegerDatum))));
  architecturedict->insert("ArrayDatum",
                           Token(new IntegerDatum(sizeof(ArrayDatum))));
  architecturedict->insert("TokenArray",
                           Token(new IntegerDatum(sizeof(TokenArray))));
  architecturedict->insert("TokenArrayObj",
                           Token(new IntegerDatum(sizeof(TokenArrayObj))));
  ...
  exitcodes->insert(exitcode_success_name,
                    Token(new IntegerDatum(EXIT_SUCCESS)));
  exitcodes->insert(exitcode_skipped_name,
                    Token(new IntegerDatum(EXITCODE_SKIPPED)));
  ...
  exitcodes->insert(exitcode_abort_name,
                    Token(new IntegerDatum(NEST_EXITCODE_ABORT)));
  exitcodes->insert(exitcode_userabort_name,
                    Token(new IntegerDatum(EXITCODE_USERABORT)));
  exitcodes->insert(exitcode_segfault_name,
                    Token(new IntegerDatum(NEST_EXITCODE_SEGFAULT)));
  exitcodes->insert(exitcode_exception_name,
                    Token(new IntegerDatum(EXITCODE_EXCEPTION)));
  exitcodes->insert(exitcode_fatal_name,
                    Token(new IntegerDatum(EXITCODE_FATAL)));
  ...
  architecturedict->insert(longlongsize_name,
                           Token(new IntegerDatum(sizeof(long long))));
#else
