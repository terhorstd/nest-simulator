...
public:
  TokenArray(void) : data(new TokenArrayObj()){};

  explicit TokenArray(size_t n, const Token &t = Token(), size_t alloc = 128)
      : data(new TokenArrayObj(n, t, alloc)) {}

  TokenArray(const TokenArray &a) : data(a.data) { data->add_reference(); }

  TokenArray(const TokenArrayObj &a) : data(new TokenArrayObj(a)) {}
   ...
   */
  Token *begin() const { return data->begin(); }

  /**
   * Return pointer to next to last element.
   */
  Token *end() const { return data->end(); }
   ...
   */
  size_t capacity(void) const { return data->capacity(); }
  ...
  const Token &operator[](size_t i) const { return (*data)[i]; }

  const Token &get(long i) const { return data->get(i); }

  bool index_is_valid(long i) const { return data->index_is_valid(i); }
   ...
   * invalidated.
   */
  bool shrink(void) { return data->shrink(); }
   ...
   */
  bool reserve(size_t n) { return data->reserve(n); }

  unsigned int references(void) { return data->references(); }
  ...
  void push_back_dont_clone(Token &t) { data->push_back_move(t); }
  ...
  void swap(TokenArray &a) { std::swap(data, a.data); }
  ...
  bool operator==(const TokenArray &a) const { return *data == *a.data; }
...
inline void TokenArray::reverse() {
  if (size() == 0) {
    return;
  }
  clone();
...
inline void TokenArray::rotate(long n = 1) {
  if (size() == 0 || n == 0) {
    return;
  }
