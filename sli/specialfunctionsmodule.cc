...
void SpecialFunctionsModule::GammaIncFunction::execute(
...
void SpecialFunctionsModule::LambertW0Function::execute(
    SLIInterpreter *i) const {
  i->EStack.pop(); // pop yourself
...
// see mathematica.sli for documentation
void SpecialFunctionsModule::LambertWm1Function::execute(
    SLIInterpreter *i) const {
  i->EStack.pop(); // pop yourself
...
void SpecialFunctionsModule::GaussDiskConvFunction::execute(
  ...
  if (y < 2 * GSL_DBL_EPSILON) { /* disk has zero diameter */
  ...
  } else if (std::fabs(R - r0) <
             2 * GSL_DBL_EPSILON) { /* Gaussian on perimeter */
    ...
    double Cerr = 0.0;
    int status =
        gsl_integration_qag(&F_, 0.0, R, 0.0, QUAD_ERR_LIM, MAX_QUAD_SIZE,
                            GSL_INTEG_GAUSS61, w_, &C, &Cerr);
