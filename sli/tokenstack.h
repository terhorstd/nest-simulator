...
public:
  TokenStack(Index n) : TokenArrayObj(0, Token(), n) {}
  ...
  void push(const Token &e) { push_back(e); }

  void push_move(Token &e) { push_back_move(e); }
   ...
   */
  void push_by_ref(const Token &e) { push_back_by_ref(e); }
   ...
   * This function increases the reference count of the datum.
   */
  void push_by_pointer(Datum *rhs) { push_back_by_pointer(rhs); }
  ...
  void pop(size_t n) { erase(end() - n, end()); }

  Token &top(void) { return *(end() - 1); }
  const Token &top(void) const { return *(end() - 1); }

  const Token &pick(size_t i) const { return *(end() - i - 1); }

  Token &pick(size_t i) { return *(end() - i - 1); }

  using TokenArrayObj::empty;
  //  using TokenArray::operator=;

  void swap(void) { (end() - 1)->swap(*(end() - 2)); }

  void swap(Token &e) { (end() - 1)->swap(e); }

  void index(Index i) { push(pick(i)); }
  ...
  TokenArray toArray(void) const { return TokenArray(*this); }
};
