...
void Get_a_aFunction::execute(SLIInterpreter *i) const {
  ...
  if (idx == NULL) {
    i->message(SLIInterpreter::M_ERROR, "get_a_a",
  ...
  for (Token *t = idx->begin(); t != idx->end(); ++t) {
    ...
    if (id == NULL) {
      ...
      i->message(SLIInterpreter::M_INFO, "get_a_a",
...
void Join_sFunction::execute(SLIInterpreter *i) const {
  ...
  if (s1 == NULL || s2 == NULL) {
    i->message(SLIInterpreter::M_ERROR, "join_s",
               "Usage: (string1) (string2) join_s");
    i->raiseerror(i->ArgumentTypeError);
    return;
  }
...
void Put_aFunction::execute(SLIInterpreter *i) const {
  ...
  if ((id->get() >= 0) && ((size_t)id->get() < ad->size())) {
    i->EStack.pop();
    ad->assign_move(id->get(),
                    i->OStack.top()); // its safe to empty top() because
