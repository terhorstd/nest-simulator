...
template <> float getValue<float>(const Token &t) {
  DoubleDatum *id = dynamic_cast<DoubleDatum *>(t.datum());
  if (id == NULL) { // we have to create a Datum object to get the name...
    DoubleDatum const d;
    throw TypeMismatch(d.gettypename().toString(),
...
template <> void setValue<float>(const Token &t, float const &value) {
  DoubleDatum *id = dynamic_cast<DoubleDatum *>(t.datum());
  if (id == NULL) { // we have to create a Datum object to get the name...
    DoubleDatum const d;
    throw TypeMismatch(d.gettypename().toString(),
...
template <> bool getValue<bool>(const Token &t) {
...
template <> void setValue<bool>(const Token &t, bool const &value) {
  BoolDatum *bd = dynamic_cast<BoolDatum *>(t.datum());
  if (bd == NULL) { // we have to create a Datum object to get the name...
    BoolDatum const d(false);
    throw TypeMismatch(d.gettypename().toString(),
...
template <> std::string getValue<std::string>(const Token &t) {
...
template <> std::vector<double> getValue<std::vector<double>>(const Token &t) {
...
template <>
void setValue<std::vector<double>>(const Token &t,
...
template <> std::vector<long> getValue<std::vector<long>>(const Token &t) {
...
template <>
void setValue<std::vector<long>>(const Token &t,
