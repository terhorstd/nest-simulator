...
void SLIgraphics::ReadPGMFunction::initRead(std::istream *in, int &width,
                                            int &height, int &maxval) const {
...
void SLIgraphics::ReadPGMFunction::readImage(std::istream *in, char magic[2],
                                             std::vector<long> &image,
...
const std::string SLIgraphics::name() const {
  return std::string("SLIgraphics");
}

const std::string SLIgraphics::commandstring() const {
