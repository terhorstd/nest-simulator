...
std::size_t Name::capacity() { return Name::handleTableInstance_().size(); }

std::size_t Name::num_handles() { return Name::handleTableInstance_().size(); }
...
void Name::list(std::ostream &out) {
  ...
  for (Name::HandleMap_::const_iterator where = map.begin(); where != map.end();
       ++where) {
...
void Name::info(std::ostream &out) { Name::list_handles(out); }
