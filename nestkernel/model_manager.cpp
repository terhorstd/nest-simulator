...
ModelManager::ModelManager()
    : pristine_models_(), models_(), pristine_prototypes_(), prototypes_(),
      modeldict_(new Dictionary), synapsedict_(new Dictionary),
      subnet_model_(0), siblingcontainer_model_(0), proxynode_model_(0),
      proxy_nodes_(), dummy_spike_sources_(), model_defaults_modified_(false) {}
...
void ModelManager::initialize() {
  if (subnet_model_ == 0 and siblingcontainer_model_ == 0 and
      proxynode_model_ == 0) {
  ...
  for (thread t = 0;
       t < static_cast<thread>(kernel().vp_manager.get_num_threads()); ++t) {
    for (index i = 0; i < pristine_models_.size(); ++i) {
      if (pristine_models_[i].first != 0) {
...
index ModelManager::copy_model(Name old_name, Name new_name,
...
index ModelManager::register_node_model_(Model *model, bool private_model) {
...
index ModelManager::copy_node_model_(index old_id, Name new_name) {
...
index ModelManager::copy_synapse_model_(index old_id, Name new_name) {
                                  ...
                                  // synapse_model = 62, see nest_types.h
  {
    LOG(M_ERROR, "ModelManager::copy_synapse_model_",
        "CopyModel cannot generate another synapse. Maximal synapse model "
        "count "
        "of 63 exceeded.");
...
void ModelManager::set_model_defaults(Name name, DictionaryDatum params) {
...
void ModelManager::set_node_defaults_(index model_id,
                                      const DictionaryDatum &params) {
  params->clear_access_flags();

  get_model(model_id)->set_status(params);

  ALL_ENTRIES_ACCESSED(*params, "ModelManager::set_node_defaults_",
                       "Unread dictionary entries: ");
}

void ModelManager::set_synapse_defaults_(index model_id,
                                         const DictionaryDatum &params) {
  params->clear_access_flags();
...
#pragma omp parallel
  ...
  ALL_ENTRIES_ACCESSED(*params, "ModelManager::set_synapse_defaults_",
                       "Unread dictionary entries: ");
}

// TODO: replace int with index and return value -1 with invalid_index, also
// change all pertaining code
int ModelManager::get_model_id(const Name name) const {
...
DictionaryDatum ModelManager::get_connector_defaults(synindex syn_id) const {
...
bool ModelManager::connector_requires_symmetric(const synindex syn_id) const {
  assert_valid_syn_id(syn_id);

  return prototypes_[0][syn_id]->requires_symmetric();
}

bool ModelManager::connector_requires_clopath_archiving(
...
void ModelManager::clear_models_(bool called_from_destructor) {
...
void ModelManager::clear_prototypes_() {
...
void ModelManager::calibrate(const TimeConverter &tc) {
  for (thread t = 0;
       t < static_cast<thread>(kernel().vp_manager.get_num_threads()); ++t) {
    for (std::vector<ConnectorModel *>::iterator pt = prototypes_[t].begin();
         pt != prototypes_[t].end(); ++pt) {
      if (*pt != 0) {
        (*pt)->calibrate(tc);
      }
    }
...
bool ModelManager::compare_model_by_id_(const int a, const int b) {
  return kernel().model_manager.get_model(a)->get_name() <
         kernel().model_manager.get_model(b)->get_name();
}

void ModelManager::memory_info() const {
...
void ModelManager::create_secondary_events_prototypes() {
  delete_secondary_events_prototypes();
  secondary_events_prototypes_.resize(kernel().vp_manager.get_num_threads());

  for (thread tid = 0;
       tid < static_cast<thread>(secondary_events_prototypes_.size()); ++tid) {
...
synindex ModelManager::register_connection_model_(ConnectorModel *cf) {
