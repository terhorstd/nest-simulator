...
public:
  ConnectorModel(const std::string, const bool is_primary, const bool has_delay,
   ...
   * valid.
   */
  virtual void add_connection(Node &src, Node &tgt,
                              std::vector<ConnectorBase *> &hetconn,
                              const synindex syn_id, const DictionaryDatum &d,
  ...
  bool has_delay() const { return has_delay_; }

  bool requires_symmetric() const { return requires_symmetric_; }

  bool requires_clopath_archiving() const {
    return requires_clopath_archiving_;
  }

  bool supports_wfr() const { return supports_wfr_; }
...
public:
  ...
  virtual typename ConnectionT::EventType *get_event() const {
    assert(false);
    return 0;
  }
...
private:
  ...
  void add_connection_(Node &src, Node &tgt,
                       std::vector<ConnectorBase *> &hetconn,
                       const synindex syn_id, ConnectionT &c,
...
public:
  GenericSecondaryConnectorModel(const std::string name, const bool has_delay,
                                 const bool requires_symmetric,
                                 const bool supports_wfr)
      : GenericConnectorModel<ConnectionT>(
            name,
            /*is _primary=*/false, has_delay, requires_symmetric, supports_wfr,
  ...
  ConnectorModel *clone(std::string name) const {
    return new GenericSecondaryConnectorModel(*this,
                                              name); // calls copy construtor
  }

  std::vector<SecondaryEvent *> create_event(size_t n) const {
  ...
  typename ConnectionT::EventType *get_event() const { return pev_; }
};
