 ...
 */
template <typename T> struct MPI_Type { static MPI_Datatype type; };

template <typename T>
void nest::MPIManager::communicate_Allgatherv(std::vector<T> &send_buffer,
...
void nest::MPIManager::communicate(const NodeListType &local_nodes,
  ...
  if (np > 1 and remote) {
    ...
    if (n_globals != 0) {
      ...
      communicate_Allgatherv<long>(localnodes, globalnodes, displacements,
                                   n_nodes);
  ...
  } else // on one proc or not including remote nodes
  {
    if (params->empty()) {
      ...
      for (typename NodeListType::iterator n = local_nodes.begin();
        ...
        if (match) {
          all_nodes.push_back(NodeAddressingData(
              (*n)->get_gid(), ((*n)->get_parent())->get_gid(),
              (*n)->get_vp()));
        }
      }
    }
