...
ConnectionID::ConnectionID(long source_gid, long target_gid, long target_thread,
    ...
    : source_gid_(source_gid), target_gid_(target_gid),
      target_thread_(target_thread), synapse_modelid_(synapse_modelid),
      port_(port) {}

ConnectionID::ConnectionID(long source_gid, long target_thread,
                           long synapse_modelid, long port)
    : source_gid_(source_gid), target_thread_(target_thread),
      synapse_modelid_(synapse_modelid), port_(port) {}
...
bool ConnectionID::operator==(const ConnectionID &c) const {
  return (source_gid_ == c.source_gid_) and (target_gid_ == c.target_gid_) and
         (target_thread_ == c.target_thread_) and (port_ == c.port_) and
