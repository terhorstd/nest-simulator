...
nest::ConnectionManager::ConnectionManager()
    : connruledict_(new Dictionary()), connbuilder_factories_(), min_delay_(1),
      max_delay_(1), keep_source_table_(true), have_connections_changed_(true),
      sort_connections_by_source_(true), has_primary_connections_(false),
      check_primary_connections_(), secondary_connections_exist_(false),
      check_secondary_connections_(), stdp_eps_(1.0e-6) {}
...
void nest::ConnectionManager::initialize() {
...
#pragma omp parallel
  ...
  std::vector<std::vector<size_t>> tmp2(kernel().vp_manager.get_num_threads(),
                                        std::vector<size_t>());
...
void nest::ConnectionManager::finalize() {
...
void nest::ConnectionManager::set_status(const DictionaryDatum &d) {
  ...
  updateValue<bool>(d, names::sort_connections_by_source,
                    sort_connections_by_source_);
  if (not sort_connections_by_source_ and
      kernel().sp_manager.is_structural_plasticity_enabled()) {
    throw KernelException("If structural plasticity is enabled, "
                          "sort_connections_by_source can not "
                          "be set to false.");
  }
...
nest::DelayChecker &nest::ConnectionManager::get_delay_checker() {
  return delay_checkers_[kernel().vp_manager.get_thread_id()];
}

void nest::ConnectionManager::get_status(DictionaryDatum &dict) {
  update_delay_extrema_();
  ...
  def<bool>(dict, names::sort_connections_by_source,
            sort_connections_by_source_);
}

DictionaryDatum nest::ConnectionManager::get_synapse_status(
    const index source_gid, const index target_gid, const thread tid,
    const synindex syn_id, const index lcid) const {
  ...
  // receiving devices
  if ((source->has_proxies() and target->has_proxies() and
       connections_[tid][syn_id] != NULL) or
      ((source->has_proxies() and not target->has_proxies() and
        not target->local_receiver() and connections_[tid][syn_id] != NULL))) {
    connections_[tid][syn_id]->get_synapse_status(tid, lcid, dict);
  } else if (source->has_proxies() and not target->has_proxies() and
             target->local_receiver()) {
    target_table_devices_.get_synapse_status_to_device(tid, source_gid, syn_id,
                                                       dict, lcid);
  } else if (not source->has_proxies()) {
    const index ldid = source->get_local_device_id();
    target_table_devices_.get_synapse_status_from_device(tid, ldid, syn_id,
                                                         dict, lcid);
  } else {
    assert(false);
  }
...
void nest::ConnectionManager::set_synapse_status(
    const index source_gid, const index target_gid, const thread tid,
    const synindex syn_id, const index lcid, const DictionaryDatum &dict) {
  kernel().model_manager.assert_valid_syn_id(syn_id);
  ...
  try {
    ...
    if ((source->has_proxies() and target->has_proxies() and
         connections_[tid][syn_id] != NULL) or
        ((source->has_proxies() and not target->has_proxies() and
          not target->local_receiver() and
          connections_[tid][syn_id] != NULL))) {
      ...
      const index ldid = source->get_local_device_id();
      target_table_devices_.set_synapse_status_from_device(tid, ldid, syn_id,
                                                           cm, dict, lcid);
    } else {
      assert(false);
    }
    ...
    throw BadProperty(String::compose(
        "Setting status of '%1' connecting from GID %2 to GID %3 via port %4: "
        "%5",
        kernel().model_manager.get_synapse_prototype(syn_id, tid).get_name(),
        source_gid, target_gid, lcid, e.message()));
  }
}
...
const nest::Time nest::ConnectionManager::get_min_delay_time_() const {
...
const nest::Time nest::ConnectionManager::get_max_delay_time_() const {
...
nest::ConnBuilder *nest::ConnectionManager::get_conn_builder(
    const std::string &name, const GIDCollection &sources,
    const GIDCollection &targets, const DictionaryDatum &conn_spec,
    const DictionaryDatum &syn_spec) {
  const size_t rule_id = connruledict_->lookup(name);
  return connbuilder_factories_.at(rule_id)->create(sources, targets, conn_spec,
                                                    syn_spec);
}
...
void nest::ConnectionManager::connect(const GIDCollection &sources,
  ...
  ALL_ENTRIES_ACCESSED(*conn_spec, "Connect",
                       "Unread dictionary entries in conn_spec: ");
  ALL_ENTRIES_ACCESSED(*syn_spec, "Connect",
                       "Unread dictionary entries in syn_spec: ");
...
void nest::ConnectionManager::connect(const index sgid, Node *target,
                                      thread target_thread,
                                      const synindex syn_id,
                                      const DictionaryDatum &params,
                                      const double delay, const double weight) {
  kernel().model_manager.assert_valid_syn_id(syn_id);
  ...
  if (source->has_proxies() and target->has_proxies()) {
    connect_(*source, *target, sgid, target_thread, syn_id, params, delay,
             weight);
  }
  ...
  // normal devices -> normal nodes and devices with proxies
    ...
    connect_from_device_(*source, *target, target_thread, syn_id, params, delay,
                         weight);
  }
...
// gid gid dict syn_id
bool nest::ConnectionManager::connect(const index sgid, const index tgid,
                                      const DictionaryDatum &params,
                                      const synindex syn_id) {
  kernel().model_manager.assert_valid_syn_id(syn_id);
...
void nest::ConnectionManager::connect_(Node &s, Node &r, const index s_gid,
                                       const thread tid, const synindex syn_id,
                                       const DictionaryDatum &params,
                                       const double delay,
                                       const double weight) {
  ...
  if (kernel().model_manager.connector_requires_clopath_archiving(syn_id) and
      not dynamic_cast<Clopath_Archiving_Node *>(&r)) {
    throw NotImplemented("This synapse model is not supported by the neuron "
                         "model of at least one "
                         "connection.");
  }
...
void nest::ConnectionManager::connect_to_device_(
    Node &s, Node &r, const index s_gid, const thread tid,
    const synindex syn_id, const DictionaryDatum &params, const double delay,
    const double weight) {
  // create entries in connection structure for connections to devices
  target_table_devices_.add_connection_to_device(s, r, s_gid, tid, syn_id,
                                                 params, delay, weight);

  increase_connection_count(tid, syn_id);
}

void nest::ConnectionManager::connect_from_device_(
    Node &s, Node &r, const thread tid, const synindex syn_id,
    const DictionaryDatum &params, const double delay, const double weight) {
  // create entries in connections vector of devices
  target_table_devices_.add_connection_from_device(s, r, tid, syn_id, params,
                                                   delay, weight);

  increase_connection_count(tid, syn_id);
}

void nest::ConnectionManager::increase_connection_count(const thread tid,
                                                        const synindex syn_id) {
...
nest::index nest::ConnectionManager::find_connection(const thread tid,
                                                     const synindex syn_id,
                                                     const index sgid,
                                                     const index tgid) {
...
void nest::ConnectionManager::disconnect(const thread tid,
                                         const synindex syn_id,
                                         const index sgid, const index tgid) {
  have_connections_changed_ = true;
...
void nest::ConnectionManager::data_connect_single(const index source_id,
...
bool nest::ConnectionManager::data_connect_connectome(
    const ArrayDatum &connectome) {
...
void nest::ConnectionManager::trigger_update_weight(
    const long vt_id, const std::vector<spikecounter> &dopa_spikes,
    const double t_trig) {
  const thread tid = kernel().vp_manager.get_thread_id();
  ...
  for (std::vector<ConnectorBase *>::iterator it = connections_[tid].begin();
    ...
    if (*it != NULL) {
      (*it)->trigger_update_weight(
          vt_id, tid, dopa_spikes, t_trig,
          kernel().model_manager.get_synapse_prototypes(tid));
    }
  }
}

size_t nest::ConnectionManager::get_num_target_data(const thread tid) const {
...
void nest::ConnectionManager::split_to_neuron_device_vectors_(
    const thread tid, TokenArray const *gid_token_array,
    std::vector<index> &neuron_gids, std::vector<index> &device_gids) const {
...
void nest::ConnectionManager::get_connections(
    std::deque<ConnectionID> &connectome, TokenArray const *source,
    TokenArray const *target, synindex syn_id, long synapse_label) const {
...
#pragma omp parallel
    {
      ...
      if (connections != NULL) {
        ...
        for (index lcid = 0; lcid < num_connections_in_thread; ++lcid) {
          ...
          connections->get_connection(source_gid, 0, tid, lcid, synapse_label,
                                      conns_in_thread);
        }
      }

      target_table_devices_.get_connections(0, 0, tid, syn_id, synapse_label,
                                            conns_in_thread);
...
#pragma omp parallel
    {
      ...
      split_to_neuron_device_vectors_(tid, target, target_neuron_gids,
                                      target_device_gids);
      ...
      if (connections != NULL) {
        ...
        for (index lcid = 0; lcid < num_connections_in_thread; ++lcid) {
          ...
          connections->get_connection_with_specified_targets(
              source_gid, target_neuron_gids, tid, lcid, synapse_label,
              conns_in_thread);
        }

        for (std::vector<index>::const_iterator t_gid =
                 target_neuron_gids.begin();
             t_gid != target_neuron_gids.end(); ++t_gid) {
      ...
      for (std::vector<index>::const_iterator t_gid =
           ...
           t_gid != target_device_gids.end(); ++t_gid) {
...
#pragma omp parallel
    {
        ...
        for (index lcid = 0; lcid < num_connections_in_thread; ++lcid) {
          ...
          if (std::binary_search(sources.begin(), sources.end(), source_gid)) {
            if (target == 0) {
              ...
              // connections.
              connections->get_connection(source_gid, 0, tid, lcid,
                                          synapse_label, conns_in_thread);
            } else {
              connections->get_connection_with_specified_targets(
                  source_gid, target_neuron_gids, tid, lcid, synapse_label,
                  conns_in_thread);
      ...
      for (size_t s_id = 0; s_id < source->size(); ++s_id) {
        ...
        if (target == 0) {
          target_table_devices_.get_connections(source_gid, 0, tid, syn_id,
                                                synapse_label, conns_in_thread);
        } else {
          for (std::vector<index>::const_iterator t_gid =
                   target_neuron_gids.begin();
            ...
            // devices. First we get connections from devices.
            target_table_devices_.get_connections_from_devices_(
                source_gid, *t_gid, tid, syn_id, synapse_label,
                conns_in_thread);
          }
          for (std::vector<index>::const_iterator t_gid =
                   target_device_gids.begin();
...
void nest::ConnectionManager::get_source_gids_(const thread tid,
                                               const synindex syn_id,
                                               const index tgid,
                                               std::vector<index> &sources) {
...
void nest::ConnectionManager::get_sources(
    const std::vector<index> &targets, const index syn_id,
...
void nest::ConnectionManager::get_targets(
    const std::vector<index> &sources, const index syn_id,
...
void nest::ConnectionManager::sort_connections(const thread tid) {
...
void nest::ConnectionManager::
    compute_compressed_secondary_recv_buffer_positions(const thread tid) {
#pragma omp single
...
bool nest::ConnectionManager::deliver_secondary_events(
    const thread tid, const bool called_from_wfr_update,
  ...
  for (synindex syn_id = 0; syn_id < syn_id_end; ++syn_id) {
    if (not called_from_wfr_update or
        kernel()
...
void nest::ConnectionManager::compress_secondary_send_buffer_pos(
    const thread tid) {
  target_table_.compress_secondary_send_buffer_pos(tid);
}

void nest::ConnectionManager::remove_disabled_connections(const thread tid) {
