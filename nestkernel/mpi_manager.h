...
public:
  ...
  void communicate(std::vector<int> &send_buffer, std::vector<int> &recv_buffer,
                   std::vector<int> &displacements);
  ...
  void communicate(const NodeListType &local_nodes,
                   std::vector<NodeAddressingData> &all_nodes,
                   DictionaryDatum params, bool remote = false);
  ...
  size_t recv_buffer_pos_to_send_buffer_pos_secondary_events(
      const size_t recv_buffer_pos, const thread source_rank);
...
public:
  ...
  public:
    ...
    OffGridSpike() : gid_(0), offset_(0.0) {}
    ...
    void set_gid(gid_external_type gid) { gid_ = static_cast<double>(gid); }
  ...
  class NodeAddressingData {
  public:
    NodeAddressingData() : gid_(0), parent_gid_(0), vp_(0) {}
    NodeAddressingData(unsigned int gid, unsigned int parent_gid,
                       unsigned int vp)
        : gid_(gid), parent_gid_(parent_gid), vp_(vp) {}

    unsigned int get_gid() const { return gid_; }
    unsigned int get_parent_gid() const { return parent_gid_; }
    unsigned int get_vp() const { return vp_; }
...
inline thread MPIManager::get_num_processes() const { return num_processes_; }

inline void MPIManager::set_num_processes(thread n_procs) {
  num_processes_ = n_procs;
}

inline thread MPIManager::get_rank() const { return rank_; }

inline bool MPIManager::is_mpi_used() { return use_mpi_; }

inline size_t MPIManager::get_buffer_size_target_data() const {
...
inline size_t MPIManager::get_buffer_size_spike_data() const {
...
inline size_t MPIManager::get_buffer_size_secondary_events_in_int() const {
  return chunk_size_secondary_events_in_int_ * get_num_processes();
}

inline void MPIManager::set_buffer_size_target_data(const size_t buffer_size) {
...
inline void MPIManager::set_buffer_size_spike_data(const size_t buffer_size) {
  assert(buffer_size >= static_cast<size_t>(2 * get_num_processes()));
...
inline void MPIManager::set_chunk_size_secondary_events_in_int(
    const size_t chunk_size_in_int) {
  chunk_size_secondary_events_in_int_ = chunk_size_in_int;
}

inline size_t MPIManager::get_chunk_size_secondary_events_in_int() const {
  return chunk_size_secondary_events_in_int_;
}

inline size_t MPIManager::recv_buffer_pos_to_send_buffer_pos_secondary_events(
    const size_t recv_buffer_pos, const thread source_rank) {
...
inline bool MPIManager::increase_buffer_size_target_data() {
  ...
  if (buffer_size_target_data_ >= max_buffer_size_target_data_) {
    ...
    if (buffer_size_target_data_ * growth_factor_buffer_target_data_ <
      ...
      set_buffer_size_target_data(static_cast<size_t>(
...
inline bool MPIManager::increase_buffer_size_spike_data() {
...
inline bool MPIManager::adaptive_target_buffers() const {
  return adaptive_target_buffers_;
}

inline bool MPIManager::adaptive_spike_buffers() const {
  return adaptive_spike_buffers_;
}

#ifndef HAVE_MPI
inline std::string MPIManager::get_processor_name() {
  char name[1024];
  name[1023] = '\0';
...
inline void MPIManager::mpi_abort(int) {}

inline void MPIManager::communicate(std::vector<int> &) {}

inline void MPIManager::communicate(std::vector<long> &) {}

inline void MPIManager::communicate_connector_properties(DictionaryDatum &) {}

inline void MPIManager::synchronize() {}

inline void test_link(int, int) {}

inline void test_links() {}

/* replaced u_long with unsigned long since u_long is not known when
 mpi.h is not available. This is a rather ugly fix.
 HEP 2007-03-09
 */
inline bool MPIManager::grng_synchrony(unsigned long) { return true; }

inline bool MPIManager::any_true(const bool my_bool) { return my_bool; }

inline double MPIManager::time_communicate(int, int) { return 0.0; }

inline double MPIManager::time_communicatev(int, int) { return 0.0; }

inline double MPIManager::time_communicate_offgrid(int, int) { return 0.0; }

inline double MPIManager::time_communicate_alltoall(int, int) { return 0.0; }

inline double MPIManager::time_communicate_alltoallv(int, int) { return 0.0; }
...
template <class D>
void MPIManager::communicate_secondary_events_Alltoall(
    std::vector<D> &send_buffer, std::vector<D> &recv_buffer) {
  void *send_buffer_int = static_cast<void *>(&send_buffer[0]);
  void *recv_buffer_int = static_cast<void *>(&recv_buffer[0]);
...
#else // HAVE_MPI
template <class D>
void MPIManager::MPIManager::communicate_Alltoall(
    std::vector<D> &send_buffer, std::vector<D> &recv_buffer,
    const unsigned int send_recv_count) {
...
void MPIManager::communicate_target_data_Alltoall(std::vector<D> &send_buffer,
  ...
  communicate_Alltoall(send_buffer, recv_buffer,
...
void MPIManager::communicate_spike_data_Alltoall(std::vector<D> &send_buffer,
  ...
  const size_t send_recv_count_spike_data_in_int_per_rank =
      sizeof(SpikeData) / sizeof(unsigned int) *
      send_recv_count_spike_data_per_rank_;

  communicate_Alltoall(send_buffer, recv_buffer,
                       send_recv_count_spike_data_in_int_per_rank);
}

template <class D>
void MPIManager::communicate_off_grid_spike_data_Alltoall(
    std::vector<D> &send_buffer, std::vector<D> &recv_buffer) {
