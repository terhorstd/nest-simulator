...
template <> MPI_Datatype MPI_Type<int>::type = MPI_INT;
template <> MPI_Datatype MPI_Type<double>::type = MPI_DOUBLE;
template <> MPI_Datatype MPI_Type<long>::type = MPI_LONG;
template <> MPI_Datatype MPI_Type<unsigned int>::type = MPI_INT;
template <> MPI_Datatype MPI_Type<unsigned long>::type = MPI_UNSIGNED_LONG;

#endif /* #ifdef HAVE_MPI */

nest::MPIManager::MPIManager()
    : num_processes_(1), rank_(0), use_mpi_(false), buffer_size_target_data_(1),
      buffer_size_spike_data_(1), chunk_size_secondary_events_in_int_(0),
      max_buffer_size_target_data_(16777216),
      max_buffer_size_spike_data_(8388608), adaptive_target_buffers_(true),
      adaptive_spike_buffers_(true), growth_factor_buffer_spike_data_(1.5),
      ...
      COMM_OVERFLOW_ERROR(std::numeric_limits<unsigned int>::max()), comm(0),
      MPI_OFFGRID_SPIKE(0)
#endif
{
}
...
#ifndef HAVE_MPI
void nest::MPIManager::init_mpi(int *argc, char **argv[]) {
...
void nest::MPIManager::init_mpi(int *argc, char **argv[]) {
  ...
  MPI_Type_create_struct(2, blockcounts, offsets, source_types,
                         &MPI_OFFGRID_SPIKE);
...
void nest::MPIManager::initialize() {}

void nest::MPIManager::finalize() {}

void nest::MPIManager::set_status(const DictionaryDatum &dict) {
  updateValue<bool>(dict, names::adaptive_target_buffers,
                    adaptive_target_buffers_);
  updateValue<bool>(dict, names::adaptive_spike_buffers,
                    adaptive_spike_buffers_);

  long new_buffer_size_target_data = buffer_size_target_data_;
  updateValue<long>(dict, names::buffer_size_target_data,
                    new_buffer_size_target_data);
  if (new_buffer_size_target_data !=
          static_cast<long>(buffer_size_target_data_) and
      new_buffer_size_target_data <
  ...
  updateValue<long>(dict, names::buffer_size_spike_data,
                    new_buffer_size_spike_data);
  if (new_buffer_size_spike_data !=
          static_cast<long>(buffer_size_spike_data_) and
      new_buffer_size_spike_data <
          static_cast<long>(max_buffer_size_spike_data_)) {
    set_buffer_size_spike_data(new_buffer_size_spike_data);
  }

  updateValue<double>(dict, names::growth_factor_buffer_spike_data,
                      growth_factor_buffer_spike_data_);
  updateValue<double>(dict, names::growth_factor_buffer_target_data,
                      growth_factor_buffer_target_data_);

  updateValue<long>(dict, names::max_buffer_size_target_data,
                    max_buffer_size_target_data_);
  updateValue<long>(dict, names::max_buffer_size_spike_data,
                    max_buffer_size_spike_data_);
}

void nest::MPIManager::get_status(DictionaryDatum &dict) {
  ...
  def<size_t>(dict, names::buffer_size_secondary_events,
              get_buffer_size_secondary_events_in_int());
  def<size_t>(dict, names::max_buffer_size_spike_data,
              max_buffer_size_spike_data_);
  def<size_t>(dict, names::max_buffer_size_target_data,
              max_buffer_size_target_data_);
  def<double>(dict, names::growth_factor_buffer_spike_data,
              growth_factor_buffer_spike_data_);
  def<double>(dict, names::growth_factor_buffer_target_data,
              growth_factor_buffer_target_data_);
}
...
void nest::MPIManager::mpi_finalize(int exitcode) {
  ...
  if (finalized == 0 and initialized == 1) {
    if (exitcode == 0) {
      ...
      LOG(M_INFO, "MPIManager::finalize()",
...
void nest::MPIManager::communicate_Allgather(
    std::vector<unsigned int> &send_buffer,
    std::vector<unsigned int> &recv_buffer, std::vector<int> &displacements) {
...
template <typename T>
void nest::MPIManager::communicate_Allgather(std::vector<T> &send_buffer,
...
void nest::MPIManager::communicate_Allgather(
    ...
    std::vector<OffGridSpike> &recv_buffer, std::vector<int> &displacements) {
  std::vector<int> recv_counts(get_num_processes(), send_buffer_size_);
  // attempt Allgather
  if (send_buffer.size() == static_cast<unsigned int>(send_buffer_size_)) {
 ...
 */
void nest::MPIManager::communicate(std::vector<int> &buffer) {
  communicate_Allgather(buffer);
}

void nest::MPIManager::communicate(std::vector<long> &buffer) {
  communicate_Allgather(buffer);
}

void nest::MPIManager::communicate_Allgather(std::vector<int> &buffer) {
...
void nest::MPIManager::communicate_Allreduce_sum_in_place(double buffer) {
  MPI_Allreduce(MPI_IN_PLACE, &buffer, 1, MPI_Type<double>::type, MPI_SUM,
                comm);
}

void nest::MPIManager::communicate_Allreduce_sum_in_place(
    std::vector<double> &buffer) {
  MPI_Allreduce(MPI_IN_PLACE, &buffer[0], buffer.size(), MPI_Type<double>::type,
                MPI_SUM, comm);
}

void nest::MPIManager::communicate_Allreduce_sum_in_place(
    std::vector<int> &buffer) {
  MPI_Allreduce(MPI_IN_PLACE, &buffer[0], buffer.size(), MPI_Type<int>::type,
                MPI_SUM, comm);
}

void nest::MPIManager::communicate_Allreduce_sum(
    std::vector<double> &send_buffer, std::vector<double> &recv_buffer) {
  assert(recv_buffer.size() == send_buffer.size());
  MPI_Allreduce(&send_buffer[0], &recv_buffer[0], send_buffer.size(),
                MPI_Type<double>::type, MPI_SUM, comm);
}

void nest::MPIManager::communicate_Allreduce_max_in_place(
    std::vector<long> &buffer) {
  MPI_Allreduce(MPI_IN_PLACE, &buffer[0], 1, MPI_LONG, MPI_MAX, comm);
}

void nest::MPIManager::communicate_Allgather(std::vector<long> &buffer) {
  // avoid aliasing, see http://www.mpi-forum.org/docs/mpi-11-html/node10.html
  long my_val = buffer[get_rank()];
  MPI_Allgather(&my_val, 1, MPI_LONG, &buffer[0], 1, MPI_LONG, comm);
}

void nest::MPIManager::communicate_Alltoall_(
    void *send_buffer, void *recv_buffer, const unsigned int send_recv_count) {
  MPI_Alltoall(send_buffer, send_recv_count, MPI_UNSIGNED, recv_buffer,
               send_recv_count, MPI_UNSIGNED, comm);
}

void nest::MPIManager::communicate_secondary_events_Alltoall_(
    void *send_buffer, void *recv_buffer) {
  MPI_Alltoall(send_buffer, chunk_size_secondary_events_in_int_, MPI_UNSIGNED,
               recv_buffer, chunk_size_secondary_events_in_int_, MPI_UNSIGNED,
               comm);
}
 ...
 */
void nest::MPIManager::synchronize() { MPI_Barrier(comm); }

void nest::MPIManager::test_link(int sender, int receiver) {
...
void nest::MPIManager::test_links() {
...
bool nest::MPIManager::grng_synchrony(unsigned long process_rnd_number) {
  if (get_num_processes() > 1) {
    std::vector<unsigned long> rnd_numbers(get_num_processes());
    MPI_Allgather(&process_rnd_number, 1, MPI_UNSIGNED_LONG, &rnd_numbers[0], 1,
...
double nest::MPIManager::time_communicate(int num_bytes, int samples) {
  if (get_num_processes() == 1) {
    return 0.0;
  }
  ...
  std::vector<unsigned int> test_recv_buffer(packet_length *
                                             get_num_processes());
  ...
  for (int i = 0; i < samples; ++i) {
    MPI_Allgather(&test_send_buffer[0], packet_length, MPI_UNSIGNED,
                  &test_recv_buffer[0], packet_length, MPI_UNSIGNED,
                  MPI_COMM_WORLD);
  }
...
double nest::MPIManager::time_communicatev(int num_bytes, int samples) {
  if (get_num_processes() == 1) {
    return 0.0;
  }
  unsigned int packet_length = num_bytes / sizeof(unsigned int);
  ...
  foo.start();
  for (int i = 0; i < samples; ++i) {
    communicate_Allgatherv(test_send_buffer, test_recv_buffer, displacements,
                           n_nodes);
  }
...
double nest::MPIManager::time_communicate_offgrid(int num_bytes, int samples) {
  if (get_num_processes() == 1) {
    return 0.0;
  }
  ...
  std::vector<OffGridSpike> test_recv_buffer(packet_length *
                                             get_num_processes());
  // start time measurement here
  Stopwatch foo;
  foo.start();
  for (int i = 0; i < samples; ++i) {
    MPI_Allgather(&test_send_buffer[0], packet_length, MPI_OFFGRID_SPIKE,
                  &test_recv_buffer[0], packet_length, MPI_OFFGRID_SPIKE,
                  MPI_COMM_WORLD);
  }
  // finish time measurement here
  foo.stop();
...
double nest::MPIManager::time_communicate_alltoall(int num_bytes, int samples) {
  if (get_num_processes() == 1) {
    return 0.0;
  }
...
double nest::MPIManager::time_communicate_alltoallv(int num_bytes,
  ...
  foo.start();
  for (int i = 0; i < samples; ++i) {
    MPI_Alltoallv(&test_send_buffer[0], &n_nodes[0], &displacements[0],
                  MPI_UNSIGNED, &test_recv_buffer[0], &n_nodes[0],
                  &displacements[0], MPI_UNSIGNED, MPI_COMM_WORLD);
  }
  // finish time measurement here
  foo.stop();
  return foo.elapsed() / samples;
...
void nest::MPIManager::communicate_connector_properties(DictionaryDatum &dict) {
  ...
  if (get_num_processes() > 1) {
    ...
    if (num_connections_sum != 0) {
      ...
      // Start communication.
      communicate_Allgatherv<long>(targets, targets_result, displacements,
                                   num_connections);

      communicate_Allgatherv<long>(receptors, receptors_result, displacements,
                                   num_connections);

      communicate_Allgatherv<double>(weights, weights_result, displacements,
                                     num_connections);

      communicate_Allgatherv<double>(delays, delays_result, displacements,
                                     num_connections);
