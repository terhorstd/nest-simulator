...
struct sDynModule {
  ...
  bool operator==(const sDynModule &rhs) const { return name == rhs.name; }

  // operator!= must be implemented explicitly, not all compilers
  // generate it automatically from operator==
  bool operator!=(const sDynModule &rhs) const { return not(*this == rhs); }
};
...
bool has_name(SLIModule const *const m, const std::string n) {
  return m->name() == n;
}
...
void DynamicLoaderModule::LoadModuleFunction::execute(SLIInterpreter *i) const {
  ...
  if (kernel().model_manager.has_user_models() or
      kernel().model_manager.has_user_prototypes()) {
  ...
  if (std::find(dyn_modules_.begin(), dyn_modules_.end(), new_module) !=
      dyn_modules_.end()) {
    throw DynamicModuleManagementError("Module '" + new_module.name +
                                       "' is loaded already.");
  }
  ...
  if (std::find_if(
          DynamicLoaderModule::getLinkedModules().begin(),
...
void DynamicLoaderModule::init(SLIInterpreter *i) {
  ...
  if (not dl_error) {
    ...
    if (path != NULL) {
      ...
      if (dl_error) {
        LOG(M_ERROR, "DynamicLoaderModule::init",
...
int DynamicLoaderModule::registerLinkedModule(SLIModule *pModule) {
  assert(pModule != 0);
