...
nest::SourceTablePosition nest::SourceTable::find_maximal_position() const {
...
void nest::SourceTable::clean(const thread tid) {
  ...
  if (max_position.tid == tid) {
    for (synindex syn_id = max_position.syn_id; syn_id < sources_[tid].size();
         ++syn_id) {
...
void nest::SourceTable::reserve(const thread tid, const synindex syn_id,
                                const size_t count) {}

nest::index nest::SourceTable::get_gid(const thread tid, const synindex syn_id,
...
nest::index nest::SourceTable::remove_disabled_sources(const thread tid,
...
void nest::SourceTable::compute_buffer_pos_for_unique_secondary_sources(
    const thread tid,
  ...
  for (size_t syn_id = 0; syn_id < sources_[tid].size(); ++syn_id) {
    if (not kernel()
      ...
      for (BlockVector<Source>::const_iterator source_cit =
           ...
           source_cit != sources_[tid][syn_id].end(); ++source_cit) {
#pragma omp critical
        {
          (*unique_secondary_source_gid_syn_id)
...
#pragma omp single
  {
    ...
    std::vector<long> max_uint_count(
        1, *std::max_element(recv_buffer_position_by_rank.begin(),
...
void nest::SourceTable::resize_sources(const thread tid) {
  sources_[tid].resize(kernel().model_manager.get_num_synapse_prototypes());
}

bool nest::SourceTable::get_next_target_data(const thread tid,
  ...
  while (true) {
    ...
    if (const_current_source.is_processed() or
        const_current_source.is_disabled()) {
    ...
    if ((current_position.lcid - 1 >= 0) and
        (sources_[current_position.tid][current_position.syn_id]
                 [current_position.lcid - 1]
                     .get_gid() == current_source.get_gid()) and
        (not sources_[current_position.tid][current_position.syn_id]
                     [current_position.lcid - 1]
                         .is_processed())) {
      --current_position.lcid;
      continue;
      ...
      if (current_source.is_primary()) {
        ...
        const size_t recv_buffer_pos =
            kernel().connection_manager.get_secondary_recv_buffer_position(
                current_position.tid, current_position.syn_id,
                current_position.lcid);
        ...
        const size_t send_buffer_pos =
            kernel()
                .mpi_manager
                .recv_buffer_pos_to_send_buffer_pos_secondary_events(
