...
#pragma omp parallel
  {
    ...
    for (std::vector<std::vector<ConnectorBase *>>::iterator iit =
         ...
         iit != target_to_devices_[tid].end(); ++iit) {
    ...
    for (std::vector<std::vector<ConnectorBase *>>::iterator iit =
         ...
         iit != target_from_devices_[tid].end(); ++iit) {
      for (std::vector<ConnectorBase *>::iterator iiit = iit->begin();
           iiit != iit->end(); ++iiit) {
        delete *iiit;
      }
...
void nest::TargetTableDevices::get_connections_to_devices_(
    const index requested_source_gid, const index requested_target_gid,
    const thread tid, const synindex syn_id, const long synapse_label,
    std::deque<ConnectionID> &conns) const {
  if (requested_source_gid != 0) {
    const index lid = kernel().vp_manager.gid_to_lid(requested_source_gid);
    get_connections_to_device_for_lid_(lid, requested_target_gid, tid, syn_id,
                                       synapse_label, conns);
  } else {
    for (index lid = 0; lid < target_to_devices_[tid].size(); ++lid) {
      get_connections_to_device_for_lid_(lid, requested_target_gid, tid, syn_id,
...
void nest::TargetTableDevices::get_connections_to_device_for_lid_(
...
void nest::TargetTableDevices::get_connections_from_devices_(
    const index requested_source_gid, const index requested_target_gid,
    const thread tid, const synindex syn_id, const long synapse_label,
    std::deque<ConnectionID> &conns) const {
  for (std::vector<index>::const_iterator it =
    ...
    if (source_gid > 0 and
        (requested_source_gid == source_gid or requested_source_gid == 0)) {
...
void nest::TargetTableDevices::get_connections(
    const index requested_source_gid, const index requested_target_gid,
    const thread tid, const synindex syn_id, const long synapse_label,
    std::deque<ConnectionID> &conns) const {
  // collect all connections from neurons to devices
  get_connections_to_devices_(requested_source_gid, requested_target_gid, tid,
                              syn_id, synapse_label, conns);

  // collect all connections from devices
  get_connections_from_devices_(requested_source_gid, requested_target_gid, tid,
                                syn_id, synapse_label, conns);
}
