...
inline Source::Source() : gid_(0), processed_(false), primary_(true) {}

inline Source::Source(const uint64_t gid, const bool is_primary)
    : gid_(gid), processed_(false), primary_(is_primary) {
  assert(gid < GID_DISABLED);
}

inline void Source::set_gid(const uint64_t gid) {
  assert(gid < GID_DISABLED);
  gid_ = gid;
}

inline uint64_t Source::get_gid() const { return gid_; }

inline void Source::set_processed(const bool processed) {
  processed_ = processed;
}

inline bool Source::is_processed() const { return processed_; }

inline void Source::set_primary(const bool primary) { primary_ = primary; }

inline bool Source::is_primary() const { return primary_; }

inline void Source::disable() { gid_ = GID_DISABLED; }

inline bool Source::is_disabled() const { return gid_ == GID_DISABLED; }
