...
inline void SiblingContainer::push_back(Node *n) { nodes_.push_back(n); }

inline std::vector<Node *>::iterator SiblingContainer::begin() {
  return nodes_.begin();
}

inline std::vector<Node *>::iterator SiblingContainer::end() {
  return nodes_.end();
}

inline std::vector<Node *>::const_iterator SiblingContainer::begin() const {
  return nodes_.begin();
}

inline std::vector<Node *>::const_iterator SiblingContainer::end() const {
  return nodes_.end();
}

inline bool SiblingContainer::empty() const { return nodes_.empty(); }

inline size_t SiblingContainer::num_thread_siblings() const {
  return nodes_.size();
}

inline void SiblingContainer::reserve(size_t n) { nodes_.reserve(n); }

inline bool SiblingContainer::has_proxies() const { return false; }

inline bool SiblingContainer::is_subnet() const {
  return empty() ? false : nodes_[0]->is_subnet();
}

inline Node *SiblingContainer::get_thread_sibling(index i) const {
