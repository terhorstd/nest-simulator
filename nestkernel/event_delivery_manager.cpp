...
EventDeliveryManager::EventDeliveryManager()
    : off_grid_spiking_(false), moduli_(), slice_moduli_(), spike_register_(),
      off_grid_spike_register_(), send_buffer_secondary_events_(),
      recv_buffer_secondary_events_(), time_collocate_(0.0),
      time_communicate_(0.0), local_spike_counter_(), send_buffer_spike_data_(),
      recv_buffer_spike_data_(), send_buffer_off_grid_spike_data_(),
      recv_buffer_off_grid_spike_data_(), send_buffer_target_data_(),
      recv_buffer_target_data_(), buffer_size_target_data_has_changed_(false),
      buffer_size_spike_data_has_changed_(false), gather_completed_checker_() {}

EventDeliveryManager::~EventDeliveryManager() {}

void EventDeliveryManager::initialize() {
...
#pragma omp parallel
  {
    ...
    off_grid_spike_register_[tid].resize(
        num_threads, std::vector<std::vector<OffGridTarget>>(
                         kernel().connection_manager.get_min_delay(),
                         std::vector<OffGridTarget>()));
  } // of omp parallel
}

void EventDeliveryManager::finalize() {
  // clear the spike buffers
  std::vector<std::vector<std::vector<std::vector<Target>>>>().swap(
      spike_register_);
  std::vector<std::vector<std::vector<std::vector<OffGridTarget>>>>().swap(
...
void EventDeliveryManager::set_status(const DictionaryDatum &dict) {
  updateValue<bool>(dict, names::off_grid_spiking, off_grid_spiking_);
}

void EventDeliveryManager::get_status(DictionaryDatum &dict) {
  ...
  def<unsigned long>(dict, names::local_spike_counter,
                     std::accumulate(local_spike_counter_.begin(),
                                     local_spike_counter_.end(), 0));
}

void EventDeliveryManager::clear_pending_spikes() {
  configure_spike_data_buffers();
}

void EventDeliveryManager::resize_send_recv_buffers_target_data() {
...
void EventDeliveryManager::resize_send_recv_buffers_spike_data_() {
...
void EventDeliveryManager::configure_spike_data_buffers() {
...
void EventDeliveryManager::configure_secondary_buffers() {
  send_buffer_secondary_events_.clear();
...
void EventDeliveryManager::init_moduli() {
 ...
 */
void EventDeliveryManager::update_moduli() {
  delay min_delay = kernel().connection_manager.get_min_delay();
  delay max_delay = kernel().connection_manager.get_max_delay();
  assert(min_delay != 0);
  assert(max_delay != 0);
...
void EventDeliveryManager::reset_timers_counters() {
  time_collocate_ = 0.0;
...
void EventDeliveryManager::gather_secondary_events(const bool done) {
...
bool EventDeliveryManager::deliver_secondary_events(
...
void EventDeliveryManager::gather_spike_data(const thread tid) {
  if (off_grid_spiking_) {
    gather_spike_data_(tid, send_buffer_off_grid_spike_data_,
...
void EventDeliveryManager::gather_spike_data_(
    const thread tid, std::vector<SpikeDataT> &send_buffer,
...
#pragma omp single
    {
      if (kernel().mpi_manager.adaptive_spike_buffers() and
    ...
    const bool collocate_completed =
        collocate_spike_data_buffers_(tid, assigned_ranks, send_buffer_position,
                                      spike_register_, send_buffer);
    ...
    if (off_grid_spiking_) {
      const bool collocate_completed_off_grid = collocate_spike_data_buffers_(
          tid, assigned_ranks, send_buffer_position, off_grid_spike_register_,
          send_buffer);
...
#pragma omp barrier
    ...
    set_end_and_invalid_markers_(assigned_ranks, send_buffer_position,
                                 send_buffer);
    ...
    if (gather_completed_checker_.all_true()) {
      ...
      set_complete_marker_spike_data_(assigned_ranks, send_buffer_position,
                                      send_buffer);
#pragma omp barrier
    }
...
#pragma omp barrier
    ...
    if (not gather_completed_checker_.all_true() and
        kernel().mpi_manager.adaptive_spike_buffers()) {
#pragma omp single
      {
        buffer_size_spike_data_has_changed_ =
...
bool EventDeliveryManager::collocate_spike_data_buffers_(
  ...
  reset_complete_marker_spike_data_(assigned_ranks, send_buffer_position,
                                    send_buffer);
  ...
  for (typename std::vector<
           std::vector<std::vector<std::vector<TargetT>>>>::iterator it =
           spike_register.begin();
       it != spike_register.end(); ++it) {
      ...
      for (typename std::vector<TargetT>::iterator iiit =
        ...
        if (send_buffer_position.is_chunk_filled(rank)) {
          ...
          send_buffer[send_buffer_position.idx(rank)].set(
              (*iiit).get_tid(), (*iiit).get_syn_id(), (*iiit).get_lcid(), lag,
              (*iiit).get_offset());
...
template <typename SpikeDataT>
void EventDeliveryManager::set_end_and_invalid_markers_(
    const AssignedRanks &assigned_ranks,
...
template <typename SpikeDataT>
void EventDeliveryManager::set_complete_marker_spike_data_(
    const AssignedRanks &assigned_ranks,
    const SendBufferPosition &send_buffer_position,
    std::vector<SpikeDataT> &send_buffer) const {
  for (thread target_rank = assigned_ranks.begin;
       target_rank < assigned_ranks.end; ++target_rank) {
...
template <typename SpikeDataT>
bool EventDeliveryManager::deliver_events_(
    const thread tid, const std::vector<SpikeDataT> &recv_buffer) {
  ...
  std::vector<Time> prepared_timestamps(
      kernel().connection_manager.get_min_delay());
  for (size_t lag = 0;
       lag < (size_t)kernel().connection_manager.get_min_delay(); ++lag) {
  ...
  for (thread rank = 0; rank < kernel().mpi_manager.get_num_processes();
    ...
    if (not recv_buffer[(rank + 1) * send_recv_count_spike_data_per_rank - 1]
                .is_complete_marker()) {
...
void EventDeliveryManager::gather_target_data(const thread tid) {
...
#pragma omp single
    {
      if (kernel().mpi_manager.adaptive_target_buffers() and
    ...
    if (gather_completed_checker_.all_true()) {
      set_complete_marker_target_data_(tid, assigned_ranks,
...
#pragma omp barrier
    ...
    // resize mpi buffers, if necessary and allowed
    if (not gather_completed_checker_.all_true() and
        kernel().mpi_manager.adaptive_target_buffers()) {
#pragma omp single
      {
        buffer_size_target_data_has_changed_ =
...
bool EventDeliveryManager::collocate_target_data_buffers_(
    const thread tid, const AssignedRanks &assigned_ranks,
    SendBufferPosition &send_buffer_position) {
  ...
  while (true) {
    valid_next_target_data = kernel().connection_manager.get_next_target_data(
        tid, assigned_ranks.begin, assigned_ranks.end, source_rank,
        next_target_data);
...
void nest::EventDeliveryManager::set_complete_marker_target_data_(
    const thread tid, const AssignedRanks &assigned_ranks,
    const SendBufferPosition &send_buffer_position) {
  for (thread rank = assigned_ranks.begin; rank < assigned_ranks.end; ++rank) {
    const thread idx = send_buffer_position.end(rank) - 1;
...
bool nest::EventDeliveryManager::distribute_target_data_buffers_(
    const thread tid) {
  bool are_others_completed = true;
...
void EventDeliveryManager::resize_spike_register_(const thread tid) {
