...
inline SendBufferPosition::SendBufferPosition(
    ...
    const unsigned int send_recv_count_per_rank)
    : num_spike_data_written_(0), max_size_(assigned_ranks.max_size),
...
inline thread SendBufferPosition::rank_to_index_(const thread rank) const {
  return rank % max_size_;
}

inline unsigned int SendBufferPosition::idx(const thread rank) const {
  return idx_[rank_to_index_(rank)];
}

inline unsigned int SendBufferPosition::begin(const thread rank) const {
  return begin_[rank_to_index_(rank)];
}

inline unsigned int SendBufferPosition::end(const thread rank) const {
  return end_[rank_to_index_(rank)];
}

inline bool SendBufferPosition::is_chunk_filled(const thread rank) const {
  return idx(rank) == end(rank);
}

inline bool SendBufferPosition::are_all_chunks_filled() const {
  return num_spike_data_written_ == send_recv_count_per_rank * idx_.size();
}

inline void SendBufferPosition::increase(const thread rank) {
