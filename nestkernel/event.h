...
inline SpikeEvent::SpikeEvent() : multiplicity_(1) {}

inline SpikeEvent *SpikeEvent::clone() const { return new SpikeEvent(*this); }

inline void SpikeEvent::set_multiplicity(int multiplicity) {
  multiplicity_ = multiplicity;
}

inline int SpikeEvent::get_multiplicity() const { return multiplicity_; }
...
inline WeightRecorderEvent::WeightRecorderEvent() : receiver_gid_(0) {}

inline WeightRecorderEvent *WeightRecorderEvent::clone() const {
  return new WeightRecorderEvent(*this);
}

inline void WeightRecorderEvent::set_receiver_gid(index gid) {
  receiver_gid_ = gid;
}

inline index WeightRecorderEvent::get_receiver_gid(void) const {
...
inline RateEvent *RateEvent::clone() const { return new RateEvent(*this); }

inline void RateEvent::set_rate(double r) { r_ = r; }

inline double RateEvent::get_rate() const { return r_; }
...
inline CurrentEvent *CurrentEvent::clone() const {
  return new CurrentEvent(*this);
}

inline void CurrentEvent::set_current(double c) { c_ = c; }

inline double CurrentEvent::get_current() const { return c_; }
...
inline DataLoggingRequest::DataLoggingRequest()
    : Event(), recording_interval_(Time::neg_inf()),
      recording_offset_(Time::ms(0.)), record_from_(0) {}

inline DataLoggingRequest::DataLoggingRequest(const Time &rec_int,
                                              const std::vector<Name> &recs)
    : Event(), recording_interval_(rec_int), record_from_(&recs) {}

inline DataLoggingRequest::DataLoggingRequest(const Time &rec_int,
                                              const Time &rec_offset,
                                              const std::vector<Name> &recs)
    : Event(), recording_interval_(rec_int), recording_offset_(rec_offset),
      record_from_(&recs) {}

inline DataLoggingRequest *DataLoggingRequest::clone() const {
  return new DataLoggingRequest(*this);
}

inline const Time &DataLoggingRequest::get_recording_interval() const {
...
inline const Time &DataLoggingRequest::get_recording_offset() const {
  assert(recording_offset_.is_finite());
  return recording_offset_;
}

inline const std::vector<Name> &DataLoggingRequest::record_from() const {
...
inline ConductanceEvent *ConductanceEvent::clone() const {
  return new ConductanceEvent(*this);
}

inline void ConductanceEvent::set_conductance(double g) { g_ = g; }

inline double ConductanceEvent::get_conductance() const { return g_; }
 ...
 *        otherwise.
 */
template <typename D> class DataEvent : public Event {
  lockPTR<D> data_;
...
template <typename D> inline void DataEvent<D>::set_pointer(D &data) {
  data_ = data;
}

template <typename D> inline lockPTR<D> DataEvent<D>::get_pointer() const {
  return data_;
}
...
inline DoubleDataEvent *DoubleDataEvent::clone() const {
...
public:
  ...
  virtual std::vector<unsigned int>::iterator &
 ...
 */
template <typename T> size_t number_of_uints_covered(void) {
...
void write_to_comm_buffer(T d, std::vector<unsigned int>::iterator &pos) {
  ...
  for (size_t i = 0; i < num_uints; i++) {
    memcpy(&(*(pos + i)), c + i * sizeof(unsigned int),
...
void read_from_comm_buffer(T &d, std::vector<unsigned int>::iterator &pos) {
  ...
  for (size_t i = 0; i < num_uints; i++) {
    memcpy(c + i * sizeof(unsigned int), &(*(pos + i)),
           std::min(left_to_copy, sizeof(unsigned int)));
    left_to_copy -= sizeof(unsigned int);
  }
...
public:
   ...
   */
  static void set_syn_id(const synindex synid) {
   ...
   */
  void add_syn_id(const synindex synid) {
    assert(not supports_syn_id(synid));
    VPManager::assert_single_threaded();
    supported_syn_ids_.push_back(synid);
  }

  const std::vector<synindex> &get_supported_syn_ids() const {
  ...
  bool supports_syn_id(const synindex synid) const {
    return (std::find(supported_syn_ids_.begin(), supported_syn_ids_.end(),
                      synid) != supported_syn_ids_.end());
  }
...
public:
  ...
  void set_diffusion_factor(weight t) { diffusion_factor_ = t; };

  void set_drift_factor(weight t) { drift_factor_ = t; };
...
template <typename DataType, typename Subclass>
inline DataType DataSecondaryEvent<DataType, Subclass>::get_coeffvalue(
...
inline GapJunctionEvent *GapJunctionEvent::clone() const {
...
inline DelayedRateConnectionEvent *DelayedRateConnectionEvent::clone() const {
  return new DelayedRateConnectionEvent(*this);
}

inline DiffusionConnectionEvent *DiffusionConnectionEvent::clone() const {
  return new DiffusionConnectionEvent(*this);
}

inline weight DiffusionConnectionEvent::get_drift_factor() const {
  return drift_factor_;
}

inline weight DiffusionConnectionEvent::get_diffusion_factor() const {
...
inline void Event::set_receiver(Node &r) { receiver_ = &r; }

inline void Event::set_sender(Node &s) { sender_ = &s; }

inline void Event::set_sender_gid(index gid) { sender_gid_ = gid; }

inline Node &Event::get_receiver(void) const { return *receiver_; }

inline Node &Event::get_sender(void) const { return *sender_; }

inline index Event::get_sender_gid(void) const {
  assert(sender_gid_ > 0);
  return sender_gid_;
}

inline weight Event::get_weight() const { return w_; }

inline void Event::set_weight(weight w) { w_ = w; }

inline Time const &Event::get_stamp() const { return stamp_; }

inline void Event::set_stamp(Time const &s) {
  stamp_ = s;
...
inline delay Event::get_delay_steps() const { return d_; }

inline long Event::get_rel_delivery_steps(const Time &t) const {
  if (stamp_steps_ == 0) {
...
inline void Event::set_delay_steps(delay d) { d_ = d; }

inline double Event::get_offset() const { return offset_; }

inline void Event::set_offset(double t) { offset_ = t; }

inline port Event::get_port() const { return p_; }

inline rport Event::get_rport() const { return rp_; }

inline void Event::set_port(port p) { p_ = p; }

inline void Event::set_rport(rport rp) { rp_ = rp; }
} // namespace nest
