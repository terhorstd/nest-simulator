...
nest::DynamicUniversalDataLogger<HostNode>::DynamicUniversalDataLogger(
    HostNode &host)
    : host_(host), data_loggers_() {}

template <typename HostNode>
void nest::DynamicUniversalDataLogger<HostNode>::reset() {
...
template <typename HostNode>
void nest::DynamicUniversalDataLogger<HostNode>::init() {
  for (DLiter_ it = data_loggers_.begin(); it != data_loggers_.end(); ++it) {
    it->init();
  }
}
...
template <typename HostNode>
void nest::DynamicUniversalDataLogger<HostNode>::record_data(long step) {
  for (DLiter_ it = data_loggers_.begin(); it != data_loggers_.end(); ++it) {
...
template <typename HostNode>
void nest::DynamicUniversalDataLogger<HostNode>::handle(
...
template <typename HostNode>
void nest::DynamicUniversalDataLogger<HostNode>::DataLogger_::reset() {
  data_.clear();
  next_rec_step_ = -1; // flag as uninitialized
}

template <typename HostNode>
void nest::DynamicUniversalDataLogger<HostNode>::DataLogger_::init() {
  if (num_vars_ < 1) {
    return;
  ...
  data_.resize(2, DataLoggingReply::Container(
...
template <typename HostNode>
void nest::DynamicUniversalDataLogger<HostNode>::DataLogger_::record_data(
    const HostNode &host, long step) {
...
template <typename HostNode>
void nest::DynamicUniversalDataLogger<HostNode>::DataLogger_::handle(
    HostNode &host, const DataLoggingRequest &request) {
  if (num_vars_ < 1) {
    return;
  } // nothing to do
...
template <typename HostNode> void nest::UniversalDataLogger<HostNode>::reset() {
  for (DLiter_ it = data_loggers_.begin(); it != data_loggers_.end(); ++it) {
    it->reset();
  }
}

template <typename HostNode> void nest::UniversalDataLogger<HostNode>::init() {
  for (DLiter_ it = data_loggers_.begin(); it != data_loggers_.end(); ++it) {
    it->init();
  }
}
...
template <typename HostNode>
void nest::UniversalDataLogger<HostNode>::record_data(long step) {
  for (DLiter_ it = data_loggers_.begin(); it != data_loggers_.end(); ++it) {
    it->record_data(host_, step);
  }
}
...
template <typename HostNode>
void nest::UniversalDataLogger<HostNode>::handle(
    const DataLoggingRequest &dlr) {
  const rport rport = dlr.get_rport();
  assert(rport >= 1);
  assert(static_cast<size_t>(rport) <= data_loggers_.size());
...
template <typename HostNode>
void nest::UniversalDataLogger<HostNode>::DataLogger_::record_data(
    const HostNode &host, long step) {
...
template <typename HostNode>
void nest::UniversalDataLogger<HostNode>::DataLogger_::handle(
    HostNode &host, const DataLoggingRequest &request) {
  if (num_vars_ < 1) {
    return;
  } // nothing to do
