...
ConnectorModel::ConnectorModel(const std::string name, const bool is_primary,
                               const bool has_delay,
                               const bool requires_symmetric,
                               const bool supports_wfr,
                               const bool requires_clopath_archiving)
    : name_(name), default_delay_needs_check_(true), is_primary_(is_primary),
      has_delay_(has_delay), requires_symmetric_(requires_symmetric),
...
ConnectorModel::ConnectorModel(const ConnectorModel &cm, const std::string name)
    : name_(name), default_delay_needs_check_(true),
      is_primary_(cm.is_primary_), has_delay_(cm.has_delay_),
