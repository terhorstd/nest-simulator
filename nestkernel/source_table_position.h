...
inline SourceTablePosition::SourceTablePosition()
    : tid(-1), syn_id(-1), lcid(-1) {}

inline SourceTablePosition::SourceTablePosition(const long tid,
                                                const long syn_id,
                                                const long lcid)
    : tid(tid), syn_id(syn_id), lcid(lcid) {}

inline SourceTablePosition::SourceTablePosition(const SourceTablePosition &rhs)
    : tid(rhs.tid), syn_id(rhs.syn_id), lcid(rhs.lcid) {}

template <typename T>
inline void SourceTablePosition::wrap_position(
...
inline bool operator==(const SourceTablePosition &lhs,
  ...
  return ((lhs.tid == rhs.tid) and (lhs.syn_id == rhs.syn_id) and
          (lhs.lcid == rhs.lcid));
}
