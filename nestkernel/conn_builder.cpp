...
nest::ConnBuilder::ConnBuilder(const GIDCollection &sources,
    ...
    : sources_(&sources), targets_(&targets), autapses_(true), multapses_(true),
      make_symmetric_(false), creates_symmetric_connections_(false),
      exceptions_raised_(kernel().vp_manager.get_num_threads()),
      synapse_model_id_(
          kernel().model_manager.get_synapsedict()->lookup("static_synapse")),
      weight_(0), delay_(0), param_dicts_(), parameters_requiring_skipping_() {
  ...
  if (syn_spec->known(names::pre_synaptic_element) and
    ...
    if (syn_spec->known(names::pre_synaptic_element) or
        syn_spec->known(names::post_synaptic_element)) {
      throw BadProperty(
  ...
  for (Dictionary::const_iterator default_it = syn_defaults->begin();
       default_it != syn_defaults->end(); ++default_it) {
  ...
  if (synapse_params_.size() > 0) {
    for (thread tid = 0; tid < kernel().vp_manager.get_num_threads(); ++tid) {
      ...
      for (ConnParameterMap::const_iterator it = synapse_params_.begin();
           it != synapse_params_.end(); ++it) {
        if (it->first == names::receptor_type or
            it->first == names::music_channel or
 ...
 * @return
 */
bool nest::ConnBuilder::change_connected_synaptic_elements(index sgid,
                                                           index tgid,
                                                           const thread tid,
                                                           int update) {
...
void nest::ConnBuilder::connect() {
  ...
  if (kernel().model_manager.connector_requires_symmetric(synapse_model_id_) and
      not(is_symmetric() or make_symmetric_)) {
    throw BadProperty(
...
void nest::ConnBuilder::single_connect_(index sgid, Node &target,
                                        thread target_thread,
                                        librandom::RngPtr &rng) {
  if (this->requires_proxies() and not target.has_proxies()) {
    throw IllegalConnection("Cannot use this rule to connect to nodes"
  ...
  if (param_dicts_.empty()) // indicates we have no synapse params
  {
    if (default_weight_and_delay_) {
      kernel().connection_manager.connect(sgid, &target, target_thread,
                                          synapse_model_id_, dummy_param_);
    } else if (default_weight_) {
      ...
      kernel().connection_manager.connect(
          sgid, &target, target_thread, synapse_model_id_, dummy_param_,
          numerics::nan, weight_->value_double(target_thread, rng));
    } else {
    ...
    if (default_weight_and_delay_) {
      ...
      kernel().connection_manager.connect(
          sgid, &target, target_thread, synapse_model_id_,
          param_dicts_[target_thread], numerics::nan,
...
void nest::ConnBuilder::set_pre_synaptic_element_name(const std::string &name) {
  if (name.empty()) {
...
void nest::ConnBuilder::set_post_synaptic_element_name(
...
bool nest::ConnBuilder::all_parameters_scalar_() const {
...
bool nest::ConnBuilder::loop_over_targets_() const {
  return targets_->size() < kernel().node_manager.local_nodes_size() or
         not targets_->is_range() or parameters_requiring_skipping_.size() > 0;
}
    ...
    try {
      ...
      if (loop_over_targets_()) {
        ...
        for (SparseNodeArray::const_iterator it =
             ...
             it != kernel().node_manager.local_nodes_end(); ++it) {
...
void nest::AllToAllBuilder::inner_connect_(const int tid,
                                           librandom::RngPtr &rng, Node *target,
                                           index tgid, bool skip) {
  const thread target_thread = target->get_thread();
...
nest::FixedInDegreeBuilder::FixedInDegreeBuilder(
    const GIDCollection &sources, const GIDCollection &targets,
  ...
  if (not multapses_) {
    if (indegree_ > n_sources) {
      ...
      LOG(M_WARNING, "FixedInDegreeBuilder::connect",
...
void nest::FixedInDegreeBuilder::inner_connect_(const int tid,
                                                librandom::RngPtr &rng,
                                                Node *target, index tgid,
                                                bool skip) {
  const thread target_thread = target->get_thread();
  ...
  for (long j = 0; j < indegree_; ++j) {
    ...
    do {
      ...
      sgid = (*sources_)[s_id];
    } while ((not autapses_ and sgid == tgid) or
...
nest::FixedOutDegreeBuilder::FixedOutDegreeBuilder(
  ...
  if (not multapses_) {
    if (outdegree_ > n_targets) {
      ...
      LOG(M_WARNING, "FixedOutDegreeBuilder::connect",
          "Multapses and autapses prohibited. When the sources and the targets "
          "have a non-empty "
          "intersection, the connect algorithm will enter an infinite loop.");
      return;
  ...
  for (GIDCollection::const_iterator sgid = sources_->begin();
    ...
    for (long j = 0; j < outdegree_; ++j) {
      ...
      do {
        ...
        tgid = (*targets_)[t_id];
      } while ((not autapses_ and tgid == *sgid) or
...
void nest::FixedTotalNumberBuilder::connect_() {
  ...
  local_targets.reserve(size_targets /
...
void nest::BernoulliBuilder::inner_connect_(const int tid,
                                            librandom::RngPtr &rng,
                                            Node *target, index tgid) {
  const thread target_thread = target->get_thread();
    ...
    try {
      ...
      for (GIDCollection::const_iterator tgid = targets_->begin();
        ...
        while (i < indegree) {
          ...
          if (sgid == *tgid or
              previous_sgids.find(sgid) != previous_sgids.end()) {
            continue;
          }
...
void nest::SPBuilder::update_delay(delay &d) const {
...
void nest::SPBuilder::sp_connect(GIDCollection sources, GIDCollection targets) {
 ...
 */
void nest::SPBuilder::connect_(GIDCollection sources, GIDCollection targets) {
