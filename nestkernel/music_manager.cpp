...
/*
     - set the ... properties
*/
void MUSICManager::set_status(const DictionaryDatum &) {}

void MUSICManager::get_status(DictionaryDatum &) {}

void MUSICManager::init_music(int *argc, char **argv[]) {
#ifdef HAVE_MUSIC
  int provided_thread_level;
  music_setup = new MUSIC::Setup(*argc, *argv, MPI_THREAD_FUNNELED,
...
MUSIC::Setup *MUSICManager::get_music_setup() { return music_setup; }

MUSIC::Runtime *MUSICManager::get_music_runtime() { return music_runtime; }

void MUSICManager::advance_music_time() { music_runtime->tick(); }

void MUSICManager::register_music_in_port(std::string portname, bool pristine) {
...
void MUSICManager::unregister_music_in_port(std::string portname) {
  std::map<std::string, MusicPortData>::iterator it;
  it = music_in_portlist_.find(portname);
  if (it == music_in_portlist_.end()) {
...
void MUSICManager::register_music_event_in_proxy(std::string portname,
                                                 int channel, nest::Node *mp) {
...
void MUSICManager::set_music_in_port_acceptable_latency(std::string portname,
                                                        double latency) {
  std::map<std::string, MusicPortData>::iterator it;
  it = music_in_portlist_.find(portname);
  if (it == music_in_portlist_.end()) {
...
void MUSICManager::set_music_in_port_max_buffered(std::string portname,
                                                  int maxbuffered) {
  std::map<std::string, MusicPortData>::iterator it;
  it = music_in_portlist_.find(portname);
  if (it == music_in_portlist_.end()) {
...
void MUSICManager::publish_music_in_ports_() {
  std::map<std::string, MusicEventHandler>::iterator it;
...
void MUSICManager::update_music_event_handlers(Time const &origin,
