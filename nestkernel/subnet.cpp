...
nest::Subnet::Subnet()
    : Node(), nodes_(), gids_(), label_(), customdict_(new Dictionary),
      homogeneous_(true), last_mid_(0) {
  set_frozen_(true); // freeze subnet by default
}

nest::Subnet::Subnet(const Subnet &c)
    : Node(c), nodes_(c.nodes_), gids_(c.gids_), label_(c.label_),
      customdict_(new Dictionary(*(c.customdict_))),
      homogeneous_(c.homogeneous_), last_mid_(c.last_mid_) {}
...
std::string nest::Subnet::print_network(int max_depth, int level,
                                        std::string prefix) {
...
void nest::Subnet::set_label(std::string const label) {
...
bool nest::Subnet::is_subnet() const { return true; }
