...
inline Multirange::Multirange() : ranges_(), size_(0) {}

inline void Multirange::push_back(index x) {
  if (contains(x)) {
    return;
  }
...
inline void Multirange::add_range(index start, index end) {
  ranges_.push_back(Range(start, end));
  size_ += end - start + 1;
}

inline bool Multirange::contains(index x) {
...
inline void Multirange::clear() {
  ranges_.clear();
  size_ = 0;
}

inline index Multirange::size() const { return size_; }

inline bool Multirange::empty() const { return size_ == 0; }
...
inline Multirange::iterator Multirange::begin() const {
  return Multirange::iterator(ranges_.begin(), 0);
}

inline Multirange::iterator Multirange::end() const {
