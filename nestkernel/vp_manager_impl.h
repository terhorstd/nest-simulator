...
inline thread VPManager::get_vp() const {
  return kernel().mpi_manager.get_rank() +
         get_thread_id() * kernel().mpi_manager.get_num_processes();
}

inline thread VPManager::suggest_vp_for_gid(const index gid) const {
  return gid % get_num_virtual_processes();
}

inline thread VPManager::vp_to_thread(const thread vp) const {
  return vp / kernel().mpi_manager.get_num_processes();
}

inline thread VPManager::get_num_virtual_processes() const {
  return get_num_threads() * kernel().mpi_manager.get_num_processes();
}

inline bool VPManager::is_local_vp(const thread vp) const {
  return kernel().mpi_manager.get_process_id_of_vp(vp) ==
         kernel().mpi_manager.get_rank();
}

inline thread VPManager::thread_to_vp(const thread tid) const {
  return tid * kernel().mpi_manager.get_num_processes() +
         kernel().mpi_manager.get_rank();
}

inline bool VPManager::is_gid_vp_local(const index gid) const {
  return (gid % get_num_virtual_processes() == static_cast<index>(get_vp()));
}

inline index VPManager::gid_to_lid(const index gid) const {
...
inline index VPManager::lid_to_gid(const index lid) const {
  const index vp = get_vp();
  return (lid + static_cast<index>(vp == 0)) * get_num_virtual_processes() + vp;
}

inline thread VPManager::get_num_assigned_ranks_per_thread() const {
  return ceil(float(kernel().mpi_manager.get_num_processes()) / n_threads_);
}

inline thread VPManager::get_start_rank_per_thread(const thread tid) const {
  return tid * get_num_assigned_ranks_per_thread();
}

inline thread VPManager::get_end_rank_per_thread(
    const thread tid, const thread rank_start,
...
inline AssignedRanks VPManager::get_assigned_ranks(const thread tid) {
  ...
  assigned_ranks.end = get_end_rank_per_thread(tid, assigned_ranks.begin,
                                               assigned_ranks.max_size);
