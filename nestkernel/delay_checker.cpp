...
nest::DelayChecker::DelayChecker()
    : min_delay_(Time::pos_inf()), max_delay_(Time::neg_inf()),
      user_set_delay_extrema_(false), freeze_delay_update_(false) {}

nest::DelayChecker::DelayChecker(const DelayChecker &cr)
    : min_delay_(cr.min_delay_), max_delay_(cr.max_delay_),
...
void nest::DelayChecker::get_status(DictionaryDatum &d) const {
  (*d)[names::min_delay] = get_min_delay().get_ms();
  (*d)[names::max_delay] = get_max_delay().get_ms();
}

void nest::DelayChecker::set_status(const DictionaryDatum &d) {
...
void nest::DelayChecker::assert_two_valid_delays_steps(delay new_delay1,
                                                       delay new_delay2) {
