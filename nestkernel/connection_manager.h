...
public:
   ...
   */
  void connect(const index sgid, Node *target, thread target_thread,
               const synindex syn_id, const DictionaryDatum &params,
   ...
   * \param syn_id The synapse model to use.
   */
  bool connect(const index sgid, const index target,
               const DictionaryDatum &params, const synindex syn_id);

  index find_connection(const thread tid, const synindex syn_id,
                        const index sgid, const index tgid);

  void disconnect(const thread tid, const synindex syn_id, const index sgid,
                  const index tgid);
   ...
   * AKA DataConnect
   */
  void data_connect_single(const index source_id, DictionaryDatum d,
                           const index syn);

  // aka conndatum GetStatus
  DictionaryDatum get_synapse_status(const index source_gid,
                                     const index target_gid, const thread tid,
                                     const synindex syn_id,
                                     const index lcid) const;

  // aka conndatum SetStatus
  void set_synapse_status(const index source_gid, const index target_gid,
                          const thread tid, const synindex syn_id,
                          const index lcid, const DictionaryDatum &dict);
  ...
  void get_connections(std::deque<ConnectionID> &connectome,
                       TokenArray const *source, TokenArray const *target,
                       synindex syn_id, long synapse_label) const;
  ...
  void get_sources(const std::vector<index> &targets, const index syn_id,
                   std::vector<std::vector<index>> &sources);

  void get_targets(const std::vector<index> &sources, const index syn_id,
  ...
  index get_target_gid(const thread tid, const synindex syn_id,
                       const index lcid) const;
  ...
  void send(const thread tid, const synindex syn_id, const index lcid,
  ...
  bool get_next_target_data(const thread tid, const thread rank_start,
                            const thread rank_end, thread &target_rank,
  ...
  void add_target(const thread tid, const thread target_rank,
  ...
  get_secondary_send_buffer_positions(const thread tid, const index lid,
  ...
  index get_source_gid(const thread tid, const synindex syn_id,
                       const index lcid);
...
private:
  ...
  void get_source_gids_(const thread tid, const synindex syn_id,
                        const index tgid, std::vector<index> &sources);
   ...
   */
  void connect_(Node &source, Node &target, const index s_gid, const thread tid,
                const synindex syn_id, const DictionaryDatum &params,
   ...
   * \param weight The weight of the connection (optional).
   */
  void connect_to_device_(Node &source, Node &target, const index s_gid,
                          const thread tid, const synindex syn_id,
                          const DictionaryDatum &params,
                          const double delay = NAN, const double weight = NAN);
   ...
   * \param weight The weight of the connection (optional).
   */
  void connect_from_device_(Node &source, Node &target, const thread tid,
                            const synindex syn_id,
                            const DictionaryDatum &params,
...
inline DictionaryDatum &ConnectionManager::get_connruledict() {
  return connruledict_;
}

inline delay ConnectionManager::get_min_delay() const { return min_delay_; }

inline delay ConnectionManager::get_max_delay() const { return max_delay_; }

inline void ConnectionManager::clean_source_table(const thread tid) {
  if (not keep_source_table_) {
    source_table_.clean(tid);
  }
}

inline void ConnectionManager::clear_source_table(const thread tid) {
  if (not keep_source_table_) {
    source_table_.clear(tid);
  }
}

inline bool ConnectionManager::get_keep_source_table() const {
  return keep_source_table_;
}

inline bool ConnectionManager::is_source_table_cleared() const {
...
inline void ConnectionManager::reject_last_target_data(const thread tid) {
  source_table_.reject_last_target_data(tid);
}

inline void ConnectionManager::save_source_table_entry_point(const thread tid) {
  source_table_.save_entry_point(tid);
}

inline void ConnectionManager::no_targets_to_process(const thread tid) {
...
inline void ConnectionManager::prepare_target_table(const thread tid) {
...
inline bool ConnectionManager::have_connections_changed() const {
...
inline void ConnectionManager::add_target(const thread tid,
                                          const thread target_rank,
                                          const TargetData &target_data) {
  target_table_.add_target(tid, target_rank, target_data);
}

inline bool ConnectionManager::get_next_target_data(
    const thread tid, const thread rank_start, const thread rank_end,
    thread &target_rank, TargetData &next_target_data) {
  return source_table_.get_next_target_data(tid, rank_start, rank_end,
                                            target_rank, next_target_data);
}

inline const std::vector<size_t> &
ConnectionManager::get_secondary_send_buffer_positions(
    const thread tid, const index lid, const synindex syn_id) const {
  return target_table_.get_secondary_send_buffer_positions(tid, lid, syn_id);
}

inline size_t ConnectionManager::get_secondary_recv_buffer_position(
    const thread tid, const synindex syn_id, const index lcid) const {
...
inline index ConnectionManager::get_source_gid(const thread tid,
                                               const synindex syn_index,
                                               const index lcid) {
  return source_table_.get_gid(tid, syn_index, lcid);
}

inline bool ConnectionManager::has_primary_connections() const {
  return has_primary_connections_;
}

inline bool ConnectionManager::secondary_connections_exist() const {
  return secondary_connections_exist_;
}

inline bool ConnectionManager::get_sort_connections_by_source() const {
  return sort_connections_by_source_;
}

inline double ConnectionManager::get_stdp_eps() const { return stdp_eps_; }

inline index ConnectionManager::get_target_gid(const thread tid,
                                               const synindex syn_id,
                                               const index lcid) const {
  return connections_[tid][syn_id]->get_target_gid(tid, lcid);
}

inline void ConnectionManager::send(const thread tid, const synindex syn_id,
                                    const index lcid,
...
inline void ConnectionManager::restructure_connection_tables(const thread tid) {
  assert(not source_table_.is_cleared());
  target_table_.clear(tid);
  source_table_.reset_processed_flags(tid);
}

inline void ConnectionManager::set_has_source_subsequent_targets(
    const thread tid, const synindex syn_id, const index lcid,
    const bool subsequent_targets) {
