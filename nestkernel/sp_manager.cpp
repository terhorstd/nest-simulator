...
SPManager::SPManager()
    : ManagerInterface(), structural_plasticity_update_interval_(1000),
      structural_plasticity_enabled_(false), sp_conn_builders_(),
      growthcurvedict_(new Dictionary()), growthcurve_factories_() {}

SPManager::~SPManager() { finalize(); }
 ...
 */
void SPManager::get_status(DictionaryDatum &d) {
  ...
  for (std::vector<SPBuilder *>::const_iterator i = sp_conn_builders_.begin();
    ...
    def<std::string>(sp_synapse, names::pre_synaptic_element,
                     (*i)->get_pre_synaptic_element_name());
    def<std::string>(sp_synapse, names::post_synaptic_element,
                     (*i)->get_post_synaptic_element_name());
    def<std::string>(
        sp_synapse, names::model,
        kernel()
  ...
  def<long>(d, names::structural_plasticity_update_interval,
 ...
 */
void SPManager::set_status(const DictionaryDatum &d) {
  if (d->known(names::structural_plasticity_update_interval)) {
    updateValue<long>(d, names::structural_plasticity_update_interval,
                      structural_plasticity_update_interval_);
  }
  ...
  sp_conn_builders_.clear();
  updateValue<DictionaryDatum>(d, names::structural_plasticity_synapses,
                               syn_specs);
  for (Dictionary::const_iterator i = syn_specs->begin(); i != syn_specs->end();
       ++i) {
    ...
    if (not conn_builder->get_default_delay() and
        not kernel().connection_manager.get_user_set_delay_extrema()) {
      throw BadProperty(
          "Structural Plasticity: to use different delays for synapses you "
          "must "
...
delay SPManager::builder_min_delay() const {
...
delay SPManager::builder_max_delay() const {
 ...
 */
void SPManager::disconnect_single(index sgid, Node *target,
                                  thread target_thread, DictionaryDatum &syn) {
  if (kernel().connection_manager.have_connections_changed()) {
    if (kernel().connection_manager.secondary_connections_exist()) {
      kernel()
          .model_manager
          .create_secondary_events_prototypes(); // necessary before
                                                 // updating
                                                 // connection
                                                 // infrastructure
    }
#pragma omp parallel
  ...
  if (syn->known(names::pre_synaptic_element) and
    ...
    cb->change_connected_synaptic_elements(sgid, target->get_gid(),
                                           target->get_thread(), -1);
  }
  const std::string syn_name = (*syn)[names::model];
  disconnect(sgid, target, target_thread,
 ...
 * @param syn_id
 */
void SPManager::disconnect(const index sgid, Node *target, thread target_thread,
                           const index syn_id) {
  Node *const source = kernel().node_manager.get_node(sgid);
  // normal nodes and devices with proxies
  if (target->has_proxies()) {
    kernel().connection_manager.disconnect(target_thread, syn_id, sgid,
                                           target->get_gid());
 ...
 */
void SPManager::disconnect(GIDCollection &sources, GIDCollection &targets,
...
#pragma omp parallel
  ...
  if (not sp_conn_builders_
              .empty()) { // Implement a getter for sp_conn_builders_
...
void SPManager::update_structural_plasticity() {
  for (std::vector<SPBuilder *>::const_iterator i = sp_conn_builders_.begin();
       i != sp_conn_builders_.end(); ++i) {
 ...
 */
void SPManager::update_structural_plasticity(SPBuilder *sp_builder) {
  ...
  get_synaptic_elements(sp_builder->get_pre_synaptic_element_name(),
                        pre_vacant_id, pre_vacant_n, pre_deleted_id,
                        pre_deleted_n);

  // Communicate the number of deleted pre-synaptic elements
  kernel().mpi_manager.communicate(pre_deleted_id, pre_deleted_id_global,
                                   displacements);
  kernel().mpi_manager.communicate(pre_deleted_n, pre_deleted_n_global,
                                   displacements);

  if (pre_deleted_id_global.size() > 0) {
    delete_synapses_from_pre(pre_deleted_id_global, pre_deleted_n_global,
  ...
  get_synaptic_elements(sp_builder->get_post_synaptic_element_name(),
                        post_vacant_id, post_vacant_n, post_deleted_id,
                        post_deleted_n);
  // Communicate the number of deleted post-synaptic elements
  kernel().mpi_manager.communicate(post_deleted_id, post_deleted_id_global,
                                   displacements);
  kernel().mpi_manager.communicate(post_deleted_n, post_deleted_n_global,
                                   displacements);

  if (post_deleted_id_global.size() > 0) {
    delete_synapses_from_post(post_deleted_id_global, post_deleted_n_global,
                              sp_builder->get_synapse_model(),
                              sp_builder->get_pre_synaptic_element_name(),
                              sp_builder->get_post_synaptic_element_name());
    get_synaptic_elements(sp_builder->get_pre_synaptic_element_name(),
  ...
  kernel().mpi_manager.communicate(pre_vacant_id, pre_vacant_id_global,
                                   displacements);
  kernel().mpi_manager.communicate(pre_vacant_n, pre_vacant_n_global,
                                   displacements);
  kernel().mpi_manager.communicate(post_vacant_id, post_vacant_id_global,
                                   displacements);
  kernel().mpi_manager.communicate(post_vacant_n, post_vacant_n_global,
                                   displacements);

  if (pre_vacant_id_global.size() > 0 and post_vacant_id_global.size() > 0) {
    create_synapses(pre_vacant_id_global, pre_vacant_n_global,
                    post_vacant_id_global, post_vacant_n_global, sp_builder);
  }
}
 ...
 */
void SPManager::create_synapses(std::vector<index> &pre_id,
 ...
 */
void SPManager::delete_synapses_from_pre(
    const std::vector<index> &pre_deleted_id, std::vector<int> &pre_deleted_n,
    const index synapse_model, const std::string &se_pre_name,
  ...
  kernel().connection_manager.get_targets(pre_deleted_id, synapse_model,
                                          se_post_name, connectivity);
  ...
  for (; id_it != pre_deleted_id.end() and n_it != pre_deleted_n.end();
    ...
    kernel().mpi_manager.communicate(*connectivity_it, global_targets,
                                     displacements);
    ...
    {
      delete_synapse(*id_it, global_targets[i], synapse_model, se_pre_name,
                     se_post_name);
    }
  }
}
 ...
 */
void SPManager::delete_synapse(const index sgid, const index tgid,
                               const long syn_id, const std::string se_pre_name,
                               const std::string se_post_name) {
  // get thread id
 ...
 * @param se_post_name post synaptic element name
 */
void SPManager::delete_synapses_from_post(std::vector<index> &post_deleted_id,
  ...
  kernel().connection_manager.get_sources(post_deleted_id, synapse_model,
                                          connectivity);
  ...
  for (; id_it != post_deleted_id.end() and n_it != post_deleted_n.end();
    ...
    kernel().mpi_manager.communicate(*connectivity_it, global_sources,
                                     displacements);
    // shuffle only the first n items, n is the number of deleted synaptic
    // elements
    ...
    for (int i = 0; i < -(*n_it); i++) // n is negative
    {
      delete_synapse(global_sources[i], *id_it, synapse_model, se_pre_name,
                     se_post_name);
    }
  }
}
...
void nest::SPManager::serialize_id(std::vector<index> &id, std::vector<int> &n,
...
void nest::SPManager::global_shuffle(std::vector<index> &v) {
 ...
 */
void nest::SPManager::global_shuffle(std::vector<index> &v, size_t n) {
  assert(n <= v.size());
...
void nest::SPManager::enable_structural_plasticity() {
  ...
  if (not kernel().connection_manager.get_keep_source_table()) {
    throw KernelException("Structural plasticity can not be enabled if "
                          "keep_source_table has been "
                          "set to false.");
  }
