...
nest::SimulationManager::SimulationManager()
    : clock_(Time::tic(0L)), slice_(0L), to_do_(0L), to_do_total_(0L),
      from_step_(0L), to_step_(0L) // consistent with to_do_ == 0
      ,
      t_real_(0L), simulating_(false), simulated_(false),
      exit_on_user_signal_(false), inconsistent_state_(false),
      print_time_(false), use_wfr_(true), wfr_comm_interval_(1.0),
      wfr_tol_(0.0001), wfr_max_iterations_(15), wfr_interpolation_order_(3) {}

void nest::SimulationManager::initialize() {
...
void nest::SimulationManager::finalize() {
...
void nest::SimulationManager::set_status(const DictionaryDatum &d) {
  ...
  if (updateValue<double>(d, names::time, time)) {
    ...
    if (clock_ > TimeZero) {
      ...
      LOG(M_WARNING, "SimulationManager::set_status",
          "Simulation time reset to t=0.0. Resetting the simulation time is "
          "not "
  ...
  if (tics_per_ms_updated or res_updated) {
    ...
    {
      LOG(M_ERROR, "SimulationManager::set_status",
                                                    ...
                                                    // resolution
    {
      if (resd < 1.0 / tics_per_ms) {
        ...
        LOG(M_ERROR, "SimulationManager::set_status",
            "Resolution must be a multiple of the tic length. Value "
            "unchanged.");
        throw KernelException();
      } else {
        ...
        LOG(M_INFO, "SimulationManager::set_status",
        ...
        if (wfr_comm_interval_ < Time::get_resolution().get_ms() or
            not use_wfr_) {
  ...
  if (updateValue<double>(d, names::wfr_comm_interval, wfr_interval)) {
    if (not use_wfr_) {
      ...
      LOG(M_ERROR, "SimulationManager::set_status",
          "Cannot change waveform communication interval after connections "
          "have "
          "been created. Please call ResetKernel first.");
      throw KernelException();
    } else if (wfr_interval < Time::get_resolution().get_ms()) {
      LOG(M_ERROR, "SimulationManager::set_status",
          "Communication interval of the waveform relaxation must be greater "
          "or "
...
void nest::SimulationManager::get_status(DictionaryDatum &d) {
...
void nest::SimulationManager::prepare() {
  ...
  if (inconsistent_state_) {
    throw KernelException("Kernel is in inconsistent state after an "
  ...
  if (kernel().mpi_manager.get_num_processes() > 1) {
    if (not kernel().mpi_manager.grng_synchrony(
      ...
      LOG(M_ERROR, "SimulationManager::prepare",
  ...
  if (kernel().node_manager.have_nodes_changed() or
...
void nest::SimulationManager::simulate(Time const &t) {
  prepare();
  run(t);
  cleanup();
}
...
void nest::SimulationManager::assert_valid_simtime(Time const &t) {
  ...
  if (t < Time::step(1)) {
    LOG(M_ERROR, "SimulationManager::run",
  ...
  if (t.is_finite()) {
    ...
    if (not time1.is_finite()) {
      std::string msg = String::compose(
          ...
          "clock first!",
          (Time::max() - clock_).get_ms(), t.get_ms());
      LOG(M_ERROR, "SimulationManager::run", msg);
      throw KernelException();
    }
  } else {
    std::string msg =
        String::compose("The requested simulation time exceeds the largest "
                        "time NEST can handle "
...
void nest::SimulationManager::run(Time const &t) {
  ...
  if (t.get_steps() % kernel().connection_manager.get_min_delay() != 0) {
    LOG(M_WARNING, "SimulationManager::run",
        "The requested simulation time is not an integer multiple of the "
        "minimal "
        "delay in the network. This may result in inconsistent results under "
        "the "
...
void nest::SimulationManager::cleanup() {
  ...
  if (kernel().mpi_manager.get_num_processes() > 1) {
    if (not kernel().mpi_manager.grng_synchrony(
            kernel().rng_manager.get_grng()->ulrand(100000))) {
      throw KernelException("In SimulationManager::cleanup(): "
...
void nest::SimulationManager::update_connection_infrastructure(
    const thread tid) {
...
#pragma omp parallel
  {
    ...
    do {
      ...
      if (kernel().sp_manager.is_structural_plasticity_enabled() and
        ...
        for (std::vector<Node *>::const_iterator i =
             ...
             i != kernel().node_manager.get_nodes_on_thread(tid).end(); ++i) {
...
#pragma omp master
        {
          ...
          kernel().music_manager.update_music_event_handlers(clock_, from_step_,
                                                             to_step_);
        }
...
#pragma omp barrier
      ...
      // wfr
      if (kernel().connection_manager.secondary_connections_exist() and
          kernel().node_manager.wfr_is_used()) {
#pragma omp single
        {
        ...
        for (long n = 0; n < wfr_max_iterations_; ++n) {
          ...
          for (std::vector<Node *>::const_iterator i =
               ...
               i != thread_local_wfr_nodes.end(); ++i) {
...
#pragma omp master
      {
        ...
        if (SLIsignalflag != 0) {
          LOG(M_INFO, "SimulationManager::update",
...
#pragma omp barrier
    ...
    } while (to_do_ > 0 and not exit_on_user_signal_ and
             not exceptions_raised.at(tid));
...
void nest::SimulationManager::reset_network() {
  ...
  LOG(M_WARNING, "SimulationManager::ResetNetwork",
...
nest::Time const nest::SimulationManager::get_previous_slice_origin() const {
