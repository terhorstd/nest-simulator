...
nest::RecordingDevice::Parameters_::Parameters_(const std::string &file_ext,
                                                ...
                                                bool withweight,
                                                bool withtargetgid,
                                                bool withport, bool withrport)
    : to_file_(false), to_screen_(false), to_memory_(true),
      to_accumulator_(false), time_in_steps_(false), precise_times_(false),
      withgid_(withgid), withtime_(withtime), withweight_(withweight),
      withtargetgid_(withtargetgid), withport_(withport), withrport_(withrport),
      precision_(3), scientific_(false), user_set_precise_times_(false),
      user_set_precision_(false), binary_(false),
      fbuffer_size_(-1) // -1 marks use of default buffer
      ,
      label_(), file_ext_(file_ext), filename_(), close_after_simulate_(false),
      flush_after_simulate_(true), flush_records_(false), close_on_reset_(true),
      use_gid_in_filename_(true) {}

nest::RecordingDevice::State_::State_()
    : events_(0), event_senders_(), event_targets_(), event_ports_(),
      event_rports_(), event_times_ms_(), event_times_steps_(),
...
void nest::RecordingDevice::Parameters_::set(const RecordingDevice &rd,
  ...
  if (rd.mode_ == RecordingDevice::SPIKE_DETECTOR or
      rd.mode_ == RecordingDevice::WEIGHT_RECORDER or
      rd.mode_ == RecordingDevice::SPIN_DETECTOR) {
  ...
  if (updateValue<long>(d, names::fbuffer_size, fbuffer_size)) {
    ...
    if (fbuffer_size < 0 and
        not(rd.node_.get_vp() == -1 and fbuffer_size == -1)) {
  ...
  if (rd.mode_ == RecordingDevice::MULTIMETER) {
    rec_change = updateValue<bool>(d, names::to_accumulator, to_accumulator_) or
                 rec_change;
  }
  ...
  if (have_record_to) {
    ...
    for (Token *t = ad.begin(); t != ad.end(); ++t) {
      if (*t == LiteralDatum(names::file) or
          *t == Token(names::file.toString())) {
        to_file_ = true;
      } else if (*t == LiteralDatum(names::memory) or
                 *t == Token(names::memory.toString())) {
        to_memory_ = true;
      } else if (*t == LiteralDatum(names::screen) or
                 *t == Token(names::screen.toString())) {
        to_screen_ = true;
      } else if (rd.mode_ == RecordingDevice::MULTIMETER and
                 (*t == LiteralDatum(names::accumulator) or
  ...
  if ((rec_change or have_record_to) and to_file_ and to_memory_) {
    LOG(M_INFO, "RecordingDevice::set_status",
        "Data will be recorded to file and to memory.");
  }

  if (to_accumulator_ and
      (to_file_ or to_screen_ or to_memory_ or withgid_ or withweight_)) {
    to_file_ = to_screen_ = to_memory_ = withgid_ = withweight_ = false;
    LOG(M_WARNING, "RecordingDevice::set_status()",
  ...
  if (p.withtime_) {
    if (p.time_in_steps_) {
      ...
      if (not p.to_accumulator_) {
        append_property(dict, names::times,
                        std::vector<long>(event_times_steps_));
      } else {
        provide_property(dict, names::times,
                         std::vector<long>(event_times_steps_));
      }
      ...
      if (p.precise_times_) {
        ...
        if (not p.to_accumulator_) {
          append_property(dict, names::offsets,
                          std::vector<double>(event_times_offsets_));
        } else {
          provide_property(dict, names::offsets,
                           std::vector<double>(event_times_offsets_));
        }
      }
    } else {
      ...
      if (not p.to_accumulator_) {
        append_property(dict, names::times,
                        std::vector<double>(event_times_ms_));
      } else {
        provide_property(dict, names::times,
                         std::vector<double>(event_times_ms_));
      }
...
nest::RecordingDevice::Buffers_::Buffers_()
    : fs_(), fbuffer_(0), fbuffer_size_(-1) {}
...
nest::RecordingDevice::RecordingDevice(const Node &n, Mode mode,
                                       const std::string &file_ext,
                                       bool withtime, bool withgid,
                                       bool withweight, bool withtargetgid,
                                       bool withport, bool withrport)
    : Device(), node_(n), mode_(mode),
      P_(file_ext, withtime, withgid, withweight, withtargetgid, withport,
         withrport),
      S_(), B_() {}

nest::RecordingDevice::RecordingDevice(const Node &n, const RecordingDevice &d)
    : Device(d), node_(n), mode_(d.mode_), P_(d.P_), S_(d.S_),
      B_() // do not copy
{}
  ...
  if (P_.to_file_) {
    ...
    if (newfile) {
      ...
      if (P_.fbuffer_size_ >= 0) {
        ...
        if (res == 0) {
          LOG(M_ERROR, "RecordingDevice::calibrate()",
      ...
      if (kernel().io_manager.overwrite_files()) {
        ...
        if (test.good()) {
          std::string msg = String::compose(
              "The device file '%1' exists already and will not be "
              "overwritten. "
...
void nest::RecordingDevice::post_run_cleanup() {
  if (B_.fs_.is_open()) {
...
void nest::RecordingDevice::print_time_(std::ostream &os, const Time &t,
                                        double offs) {
  if (not P_.withtime_) {
    return;
  }
...
void nest::RecordingDevice::print_target_(std::ostream &os, index gid) {
  if (P_.withtargetgid_) {
    os << gid << '\t';
  }
}

void nest::RecordingDevice::print_port_(std::ostream &os, long port) {
  if (P_.withport_) {
    os << port << '\t';
  }
}

void nest::RecordingDevice::print_rport_(std::ostream &os, long rport) {
  if (P_.withrport_) {
    os << rport << '\t';
  }
}

void nest::RecordingDevice::store_data_(index sender, const Time &t,
                                        double offs, double weight,
                                        index target, long port, long rport) {
  if (P_.withgid_) {
...
const std::string nest::RecordingDevice::build_filename_() const {
  ...
  const int gidigits = static_cast<int>(
      std::floor(std::log10(static_cast<float>(kernel().node_manager.size()))) +
      1);
