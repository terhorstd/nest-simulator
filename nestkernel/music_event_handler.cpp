...
MusicEventHandler::MusicEventHandler()
    : music_port_(0), music_perm_ind_(0), published_(false), portname_(""),
      acceptable_latency_(0.0), max_buffered_(-1) {}

MusicEventHandler::MusicEventHandler(std::string portname,
                                     double acceptable_latency,
                                     int max_buffered)
    : music_port_(0), music_perm_ind_(0), published_(false),
      portname_(portname), acceptable_latency_(acceptable_latency),
...
void MusicEventHandler::publish_port() {
  if (not published_) {
    ...
    if (channelmap_.size() > music_port_width) {
      throw MUSICChannelUnknown("MusicEventHandler", portname_,
                                channelmap_.size() - 1);
    }
...
void MusicEventHandler::update(Time const &origin, const long from,
                               const long to) {
  for (size_t channel = 0; channel < channelmap_.size(); ++channel) {
    if (channelmap_[channel] != 0) {
      while (not eventqueue_[channel].empty()) {
        ...
        if (T > origin + Time::step(from) - Time::ms(acceptable_latency_) and
