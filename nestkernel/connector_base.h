...
public:
   ...
   * dict.
   */
  virtual void get_synapse_status(const thread tid, const index lcid,
   ...
   * dictionary dict.
   */
  virtual void set_synapse_status(const index lcid, const DictionaryDatum &dict,
                                  ConnectorModel &cm) = 0;
   ...
   */
  virtual void get_connection(const index source_gid, const index target_gid,
                              const thread tid, const index lcid,
   ...
   */
  virtual void get_connection_with_specified_targets(
      const index source_gid, const std::vector<size_t> &target_neuron_gids,
      const thread tid, const index lcid, const long synapse_label,
      std::deque<ConnectionID> &conns) const = 0;
   ...
   */
  virtual void get_source_lcids(const thread tid, const index target_gid,
   ...
   */
  virtual void get_target_gids(const thread tid, const index start_lcid,
   ...
   * source.
   */
  virtual index send(const thread tid, const index lcid,
                     const std::vector<ConnectorModel *> &cm, Event &e) = 0;

  virtual void send_weight_event(const thread tid, const unsigned int lcid,
                                 Event &e,
   ...
   */
  virtual void
  trigger_update_weight(const long vt_gid, const thread tid,
   ...
   */
  virtual index find_first_target(const thread tid, const index start_lcid,
 ...
 */
template <typename ConnectionT> class Connector : public ConnectorBase {
private:
  BlockVector<ConnectionT> C_;
  const synindex syn_id_;

public:
  explicit Connector(const synindex syn_id) : syn_id_(syn_id) {}

  ~Connector() { C_.clear(); }

  synindex get_syn_id() const { return syn_id_; }

  size_t size() const { return C_.size(); }

  void get_synapse_status(const thread tid, const index lcid,
  ...
  void set_synapse_status(const index lcid, const DictionaryDatum &dict,
                          ConnectorModel &cm) {
    assert(lcid < C_.size());

    C_[lcid].set_status(dict,
                        static_cast<GenericConnectorModel<ConnectionT> &>(cm));
  }
  ...
  void get_connection(const index source_gid, const index target_gid,
                      const thread tid, const index lcid,
                      const long synapse_label,
                      std::deque<ConnectionID> &conns) const {
    if (not C_[lcid].is_disabled()) {
      if (synapse_label == UNLABELED_CONNECTION or
  ...
  void get_connection_with_specified_targets(
      const index source_gid, const std::vector<size_t> &target_neuron_gids,
      const thread tid, const index lcid, const long synapse_label,
      std::deque<ConnectionID> &conns) const {
    if (not C_[lcid].is_disabled()) {
      if (synapse_label == UNLABELED_CONNECTION or
          C_[lcid].get_label() == synapse_label) {
        const index current_target_gid = C_[lcid].get_target(tid)->get_gid();
        if (std::find(target_neuron_gids.begin(), target_neuron_gids.end(),
  ...
  void get_all_connections(const index source_gid, const index target_gid,
                           const thread tid, const long synapse_label,
                           std::deque<ConnectionID> &conns) const {
  ...
  void get_source_lcids(const thread tid, const index target_gid,
  ...
  void get_target_gids(const thread tid, const index start_lcid,
                       const std::string &post_synaptic_element,
                       std::vector<index> &target_gids) const {
    index lcid = start_lcid;
    while (true) {
      if (C_[lcid].get_target(tid)->get_synaptic_elements(
              post_synaptic_element) != 0.0 and
          not C_[lcid].is_disabled()) {
  ...
  index get_target_gid(const thread tid, const unsigned int lcid) const {
    return C_[lcid].get_target(tid)->get_gid();
  }

  void send_to_all(const thread tid, const std::vector<ConnectorModel *> &cm,
                   Event &e) {
    for (size_t lcid = 0; lcid < C_.size(); ++lcid) {
      e.set_port(lcid);
  ...
  index send(const thread tid, const index lcid,
  ...
  void send_weight_event(const thread tid, const unsigned int lcid, Event &e,
                         const CommonSynapseProperties &cp);

  void trigger_update_weight(const long vt_gid, const thread tid,
                             const std::vector<spikecounter> &dopa_spikes,
                             const double t_trig,
                             const std::vector<ConnectorModel *> &cm) {
    for (size_t i = 0; i < C_.size(); ++i) {
      if (static_cast<GenericConnectorModel<ConnectionT> *>(cm[syn_id_])
              ->get_common_properties()
              .get_vt_gid() == vt_gid) {
        C_[i].trigger_update_weight(
            tid, dopa_spikes, t_trig,
            static_cast<GenericConnectorModel<ConnectionT> *>(cm[syn_id_])
                ->get_common_properties());
      }
    }
  ...
  void sort_connections(BlockVector<Source> &sources) {
    nest::sort(sources, C_);
  }

  void set_has_source_subsequent_targets(const index lcid,
                                         const bool has_subsequent_targets) {
    C_[lcid].set_has_source_subsequent_targets(has_subsequent_targets);
  }

  index find_first_target(const thread tid, const index start_lcid,
                          const index target_gid) const {
    index lcid = start_lcid;
    while (true) {
      if (C_[lcid].get_target(tid)->get_gid() == target_gid and
          not C_[lcid].is_disabled()) {
        return lcid;
      }
  ...
  index find_matching_target(const thread tid,
                             const std::vector<index> &matching_lcids,
                             const index target_gid) const {
  ...
  void disable_connection(const index lcid) {
    assert(not C_[lcid].is_disabled());
    C_[lcid].disable();
  }

  void remove_disabled_connections(const index first_disabled_index) {
