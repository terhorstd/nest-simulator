...
void nest::VPManager::initialize() {
...
void nest::VPManager::finalize() {}

void nest::VPManager::set_status(const DictionaryDatum &d) {
  ...
  if (n_threads_updated) {
    ...
    if (n_threads > 1 and force_singlethreading_) {
      LOG(M_WARNING, "VPManager::set_status",
...
void nest::VPManager::assert_single_threaded() {
#ifdef _OPENMP
