...
NodeManager::NodeManager()
    : local_nodes_(), root_(0), current_(0), siblingcontainer_model_(0),
      nodes_vec_(), wfr_nodes_vec_(), wfr_is_used_(false),
...
void NodeManager::finalize() { destruct_nodes_(); }

void NodeManager::reinit_nodes() {
...
DictionaryDatum NodeManager::get_status(index idx) {
  assert(idx != 0);
...
index NodeManager::add_node(index mod, long n) // no_p
{
  ...
  if (max_gid > local_nodes_.max_size() or max_gid < min_gid) {
    LOG(M_ERROR, "NodeManager::add:node",
  ...
  if (model->has_proxies()) {
    ...
    for (index gid = min_gid; gid < max_gid; ++gid) {
      ...
      container->set_model_id(-1);   // mark as pseudo-container wrapping
                                     // replicas, see reset_network()
  ...
  if (model->is_off_grid()) {
    ...
    LOG(M_INFO, "NodeManager::add_node",
...
void NodeManager::restore_nodes(const ArrayDatum &node_list) {
  Subnet *root = get_cwn();
...
void NodeManager::init_state(index GID) {
  Node *n = get_node(GID);
  if (n == 0) {
...
bool NodeManager::is_local_node(Node *n) const {
  return kernel().vp_manager.is_local_vp(n->get_vp());
}

inline index NodeManager::next_local_gid_(index curr_gid) const {
...
index NodeManager::get_max_num_local_nodes() const {
  return static_cast<index>(
      ceil(static_cast<double>(size()) /
           kernel().vp_manager.get_num_virtual_processes()));
}

index NodeManager::get_num_local_devices() const { return num_local_devices_; }

void NodeManager::go_to(index n) {
...
const SiblingContainer *NodeManager::get_thread_siblings(index n) const {
  Node *node = local_nodes_.get_node_by_gid(n);
  if (node->num_thread_siblings() == 0) {
...
void NodeManager::ensure_valid_thread_local_ids() {
    ...
    if (size() != nodes_vec_network_size_) {
      ...
      for (thread tid = 0; tid < kernel().vp_manager.get_num_threads(); ++tid) {
        ...
        for (size_t idx = 1; idx < local_nodes_.size(); ++idx) {
          ...
          if (not node->is_subnet() and
              (node->get_thread() == tid or node->num_thread_siblings() > 0)) {
...
void NodeManager::destruct_nodes_() {
...
void NodeManager::set_status_single_node_(Node &target,
  ...
  if (not target.is_proxy()) {
    ...
    ALL_ENTRIES_ACCESSED(*d, "NodeManager::set_status",
                         "Unread dictionary entries: ");
  }
}

void NodeManager::prepare_node_(Node *n) {
...
void NodeManager::prepare_nodes() {
...
void NodeManager::print(index p, int depth) {
...
void NodeManager::set_status(index gid, const DictionaryDatum &d) {
...
void NodeManager::set_status(const DictionaryDatum &d) {
  std::string tmp;
...
void NodeManager::reset_nodes_state() {
  /* Reinitialize state on all nodes, force init_buffers() on next
     call to simulate().
     Finding all nodes is non-trivial:
     - We iterate over local nodes only.
