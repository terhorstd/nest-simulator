...
void reset_network() {
  ...
  LOG(M_INFO, "ResetNetworkFunction",
...
void print_network(index gid, index depth, std::ostream &) {
...
librandom::RngPtr get_global_rng() { return kernel().rng_manager.get_grng(); }
...
void set_connection_status(const ConnectionDatum &conn,
  ...
  kernel().connection_manager.set_synapse_status(source_gid, target_gid, tid,
                                                 syn_id, p, dict);

  ALL_ENTRIES_ACCESSED2(
      *dict, "SetStatus", "Unread dictionary entries: ",
...
index create(const Name &model_name, const index n_nodes) {
  if (n_nodes == 0) {
    throw RangeCheck();
  }
...
void simulate(const double &time) {
  ...
  if (not t_sim.is_grid_time()) {
    throw BadParameter("The simulation time must be a multiple "
...
void prepare() { kernel().simulation_manager.prepare(); }

void cleanup() { kernel().simulation_manager.cleanup(); }
...
ArrayDatum get_nodes(const index node_id, const DictionaryDatum &params,
  ...
  if (params->empty()) {
    ...
    kernel().mpi_manager.communicate(localnodes, globalnodes, params,
                                     include_remotes);
  }
...
ArrayDatum get_leaves(const index node_id, const DictionaryDatum &params,
...
ArrayDatum get_children(const index node_id, const DictionaryDatum &params,
                        const bool include_remotes) {
  Subnet *subnet =
      dynamic_cast<Subnet *>(kernel().node_manager.get_node(node_id));
  if (subnet == NULL) {
