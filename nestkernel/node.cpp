...
Node::Node()
    : gid_(0), lid_(0), thread_lid_(invalid_index), model_id_(-1), parent_(0),
      thread_(0), vp_(invalid_thread_), frozen_(false),
      buffers_initialized_(false), node_uses_wfr_(false) {}

Node::Node(const Node &n)
    : gid_(0), lid_(0), thread_lid_(n.thread_lid_), model_id_(n.model_id_),
      parent_(n.parent_), thread_(n.thread_), vp_(n.vp_), frozen_(n.frozen_)
      // copy must always initialized its own buffers
      ,
      buffers_initialized_(false), node_uses_wfr_(n.node_uses_wfr_) {}
...
void Node::set_status_base(const DictionaryDatum &dict) {
  ...
  try {
    ...
    throw BadProperty(
        String::compose("Setting status of a '%1' with GID %2: %3", get_name(),
                        get_gid(), e.message()));
  }
 ...
 */
bool Node::wfr_update(Time const &, const long, const long) {
 ...
 */
void Node::register_stdp_connection(double) { throw IllegalConnection(); }
 ...
 */
void Node::handle(SpikeEvent &) { throw UnexpectedEvent(); }
...
port Node::handles_test_event(WeightRecorderEvent &, rport) {
  throw IllegalConnection();
}

void Node::handle(RateEvent &) { throw UnexpectedEvent(); }

port Node::handles_test_event(RateEvent &, rport) { throw IllegalConnection(); }
...
void Node::handle(DataLoggingRequest &) { throw UnexpectedEvent(); }
...
port Node::handles_test_event(GapJunctionEvent &, rport) {
  throw IllegalConnection();
  return invalid_port_;
}

void Node::sends_secondary_event(GapJunctionEvent &) {
  throw IllegalConnection();
}

void Node::handle(InstantaneousRateConnectionEvent &) {
  throw UnexpectedEvent();
}
...
port Node::handles_test_event(InstantaneousRateConnectionEvent &, rport) {
  throw IllegalConnection();
  return invalid_port_;
}

port Node::handles_test_event(DiffusionConnectionEvent &, rport) {
  throw IllegalConnection();
  return invalid_port_;
}

port Node::handles_test_event(DelayedRateConnectionEvent &, rport) {
  throw IllegalConnection();
  return invalid_port_;
}

void Node::sends_secondary_event(InstantaneousRateConnectionEvent &) {
  throw IllegalConnection();
}

void Node::sends_secondary_event(DiffusionConnectionEvent &) {
  throw IllegalConnection();
}

void Node::sends_secondary_event(DelayedRateConnectionEvent &) {
  throw IllegalConnection();
}

double Node::get_LTD_value(double) { throw UnexpectedEvent(); }

double Node::get_K_value(double) { throw UnexpectedEvent(); }

void Node::get_K_values(double, double &, double &) { throw UnexpectedEvent(); }

void nest::Node::get_history(double, double, std::deque<histentry>::iterator *,
                             std::deque<histentry>::iterator *) {
  throw UnexpectedEvent();
}

void nest::Node::get_LTP_history(double, double,
...
void Node::event_hook(DSCurrentEvent &e) { e.get_receiver().handle(e); }

bool Node::is_subnet() const { return false; }
