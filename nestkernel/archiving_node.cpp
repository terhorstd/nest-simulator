...
nest::Archiving_Node::Archiving_Node()
    : n_incoming_(0), Kminus_(0.0), triplet_Kminus_(0.0), tau_minus_(20.0),
      tau_minus_inv_(1. / tau_minus_), tau_minus_triplet_(110.0),
      tau_minus_triplet_inv_(1. / tau_minus_triplet_), last_spike_(-1.0),
      Ca_t_(0.0), Ca_minus_(0.0), tau_Ca_(10000.0), beta_Ca_(0.001),
      synaptic_elements_map_() {}

nest::Archiving_Node::Archiving_Node(const Archiving_Node &n)
    : Node(n), n_incoming_(n.n_incoming_), Kminus_(n.Kminus_),
      triplet_Kminus_(n.triplet_Kminus_), tau_minus_(n.tau_minus_),
      tau_minus_inv_(n.tau_minus_inv_),
      tau_minus_triplet_(n.tau_minus_triplet_),
      tau_minus_triplet_inv_(n.tau_minus_triplet_inv_),
      last_spike_(n.last_spike_), Ca_t_(n.Ca_t_), Ca_minus_(n.Ca_minus_),
      tau_Ca_(n.tau_Ca_), beta_Ca_(n.beta_Ca_),
      synaptic_elements_map_(n.synaptic_elements_map_) {}

void Archiving_Node::register_stdp_connection(double t_first_read) {
...
double nest::Archiving_Node::get_K_value(double t) {
  if (history_.empty()) {
    return Kminus_;
  }
...
void nest::Archiving_Node::get_K_values(double t, double &K_value,
...
void nest::Archiving_Node::get_history(
    double t1, double t2, std::deque<histentry>::iterator *start,
...
void nest::Archiving_Node::set_spiketime(Time const &t_sp, double offset) {
  ...
  if (n_incoming_) {
    ...
    triplet_Kminus_ = triplet_Kminus_ * std::exp((last_spike_ - t_sp_ms) *
...
void nest::Archiving_Node::get_status(DictionaryDatum &d) const {
  ...
  for (std::map<Name, SynapticElement>::const_iterator it =
       ...
       it != synaptic_elements_map_.end(); ++it) {
...
void nest::Archiving_Node::set_status(const DictionaryDatum &d) {
  ...
  if (new_beta_Ca <= 0.0) {
    throw BadProperty("For Ca to function as an integrator of the electrical "
                      "activity, beta_ca "
 ...
 * ---------------------------------------------------------------- */
double nest::Archiving_Node::get_synaptic_elements(Name n) const {
...
int nest::Archiving_Node::get_synaptic_elements_vacant(Name n) const {
  std::map<Name, SynapticElement>::const_iterator se_it;
  se_it = synaptic_elements_map_.find(n);
...
int nest::Archiving_Node::get_synaptic_elements_connected(Name n) const {
  std::map<Name, SynapticElement>::const_iterator se_it;
  se_it = synaptic_elements_map_.find(n);
...
std::map<Name, double> nest::Archiving_Node::get_synaptic_elements() const {
  ...
  for (std::map<Name, SynapticElement>::const_iterator it =
       ...
       it != synaptic_elements_map_.end(); ++it) {
    n_map.insert(
        std::pair<Name, double>(it->first, get_synaptic_elements(it->first)));
  }
  return n_map;
}

void nest::Archiving_Node::update_synaptic_elements(double t) {
  assert(t >= Ca_t_);
...
void nest::Archiving_Node::decay_synaptic_elements_vacant() {
  for (std::map<Name, SynapticElement>::iterator it =
           synaptic_elements_map_.begin();
       it != synaptic_elements_map_.end(); ++it) {
...
void nest::Archiving_Node::connect_synaptic_element(Name name, int n) {
