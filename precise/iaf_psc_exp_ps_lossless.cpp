...
template <> void RecordablesMap<iaf_psc_exp_ps_lossless>::create() {
...
nest::iaf_psc_exp_ps_lossless::State_::State_()
    : y0_(0.0), I_syn_ex_(0.0), I_syn_in_(0.0), y2_(0.0), is_refractory_(false),
...
void nest::iaf_psc_exp_ps_lossless::State_::set(const DictionaryDatum &d,
                                                const Parameters_ &p,
                                                double delta_EL) {
...
void nest::iaf_psc_exp_ps_lossless::update(const Time &origin, const long from,
                                           const long to) {
  assert(to >= 0);
...
void nest::iaf_psc_exp_ps_lossless::emit_spike_(const Time &origin,
                                                const long lag, const double t0,
                                                const double dt) {
  // dt == 0 may occur if two spikes arrive simultaneously;
...
void nest::iaf_psc_exp_ps_lossless::emit_instant_spike_(
    const Time &origin, const long lag, const double spike_offs) {
