...
private:
   ...
   */
  void emit_spike_(Time const &origin, const long lag, const double t0,
                   const double dt);
  ...
  double get_y1_() const { return S_.y1_; }

  //! Read out state variable y2
  double get_y2_() const { return S_.y2_; }
...
inline port nest::iaf_psc_alpha_canon::send_test_event(Node &target,
                                                       rport receptor_type,
                                                       synindex, bool) {
  SpikeEvent e;
  e.set_sender(*this);
...
inline port iaf_psc_alpha_canon::handles_test_event(SpikeEvent &,
                                                    rport receptor_type) {
  if (receptor_type != 0) {
...
inline port iaf_psc_alpha_canon::handles_test_event(CurrentEvent &,
                                                    rport receptor_type) {
  if (receptor_type != 0) {
    throw UnknownReceptorType(receptor_type, get_name());
  }
...
inline port iaf_psc_alpha_canon::handles_test_event(DataLoggingRequest &dlr,
                                                    rport receptor_type) {
  if (receptor_type != 0) {
    throw UnknownReceptorType(receptor_type, get_name());
  }
...
inline void iaf_psc_alpha_canon::get_status(DictionaryDatum &d) const {
  P_.get(d);
  S_.get(d, P_);
...
inline void iaf_psc_alpha_canon::set_status(const DictionaryDatum &d) {
