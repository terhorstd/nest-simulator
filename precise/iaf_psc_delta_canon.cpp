 ...
 */
template <> void RecordablesMap<iaf_psc_delta_canon>::create() {
...
nest::iaf_psc_delta_canon::State_::State_()
    : U_(0.0) //  or U_ = U_reset_;
      ,
      I_(0.), last_spike_step_(-1), last_spike_offset_(0.0),
      is_refractory_(false), with_refr_input_(false) {}
...
void iaf_psc_delta_canon::update(Time const &origin, const long from,
                                 const long to) {
  assert(to >= 0);
...
void nest::iaf_psc_delta_canon::propagate_(const double dt) {
...
void nest::iaf_psc_delta_canon::emit_spike_(Time const &origin, const long lag,
...
void nest::iaf_psc_delta_canon::emit_instant_spike_(Time const &origin,
                                                    const long lag,
