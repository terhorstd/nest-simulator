...
template <> void RecordablesMap<iaf_psc_exp_ps>::create() {
...
nest::iaf_psc_exp_ps::State_::State_()
    : y0_(0.0), y1_ex_(0.0), y1_in_(0.0), y2_(0.0), is_refractory_(false),
      last_spike_step_(-1), last_spike_offset_(0.0) {}

nest::iaf_psc_exp_ps::Buffers_::Buffers_(iaf_psc_exp_ps &n) : logger_(n) {}
...
void nest::iaf_psc_exp_ps::update(const Time &origin, const long from,
                                  const long to) {
  assert(to >= 0);
  ...
  */
  ...
  for (long lag = from; lag < to; ++lag) {
    ...
    if (not B_.events_.get_next_spike(T, false, ev_offset, ev_weight,
                                      end_of_refract)) {
      ...
      */
      ...
      } while (B_.events_.get_next_spike(T, false, ev_offset, ev_weight,
                                         end_of_refract));
...
void nest::iaf_psc_exp_ps::propagate_(const double dt) {
  // dt == 0 may occur if two spikes arrive simultaneously;
...
void nest::iaf_psc_exp_ps::emit_spike_(const Time &origin, const long lag,
...
void nest::iaf_psc_exp_ps::emit_instant_spike_(const Time &origin,
                                               const long lag,
...
double nest::iaf_psc_exp_ps::bisectioning_(const double dt) const {
  double root = 0.0;
  ...
  while (fabs(P_.U_th_ - y2_root) > 1e-14) {
    ...
    y2_root = P20 * (P_.I_e_ + V_.y0_before_) + P21_ex * V_.y1_ex_before_ +
              P21_in * V_.y1_in_before_ + expm1_tau_m * V_.y2_before_ +
              V_.y2_before_;
  }
  return root;
}
