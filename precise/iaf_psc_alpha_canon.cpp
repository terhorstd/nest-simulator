 ...
 */
template <> void RecordablesMap<iaf_psc_alpha_canon>::create() {
...
nest::iaf_psc_alpha_canon::State_::State_()
    : y0_(0.0), y1_(0.0), y2_(0.0), y3_(0.0), is_refractory_(false),
      last_spike_step_(-1), last_spike_offset_(0.0) {}
...
void nest::iaf_psc_alpha_canon::calibrate() {
  ...
  V_.gamma_sq_ =
      1 / P_.c_m_ /
      ((1 / P_.tau_syn_ - 1 / P_.tau_m_) * (1 / P_.tau_syn_ - 1 / P_.tau_m_));
...
void nest::iaf_psc_alpha_canon::update(Time const &origin, const long from,
                                       const long to) {
  assert(to >= 0);
  ...
  */
  if (S_.y3_ >= P_.U_th_) {
    emit_instant_spike_(
        origin, from, V_.h_ms_ * (1 - std::numeric_limits<double>::epsilon()));
  }
  ...
  for (long lag = from; lag < to; ++lag) {
    ...
    if (not B_.events_.get_next_spike(T, true, ev_offset, ev_weight,
                                      end_of_refract)) { // No incoming spikes,
                                                         // handle with fixed
                                                         // propagator matrix.
...
void nest::iaf_psc_alpha_canon::propagate_(const double dt) {
  // needed in any case
...
void nest::iaf_psc_alpha_canon::emit_spike_(Time const &origin, const long lag,
...
void nest::iaf_psc_alpha_canon::emit_instant_spike_(Time const &origin,
                                                    const long lag,
...
// finds threshpassing
inline double nest::iaf_psc_alpha_canon::thresh_find_(double const dt) const {
  switch (P_.Interpol_) {
  case NO_INTERPOL:
    return dt;
  case LINEAR:
...
double nest::iaf_psc_alpha_canon::thresh_find1_(double const dt) const {
  double tau = (P_.U_th_ - V_.y3_before_) * dt / (S_.y3_ - V_.y3_before_);
  return tau;
}

// finds threshpassing via quadratic interpolation
double nest::iaf_psc_alpha_canon::thresh_find2_(double const dt) const {
...
double nest::iaf_psc_alpha_canon::thresh_find3_(double const dt) const {
