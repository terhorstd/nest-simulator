 ...
 */
template <> void RecordablesMap<iaf_psc_alpha_presc>::create() {
...
nest::iaf_psc_alpha_presc::Parameters_::Parameters_()
    : tau_m_(10.0) // ms
      ...
      ,
      U_min_(-std::numeric_limits<double>::infinity()), U_reset_(-70.0 - E_L_),
      Interpol_(iaf_psc_alpha_presc::LINEAR) {}

nest::iaf_psc_alpha_presc::State_::State_()
    : y0_(0.0), y1_(0.0), y2_(0.0), y3_(0.0), r_(0), last_spike_step_(-1),
...
void nest::iaf_psc_alpha_presc::State_::set(const DictionaryDatum &d,
                                            const Parameters_ &p,
                                            double delta_EL) {
...
void nest::iaf_psc_alpha_presc::update(Time const &origin, const long from,
                                       const long to) {
  assert(to >= 0);
...
void nest::iaf_psc_alpha_presc::handle(SpikeEvent &e) {
  ...
  B_.spike_y2_.add_value(Tdeliver,
                         spike_weight * dt * ps_e_TauSyn + spike_weight * dt);
...
double nest::iaf_psc_alpha_presc::update_y3_delta_() const {
  ...
  */
  ...
  const double ps_y2 = t_th * ps_e_TauSyn * V_.y1_before_ +
                       ps_e_TauSyn * V_.y2_before_ + t_th * V_.y1_before_ +
                       V_.y2_before_;
...
// finds threshpassing
inline double nest::iaf_psc_alpha_presc::thresh_find_(double const dt) const {
  switch (P_.Interpol_) {
  case NO_INTERPOL:
    return dt;
  case LINEAR:
  ...
  default:
    LOG(M_ERROR, "iaf_psc_alpha_presc::thresh_find_()",
...
double nest::iaf_psc_alpha_presc::thresh_find1_(double const dt) const {
  double tau = (P_.U_th_ - V_.y3_before_) * dt / (S_.y3_ - V_.y3_before_);
  return tau;
}

// finds threshpassing via quadratic interpolation
double nest::iaf_psc_alpha_presc::thresh_find2_(double const dt) const {
...
double nest::iaf_psc_alpha_presc::thresh_find3_(double const dt) const {
