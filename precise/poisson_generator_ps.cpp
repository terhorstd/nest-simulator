...
void nest::poisson_generator_ps::init_buffers_() {
  ...
  B_.next_spike_.resize(P_.num_targets_,
                        Buffers_::SpikeTime(Time::neg_inf(), 0));
}
...
void nest::poisson_generator_ps::update(Time const &T, const long from,
                                        const long to) {
...
void nest::poisson_generator_ps::event_hook(DSSpikeEvent &e) {
  ...
  if (nextspk.first.is_neg_inf()) {
    ...
    if (P_.dead_time_ > 0 and
        rng->drand() < P_.dead_time_ * P_.rate_ / 1000.0) {
