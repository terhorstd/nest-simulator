 ...
 */
template <typename T>
inline size_t median3_(const BlockVector<T> &vec, const size_t i,
...
void insertion_sort(BlockVector<T1> &vec_sort, BlockVector<T2> &vec_perm,
 ...
 */
template <typename T1, typename T2>
void quicksort3way(BlockVector<T1> &vec_sort, BlockVector<T2> &vec_perm,
                   const size_t lo, const size_t hi) {
  if (lo >= hi) {
    return;
  }
  ...
  size_t m =
      median3_(vec_sort, lo + std::rand() % (hi - lo),
               lo + std::rand() % (hi - lo), lo + std::rand() % (hi - lo));
