...
template <typename value_type_> class BlockVector;
template <typename value_type_, typename ref_, typename ptr_> class bv_iterator;
...
class bv_iterator {
  ...
  template <typename, typename, typename> friend class bv_iterator;
...
public:
   ...
   */
  bv_iterator(const BlockVector<value_type_> *, const size_t,
...
inline BlockVector<value_type_>::BlockVector()
    : blockmap_(std::vector<std::vector<value_type_>>(
          1, std::vector<value_type_>(max_block_size))),
      finish_(begin()) {}
...
template <typename value_type_>
inline const value_type_ &BlockVector<value_type_>::
...
template <typename value_type_>
inline void BlockVector<value_type_>::push_back(const value_type_ &value) {
...
template <typename value_type_> inline void BlockVector<value_type_>::clear() {
...
template <typename value_type_>
inline size_t BlockVector<value_type_>::size() const {
...
BlockVector<value_type_>::erase(const_iterator first, const_iterator last) {
  ...
  if (first == last) {
    ...
    new_final_block.erase(new_final_block.begin() + element_index,
                          new_final_block.end());
    ...
    blockmap_.erase(blockmap_.begin() + repl_it.block_index_ + 1,
                    blockmap_.end());
...
template <typename value_type_>
inline void BlockVector<value_type_>::print_blocks() const {
  std::cerr << "this: \t\t" << this << "\n";
  ...
  for (size_t block_index = 0;
       block_index != blockmap_.size() and seq_iter != end(); ++block_index) {
    std::cerr << "----------------------------------------------\n";
    auto &block = blockmap_[block_index];
    std::cerr << "Block size: " << block.size() << "\n";
    for (auto block_it = block.begin();
         block_it != block.end() and seq_iter != end(); ++block_it) {
      std::cerr << *block_it << " ";
      ++seq_iter;
    }
    std::cerr << "\n----------------------------------------------\n";
...
template <typename value_type_>
inline int BlockVector<value_type_>::get_max_block_size() const {
...
inline bv_iterator<value_type_, ref_, ptr_>::bv_iterator(
    ...
    : block_vector_(&block_vector), block_index_(0),
...
inline bv_iterator<value_type_, ref_, ptr_>::bv_iterator(const iterator &other)
    : block_vector_(other.block_vector_), block_index_(other.block_index_),
      block_it_(other.block_it_), current_block_end_(other.current_block_end_) {
}

template <typename value_type_, typename ref_, typename ptr_>
inline bv_iterator<value_type_, ref_, ptr_>::bv_iterator(
    const BlockVector<value_type_> *block_vector, const size_t block_index,
    ...
    : block_vector_(block_vector), block_index_(block_index),
      block_it_(block_it), current_block_end_(current_block_end) {}
...
template <typename value_type_, typename ref_, typename ptr_>
inline bool bv_iterator<value_type_, ref_, ptr_>::
operator==(const bv_iterator<value_type_, ref_, ptr_> &rhs) const {
...
template <typename value_type_, typename ref_, typename ptr_>
inline bool bv_iterator<value_type_, ref_, ptr_>::
operator<(const bv_iterator &rhs) const {
  return (block_index_ < rhs.block_index_ or
